    ...
    for fact in facts:
        ...
        if mode == "decision_time":
            ...
            if datetime.fromisoformat(fact["available_time"]) > datetime.fromisoformat(cutoff):
                continue
            if fact.get("event_time") and datetime.fromisoformat(
                fact["event_time"]
...
def evidence_report(facts, mode="current", cutoff=None):
    ...
    conflicts = [
        {"key": key, "fact_ids": [f["id"] for f in items]}
        for (key, _), items in groups.items()
        if len({(str(f["value"]).strip().casefold(), f.get("unit")) for f in items}) > 1
    ]
    usable = [
        f
        for f in eligible
        if f.get("assertion") in {"present", "absent"} and f.get("value") not in (None, "")
    ]
    symptom = any(
        f["key"].startswith("symptom.") and f.get("assertion") == "present" for f in usable
    )
    return {
        "total_facts": len(facts),
        "confirmed_facts": sum(bool(f.get("confirmed")) for f in facts),
        "eligible_facts": len(eligible),
        "unconfirmed_facts": sum(not f.get("confirmed") for f in facts),
        "excluded_by_time": sum(bool(f.get("confirmed")) for f in facts) - len(eligible),
        "missing_units": [
            f["id"] for f in usable if f["key"].startswith(("vital.", "lab.")) and not f.get("unit")
        ],
        "unknown_times": [f["id"] for f in facts if not f.get("available_time")],
        "potential_conflicts": conflicts,
        "clinical_review_ready": symptom and len(usable) >= 2,
        "evidence": [
            {
                "ref": f"F{i + 1}",
                "fact_id": f["id"],
...
def readiness(case_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    return {
        ...
        **report,
        "clinical_population_eligible": population_eligible,
        "clinical_review_ready": report["clinical_review_ready"] and population_eligible,
    }
...
def edit_case(
    ...
    return idempotent(
        db, user, f"case.update:{case_id}", key, body.model_dump(exclude_unset=True), run
    )
    ...
    def run():
        if case.version != body.expected_version:
            raise ApiError(
                ...
                "Reload the case before recording a clinical conclusion.",
            )
        ...
        if any(ref not in facts or not facts[ref]["confirmed"] for ref in body.fact_ids):
            raise ApiError(
                422, "CONFIRMED_EVIDENCE_REQUIRED", "Select current confirmed facts from this case."
            )
        if body.run_id:
            ...
            if (
                ...
                or job.tenant_id != user.tenant_id
                or job.case_version != case.version
                or job.status not in {"partial", "succeeded"}
                or not (job.result.get("ai") or {}).get("assessment")
            ):
                raise ApiError(
                    409, "CLINICAL_REVIEW_STALE", "A current completed clinical review is required."
                )
        ...
        result = create_record(
            ...
            {
                **body.model_dump(),
                "evaluated_version": body.expected_version,
