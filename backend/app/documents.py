...
def extract_in_process(data, filename):
    environment = {key: value for key, value in os.environ.items() if key.upper() in {'PATH', 'SYSTEMROOT', 'WINDIR', 'TEMP', 'TMP', 'TESSDATA_PREFIX'}}
    environment['PYTHONUTF8'] = '1'
    try:
        result = subprocess.run([sys.executable, '-m', 'app.parse_worker', Path(filename).suffix], input=data, capture_output=True, cwd=Path(__file__).resolve().parents[1], env=environment, timeout=60, creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
    ...
    if name.endswith('.pdf') and data.startswith(b'%PDF-'):
        ...
        for ti, table in enumerate(document.tables):
            for ri, row in enumerate(table.rows):
                pages.append({'page': len(pages) + 1, 'text': ' | '.join(c.text for c in row.cells), 'locator': f'table:{ti}:row:{ri}'})
        ...
        pages = [{'page': i // 3000 + 1, 'text': text[i:i + 3000]} for i in range(0, len(text), 3000)]
        if len(pages) > 30:
...
def extract_fixture_facts(pages):
    ...
    allowed = {'allergy.substance', 'medication.substance', 'imaging.side', 'lab.potassium', 'vital.spo2', 'vital.pulse', 'symptom.complaint'}
    drafts = []
    for page in pages:
        for match in re.finditer(r'(?m)^([a-z][a-z0-9_.]+)\s*=\s*([^\n]+)$', page['text']):
            key, value = match.groups()
            if key in allowed:
                drafts.append({'key': key, 'label': key, 'value': value.strip(), 'unit': None, 'confirmed': False, 'assertion': 'present', 'provenance': 'document', 'event_time': None, 'available_time': None, 'span': f'page:{page["page"]}:chars:{match.start()}-{match.end()}', 'order_status': 'not_applicable'})
    return drafts
