...
from .demo_catalog import (COMPARISON_COPY, EPISODES, INCIDENTS, PATIENT_NAMES,
                           STAFF, clinical_profile)
...
def seed_realistic(anchor=None):
    ...
    with SessionLocal() as db:
        ...
        db.add(other)
        db.flush()
        db.add(Case(tenant_id=other.tenant_id, owner_id=other.id, alias='ISOLATED-001', full_name='Лола Демирова', age=45, sex='female', summary='Изолированный синтетический случай другого центра. Первичная запись; данные ещё собираются.', demo=True, created_at=anchor - timedelta(days=3), updated_at=anchor - timedelta(days=3)))
        ...
        def record(kind, data, case, at, role='doctor', version=None, action=None):
            r = create_record(db, users[role], kind, {**data, 'synthetic': True, 'seed_version': SEED_VERSION}, case, version)
            r.created_at = at
            if action:
        ...
        def analysis(case, at, mode='current', cutoff=None):
            ...
            job = Job(tenant_id=TENANT, case_id=case.id, actor_id=users['doctor'].id, case_version=case.version, status='partial', stage='complete', payload={'snapshot': snapshot, 'mode': mode, 'decision_time': cutoff, 'include_ai': False, 'language': 'ru'}, created_at=at, started_at=at + timedelta(seconds=1), finished_at=at + timedelta(seconds=2), result={})
            db.add(job)
            db.flush()
            alerts = [record('alert', {**item, 'run_id': job.id}, case, at + timedelta(seconds=2)) for item in candidates]
            job.result = {'coverage': coverage, 'alert_ids': [a.id for a in alerts], 'ai': None, 'limitations': ['AI_NOT_REQUESTED'], 'rule_catalog_version': RULE_VERSION, 'seed_version': SEED_VERSION, 'provenance': 'synthetic_seed', 'engine': 'deterministic_rules', 'demo_only': True, 'model_id': 'synthetic-demo', 'language': 'ru'}
        ...
        def clinical_entries(case, profile, at, *, confirmed, dmed_source=None):
            entries = []
            for offset, category in enumerate(('subjective', 'objective', 'laboratory', 'instrumental', 'doctor_conclusion')):
                when = at + timedelta(minutes=offset * 4)
                text = profile[category]
                source = dmed_source or document(case, {
                    'subjective': 'Субъективные данные', 'objective': 'Объективный осмотр',
                    'laboratory': 'Лабораторные данные', 'instrumental': 'Инструментальные данные',
                    'doctor_conclusion': 'Заключение врача',
                }[category], [text] + ([profile['treatment']] if category == 'doctor_conclusion' else []), when)
                entries.append(record('clinical_entry', {
                    'category': category, 'text': text, 'confirmed': confirmed,
                    'diagnosis': text if category == 'doctor_conclusion' else '',
                    'treatment': profile['treatment'] if category == 'doctor_conclusion' else '',
                    'source_id': source.id, 'source_mode': 'dmed_demo' if dmed_source else 'document',
                    'author_name': users['doctor'].name, 'supersedes': None,
                    'event_time': when.isoformat(), 'available_time': when.isoformat(),
                }, case, when + timedelta(seconds=30), action='clinical_entry.created'))
        ...
        def comparison_examples(case, entries, profile, pulse, spo2, at):
            ...
            supporting_refs = [refs['laboratory']] if kind.startswith('tb_') else [refs['objective']]
            review_refs = [refs['laboratory'], refs['doctor_conclusion']] if kind.startswith('tb_') else [refs['subjective'], refs['doctor_conclusion']]
            if kind == 'side':
            ...
            for offset, language in enumerate(('ru', 'uz', 'en')):
                ...
                subject = copy[kind]
                result = {
                    'case_version': case.version, 'status': 'requires_clinician_review',
                    'summary': copy['intro'] + subject,
                    'diagnosis_review': {
                        'status': 'needs_review' if kind == 'tb_wrong' else 'consistent_with_data' if kind == 'tb_supported' or documented_regimen else 'insufficient_data',
                        'summary': subject[0].upper() + subject[1:] if kind.startswith('tb_') or documented_regimen else copy['diagnosis_generic'],
                        'refs': review_refs,
                    },
                    'treatment_review': {
                        'status': 'consistent_with_data' if documented_regimen else 'needs_review' if kind == 'allergy' else 'insufficient_data',
                        'summary': copy[kind + '_treatment'] if documented_regimen else subject[0].upper() + subject[1:] if kind == 'allergy' else copy['treatment'],
                        'refs': [refs['subjective'], refs['objective' if kind == 'hypertension' else 'laboratory'], refs['doctor_conclusion']] if documented_regimen else [refs['subjective'], refs['doctor_conclusion']],
                    },
                    'supporting': [{'text': copy['tb_support'] if kind.startswith('tb_') else copy['support'].format(pulse=pulse, spo2=spo2), 'refs': supporting_refs}],
                    'discrepancies': [{'text': subject[0].upper() + subject[1:], 'refs': review_refs}] if kind == 'tb_wrong' else [],
                    'questions': [copy['followup_question'] if documented_regimen else copy['tb_question'] if kind.startswith('tb_') else copy['question']],
                    'next_steps': [copy['next']],
                    'five_year_outlook': {'status': 'qualitative_only', 'summary': copy['outlook'], 'scenarios': [{'scenario': copy['scenario'], 'conditions': copy['condition'], 'monitoring': copy['monitor'], 'refs': [refs['subjective'], refs['objective']]}]},
                ...
                result.update(evidence=evidence, requires_clinician_review=True, validated_probability=False,
                              horizon_years=5, language=language, evaluated_conclusion_id=snapshot['evaluated_conclusion_id'])
                when = at + timedelta(seconds=offset * 10)
                job = Job(tenant_id=TENANT, case_id=case.id, actor_id=users['doctor'].id,
                          case_version=case.version, kind='clinical_comparison', status='succeeded', stage='complete',
                          payload={'snapshot': {**snapshot, 'language': language}, 'mode': 'current', 'include_ai': False,
                                   'review_focus': 'clinical_comparison', 'language': language, 'seed_only': True},
                          result={'comparison': result, 'ai': None, 'limitations': [], 'language': language,
                                  'model_id': 'synthetic-demo', 'model_revision': 'authored-fixture-v2',
                                  'prompt_version': 'authored-synthetic-v2', 'provenance': 'synthetic_seed',
                                  'engine': 'synthetic_seed/template', 'demo_only': True,
                                  'clinical_validation': 'not_validated', 'seed_version': SEED_VERSION},
                          created_at=when, started_at=when, finished_at=when + timedelta(seconds=1))
                db.add(job)
                db.flush()
                audit(users['doctor'], 'demo.comparison_prepared', job.id, when + timedelta(seconds=1))
                if language == 'ru':
                    record('notification', {'run_id': job.id, 'status': 'succeeded', 'read_by': [users['doctor'].id] if when < anchor - timedelta(days=2) else [], 'signature': 'clinical-comparison:' + job.id, 'demo_only': True}, case, when + timedelta(seconds=2))
        ...
        for index, (age, sex, complaint, diagnosis, scenario, pulse, spo2, potassium) in enumerate(EPISODES):
            ...
            if index in {8, 14}:
                ...
                profile['laboratory'] += f' Результаты доступны и проверены {(start + timedelta(hours=3, minutes=23)).isoformat()}.'
                profile['doctor_conclusion'] += f' Заключение после получения этих результатов: {(start + timedelta(hours=3, minutes=31)).isoformat()}.'
            case = Case(tenant_id=TENANT, owner_id=users['doctor'].id, alias=f'AT-{anchor:%y%m}-{index + 1:03d}', full_name=PATIENT_NAMES[index], patient_phone='', age=age, sex=sex, summary=complaint, diagnosis=diagnosis, demo=True, version=1, created_at=start, updated_at=start)
            db.add(case)
            db.flush()
            cases[index] = case
            ...
            if scenario == 'side':
                ...
                for series_uid, instances in phantom.items():
                    ...
                    first = instances[0]
                    metadata.append({'id': series_id, 'uid': series_uid, 'count': len(instances), **{k: first[k] for k in ['orientation', 'spacing', 'rows', 'columns', 'modality', 'description', 'body_part', 'photometric', 'frame_of_reference_uid'] if k in first}, 'positions': [i['position'] for i in instances]})
                study.data = {**study.data, 'series': metadata}
                record('imaging_report', {'study_id': study.id, 'expected_version': case.version,
                    ...
                    'report_source_id': None, 'report_quote': '', 'author_name': users['radiologist'].name,
                    'origin': 'synthetic_seed', 'demo_only': True}, case, start + timedelta(hours=3, minutes=5), role='radiologist', action='imaging.report_saved')
                if index in {1, 2, 13}:
            ...
            if scenario == 'import':
                ...
                dmed_source = record('source', {'name': 'DMED · синтетическая приёмная запись', 'type': 'dmed_demo', 'text': 'СИНТЕТИЧЕСКИЙ ИМПОРТ · нет реального подключения DMED\n' + '\n\n'.join(profile[key] for key in ('subjective', 'objective', 'laboratory', 'instrumental', 'doctor_conclusion', 'treatment')), 'pages': [], 'limitations': ['DEMO_INTEGRATION', 'SYNTHETIC_DATA'], 'external_version': 1}, case, at)
                imported = record('import', {'source_id': dmed_source.id, 'connection_id': connection.id, 'external_version': 1, 'mode': 'demo', 'status': 'awaiting_confirmation'}, case, at, action='dmed.imported')
            case.version += 1
            entries = clinical_entries(case, profile, start + timedelta(hours=3, minutes=15),
                                       confirmed=scenario not in {'intake', 'import'}, dmed_source=dmed_source)
            if dmed_source:
                imported.data = {**imported.data, 'clinical_entry_ids': [entry.id for entry in entries]}
            if scenario in {'intake', 'import'}:
                ...
                comparison_examples(case, entries, profile, pulse, spo2, start + timedelta(hours=5, minutes=35))
                for alert in alerts:
...
def seed_commerce(anchor):
    ...
    with SessionLocal() as db:
        ...
        for email, tenant, name in (
            ...
            owner = User(tenant_id=tenant, email=email, name=name, role='owner', password_hash=password_hash)
            db.add(owner)
            db.flush()
        ...
        main.plan_id, main.plan_name, main.price_uzs, main.doctor_limit = 'clinic10', 'Clinic 10', 1390000, 10
        ...
        other.plan_id, other.plan_name, other.price_uzs, other.doctor_limit = 'solo', 'Doctor', 150000, 1
        ...
        pending = Clinic(id='clinic-pending-demo', owner_id=owners['clinic-pending-demo'].id,
                         name='Shifo Plus · синтетическая заявка', created_at=anchor - timedelta(days=1),
                         internal_demo=False, doctor_limit=0)
        expired = Clinic(id='clinic-expired-demo', owner_id=owners['clinic-expired-demo'].id,
                         name='Mehr Medical · учебное продление', created_at=anchor - timedelta(days=65),
                         plan_id='clinic25', plan_name='Clinic 25', price_uzs=2390000, doctor_limit=25,
                         starts_at=anchor - timedelta(days=35), expires_at=anchor - timedelta(days=5), internal_demo=False)
        db.add_all([pending, expired])
        db.flush()
        method = db.scalar(select(PaymentMethod).order_by(PaymentMethod.name))
        for index, (clinic, plan_id, status, days) in enumerate(((main, 'clinic10', 'approved', 12), (pending, 'clinic10', 'pending', 0), (expired, 'clinic25', 'rejected', 2))):
            ...
            for row, line in enumerate(('SYNTHETIC DEMO - NOT A BANK RECEIPT', 'NO MONEY WAS TRANSFERRED', f'Fixture: {clinic.id}', f'Plan: {plan.name}', f'Illustrative amount: {plan.price_uzs:,} UZS', f'Reference: DEMO-SEED-{index + 1:03d}')):
                draw.text((55, 60 + row * 48), line, fill='#073e3a', font_size=23)
            receipt.save(path, 'PNG')
            created = anchor - timedelta(days=days, hours=4)
            req = SubscriptionRequest(tenant_id=clinic.id, actor_id=clinic.owner_id, plan_id=plan.id,
                plan_name=plan.name, price_uzs=plan.price_uzs, doctor_limit=plan.doctor_limit,
                payment_method_id=method.id, payment_method_name=method.name,
                payment_recipient=method.recipient, payment_card_last4=method.card_number.replace(' ', '')[-4:],
                payment_reference=f'SYNTHETIC-DEMO-{index + 1:03d}', is_demo=True,
                receipt_name=f'SYNTHETIC-NOT-PAYMENT-{index + 1}.png', receipt_path=relative,
                receipt_mime='image/png', receipt_hash=hashlib.sha256(path.read_bytes()).hexdigest(),
                status=status, created_at=created, version=1 if status == 'pending' else 2,
                reviewer_id=None if status == 'pending' else developer.id,
                reviewed_at=None if status == 'pending' else created + timedelta(hours=1),
                review_note='' if status == 'pending' else ('Учебная заявка одобрена. Это синтетическая демонстрация, реального платежа нет.' if status == 'approved' else 'Учебный пример отказа: в приложении отсутствует подтверждение реальной операции. Можно подать новую заявку.'),
            ...
            db.add(req)
            db.flush()
            db.add(Audit(tenant_id=clinic.id, actor_id=clinic.owner_id, action='billing.requested', resource_id=req.id, created_at=created))
            if status != 'pending':
                db.add(Audit(tenant_id=clinic.id, actor_id=developer.id, action='billing.' + status, resource_id=req.id, created_at=req.reviewed_at))
        db.commit()
    ...
    with SessionLocal() as db:
        ...
        for job in jobs:
            ...
            if job.kind == 'clinical_comparison':
                ...
                assert job.case_version == cases[job.case_id].version
                assert result['language'] == job.result['language'] == job.payload['language'] == snapshot['language']
                assert snapshot['evaluated_conclusion_id'] in by_id
                schema = {key: result[key] for key in ComparisonResult.model_fields}
                validate_comparison(ComparisonResult.model_validate(schema).model_dump(), snapshot, evidence_context(snapshot))
        ...
        for request in requests:
            ...
            assert hashlib.sha256((settings.storage_root / request.receipt_path).read_bytes()).hexdigest() == request.receipt_hash
            assert db.get(Clinic, request.tenant_id) and db.get(Plan, request.plan_id) and db.get(PaymentMethod, request.payment_method_id)
        return {'seed_version': SEED_VERSION, 'users': len(list(db.scalars(select(User)))), 'cases': len(cases), 'records': dict(Counter(r.kind for r in records)), 'analyses': len(jobs), 'analysis_kinds': dict(Counter(job.kind for job in jobs)), 'comparison_languages': dict(Counter(job.payload['language'] for job in jobs if job.kind == 'clinical_comparison')), 'clinics': len(list(db.scalars(select(Clinic)))), 'subscription_requests': dict(Counter(request.status for request in requests)), 'audit_events': len(list(db.scalars(select(Audit))))}
