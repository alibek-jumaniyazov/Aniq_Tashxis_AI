...
def subscription(clinic):
    active = clinic.status == "active" and (
        clinic.internal_demo or bool(clinic.expires_at and utc(clinic.expires_at) > now())
    )
    status = (
        "suspended"
    ...
    return {
        ...
        "price_uzs": clinic.price_uzs,
        "starts_at": iso(clinic.starts_at),
        "expires_at": iso(clinic.expires_at),
        "internal_demo": clinic.internal_demo,
    }
...
def require_entitlement(db, user, path):
    ...
    if user.role in {"developer", "owner"}:
        raise ApiError(
            403,
            "ROLE_REQUIRED",
            "This account manages the organization; use a clinical account for patient records.",
        )
    ...
    if clinic and not subscription(clinic)["active"]:
        raise ApiError(
            ...
            "An active clinic subscription is required.",
            {"account_url": "/account", "status": subscription(clinic)["status"]},
        )
...
def clinic_json(clinic):
    return {
        ...
        "created_at": iso(clinic.created_at),
    }
...
def request_json(db, request):
    ...
    return {
        **{
        ...
        "clinic_name": clinic.name if clinic else "",
        "receipt_url": f"/api/v1/billing/requests/{request.id}/receipt",
        "created_at": iso(request.created_at),
        "reviewed_at": iso(request.reviewed_at),
        "granted_starts_at": iso(request.granted_starts_at),
        "granted_expires_at": iso(request.granted_expires_at),
    }
...
def account_json(db, user, clinic=None):
    ...
    if not clinic:
        return {
            "managed": False,
            "can_use_workspace": user.role not in {"owner", "developer"},
            "is_clinic_owner": False,
            "clinic": None,
            "subscription": None,
            "usage": usage(db, user.tenant_id),
            "requests": [],
        }
    ...
    requests = (
        list(
            db.scalars(
                ...
                .where(SubscriptionRequest.tenant_id == clinic.id)
    ...
    return {
        "managed": True,
        "can_use_workspace": subscription(clinic)["active"]
        and user.role not in {"owner", "developer"},
        "is_clinic_owner": clinic.owner_id == user.id,
        "clinic": clinic_json(clinic),
        "subscription": subscription(clinic),
        "usage": usage(db, clinic.id),
        "requests": [request_json(db, item) for item in requests],
    }
...
def own_clinic(db, user, tenant_id=None):
    ...
    if not clinic or (
        user.role != "developer" and (clinic.id != user.tenant_id or clinic.owner_id != user.id)
    ):
        raise ApiError(
            403,
            "CLINIC_OWNER_REQUIRED",
            "Only the clinic owner can manage billing and team accounts.",
        )
    return clinic
...
def enforce_seat(db, clinic, additional=1):
    if not subscription(clinic)["active"]:
        raise ApiError(
            402, "SUBSCRIPTION_REQUIRED", "Activate a subscription before adding clinical accounts."
        )
    if usage(db, clinic.id)["doctors"] + additional > clinic.doctor_limit:
        raise ApiError(
            409,
            "DOCTOR_LIMIT_REACHED",
            "The plan doctor limit has been reached.",
            {"doctor_limit": clinic.doctor_limit},
        )
    ...
    try:
        if suffix == ".pdf":
            ...
            with warnings.catch_warnings():
                ...
                if (
                    image.format != ("PNG" if suffix == ".png" else "JPEG")
        ...
        raise ApiError(
            422, "INVALID_RECEIPT", "The receipt is damaged, unsafe, encrypted or too complex."
        )
    return mime, suffix
