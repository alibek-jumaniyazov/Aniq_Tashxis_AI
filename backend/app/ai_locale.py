...
def language_instruction(language):
    ...
    return (
        ...
        + " This includes summaries, findings, diagnosis and treatment reviews, "
...
_EN_WORDS = set(
    "the a an is are was were be been being this that these those of for from "
...
_UZ_WORDS = set(
    "va yoki ammo lekin uchun bilan haqida bu ushbu shu ham faqat agar emas yo'q "
...
_INTERNAL_STATUSES = re.compile(
    ...
    r"possible_discrepancy|not_assessable|not_evaluable)\b",
    re.IGNORECASE,
)
...
def prose_language_issues(texts: Iterable[str], language):
    ...
    for raw in texts:
        ...
        if _INTERNAL_STATUSES.search(unquoted):
            return [
                "AI_LANGUAGE_MISMATCH: Keep internal status strings only in the JSON status fields. "
        ...
        if target == "uz" and re.search(
            r"\btüberk(?:ülyoz|üloz|ulyoz)\w*", unquoted, re.IGNORECASE
        ):
            return [
                "AI_LANGUAGE_MISMATCH: Use standard Uzbek medical spelling such as sil or tuberkulyoz "
                "in generated prose; preserve explicitly quoted source terms. "
                + language_instruction(target)
            ]
        ...
        mismatch = len(letters) >= 12 and (latin + cyrillic) / len(letters) < 0.65
        if target == "ru":
            ...
            mismatch |= cyrillic >= 8 and cyrillic / len(letters) > 0.35
            if target == "uz":
