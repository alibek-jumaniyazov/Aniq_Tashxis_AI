...
async def upload_document(
    ...
    identity_confirmed: bool = Form(False),
    ...
    if not identity_confirmed:
        raise ApiError(
            ...
            "Confirm that the document belongs to this case and is authorized for demo use.",
        )
    ...
    if category and len(text.strip()) > 20000:
        raise ApiError(
            ...
            "Upload without a category, then select a clinical excerpt of at most 20000 characters.",
        )
    if category and not text.strip():
        raise ApiError(
            422,
            "DOCUMENT_TEXT_REQUIRED",
            "The document needs selectable text; scanned pages require transcription.",
        )
...
def extract_with_model(
    ...
    try:
        ...
        raise ApiError(
            503, str(error), "Local extraction is unavailable. Original text remains accessible."
        )
    except (ValueError, KeyError):
        raise ApiError(
            422, "MODEL_EXTRACTION_REJECTED", "Model output is not supported by the original text."
        )
    ...
    def run():
        ...
        existing = {
            (r.data["key"], str(r.data.get("value")), r.data.get("assertion"))
            for r in db.scalars(
                select(Record).where(Record.case_id == case.id, Record.kind == "fact")
            )
            if r.data.get("source_id") == source_id
        }
        accepted = []
        for fact in proposed:
            ...
            if (
                not page
                or fact.quote not in page["text"]
                or fact.value.casefold() not in fact.quote.casefold()
            ):
                raise ApiError(
                    422, "MODEL_EXTRACTION_REJECTED", "A proposed fact has no exact source excerpt."
                )
            ...
            for fact, offset in accepted:
                record = create_record(
                    ...
                    "fact",
                    {
                        ...
                        "confirmed": False,
                        "source_id": source_id,
                        "span": f"page:{fact.page}:chars:{offset}-{offset + len(fact.quote)}",
                        "event_time": None,
                        "available_time": None,
                        "order_status": "not_applicable",
                        "extraction_model": settings.model_id,
                        "extraction_revision": settings.model_revision,
                        "extraction_prompt_version": ai.EXTRACTION_PROMPT_VERSION,
                    },
                    case,
                )
...
async def imaging_upload(
    ...
    if not deidentified_confirmed:
        raise ApiError(
            ...
            "Confirm authorized de-identified or synthetic radiology data, including burned-in text.",
        )
    ...
    def run():
        ...
        study = create_record(
            ...
            {
                "name": "/".join(
                    sorted({i["modality"] for items in groups.values() for i in items})
                )
                + " · "
                + case.alias,
                "series": [],
                "analysis_status": "not_started",
                "reason": "SELECTED_FRAME_REVIEW_AVAILABLE",
                "mask_available": False,
                "synthetic_phantom": all(
                    i["synthetic_phantom"] for series in groups.values() for i in series
                ),
                "deidentified_confirmed_by": user.id,
            },
            case,
        )
        series_meta = []
            ...
            series_meta.append(
                {
                    "id": series_id,
                    "uid": series_uid,
                    "count": sum(i["frames"] for i in instances),
                    "frame_map": [
                        {"file_index": n, "pixel_frame": f}
    ...
    return idempotent(
        ...
        f"imaging:{case_id}",
        key,
        {"hash": hashlib.sha256(data).hexdigest(), "version": expected_version},
        run,
    )
...
def frame(
    ...
    series_id: str,
    index: int,
    center: float | None = Query(None, ge=-1000000, le=1000000),
    width: float | None = Query(None, ge=1, le=2000000),
    user: User = Depends(current_user),
    db: DBSession = Depends(get_db),
):
