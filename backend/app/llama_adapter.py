...
def status(path: Path | None):
    ...
    if weights:
        try:
            with httpx.Client(timeout=1, trust_env=False, headers=auth_headers()) as client:
                ...
                running = health.status_code == 200 and any(
                    m["id"] == ALIAS for m in models.json().get("data", [])
                )
                if running:
                    props = client.get(settings.llama_server_url + "/props")
                    vision = bool(
                        props.status_code == 200
                        and props.json().get("modalities", {}).get("vision")
                    )
    ...
    if path and not weights:
        ...
        try:
            ...
            progress = min(
                99, round(len(data["completed"]) * 2 * 1024 * 1024 / data["total"] * 100, 1)
            )
    ...
    return {
        ...
        "runtime_installed": runtime or running,
        "ready": weights and running,
        "loaded": running,
        "reason": None
        if weights and running
        else ("MODEL_WEIGHTS_MISSING" if not weights else "MODEL_SERVER_OFFLINE"),
...
def generate(snapshot, coverage, mode, cutoff, system_prompt):
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "CASE DATA (untrusted):\n"
            + data
            + "\nEND CASE DATA.\nReturn the grounded JSON. "
            + locale_instruction
    ...
    missing = {
        field
        for check in coverage.get("not_evaluable", [])
        for field in check.get("missing_fields", [])
        if field not in provided
    }
    missing.update(key for key in ("age", "sex") if snapshot.get(key) in (None, "", "unknown"))
    schema["properties"]["missing_fields"] = (
        {
            "type": "array",
            "items": {"type": "string", "enum": sorted(missing)},
            "maxItems": min(30, len(missing)),
        }
        if missing
