...
def model_context(snapshot):
    ...
    context = {
        key: snapshot[key]
        for key in ("version", "demo", "age", "sex", "summary", "diagnosis")
        if key in snapshot
    }
    fact_keys = (
        "key",
    ...
    context["facts"] = [
        {key: f[key] for key in fact_keys if key in f} for f in snapshot.get("facts", [])
    ]
    note_keys = ("text", "note_type", "provenance", "event_time")
    context["notes"] = [
        {key: n[key] for key in note_keys if key in n} for n in snapshot.get("notes", [])
    ]
    return context
...
def model_status():
    ...
    available = bool(
        ...
        and (list(path.glob("*.safetensors")) or list(path.glob("*.bin")))
    )
    packages = ["torch", "transformers", "accelerate", "psutil"] + (
        ["bitsandbytes"] if settings.model_quantization == "4bit" else []
    )
    ...
    return {
        ...
        "ready": bool(available and installed),
        "loaded": _model is not None,
        "reason": None
        if available and installed
        else ("MODEL_WEIGHTS_MISSING" if not available else "AI_RUNTIME_MISSING"),
...
def parse_output(text, version, allowed_sources, knowledge_ids=()):
    ...
    for concern in result.concerns:
        if (
            any(ref not in allowed_sources for ref in concern.source_ids)
            or concern.knowledge_id not in knowledge_ids
        ):
...
def validate_summary(result, snapshot):
    ...
    provided = {
        ...
        if f.get("assertion", "present") in {"present", "absent"}
...
def validate_known_questions(questions, snapshot):
    ...
    provided = {
        ...
        and f.get("assertion", "present") in {"present", "absent"}
        and f.get("value") not in (None, "")
    }
    ...
    for question in questions:
        ...
        if any(
            key in provided and re.fullmatch(pattern, text) for key, pattern in patterns.items()
        ):
            raise ValueError(
                "Question asks for an already documented value; ask only about genuinely missing details"
            )
...
is required for every output. Return ONLY the JSON matching the supplied schema."""
        prompt += "\nOutput schema:\n" + json.dumps(
            DocumentExtraction.model_json_schema(), ensure_ascii=False
        )
        ...
        result = DocumentExtraction.model_validate_json(
            complete(
                [
                    {"role": "system", "content": prompt},
                    {"role": "user", "content": json.dumps(document_pages, ensure_ascii=False)},
                ],
        ...
        for fact in result.facts:
            ...
            if not fact.unit:
                ...
                unit = re.search(
                    re.escape(fact.value) + r"\s*(/min|bpm|уд/мин|%|mmHg|мм рт\. ст\.)", fact.quote
                )
                if unit:
    ...
    try:
        ...
        if settings.ai_backend == "llama_cpp" or ai_provider.is_openai():
            ...
            for attempt in range(2):
                ...
                result = validate_summary(
                    parse_output(
                        text, snapshot["version"], {f["source_id"] for f in snapshot["facts"]}
                    ),
                    snapshot,
                )
                try:
                    ...
                    instruction += (
                        "\nCorrect the previous response language. "
                        + language_instruction(language)
                    )
                    continue
        ...
        if _model is None:
            ...
            if torch.cuda.is_available():
                ...
                gpu_budget = min(
                    int(settings.model_gpu_memory_gb * 1024**3), free_gpu - 768 * 1024**2
                )
        ...
        for attempt in range(2):
            ...
            with torch.inference_mode():
                output = _model.generate(
                    **inputs,
                    ...
                    do_sample=False,
                    max_time=settings.inference_timeout_seconds,
                )
            ...
            try:
                ...
                if attempt:
                    raise ModelUnavailable("AI_LANGUAGE_MISMATCH") from exc
                instruction += "\nCorrect the previous response language. " + language_instruction(
                    language
                )
                continue
            return {**result, "language": language, **ai_provider.result_metadata()}
