...
def _error(response):
    ...
    if response.status_code == 429:
        return 'OPENAI_QUOTA_EXCEEDED' if code in {'insufficient_quota', 'billing_hard_limit_reached'} else 'OPENAI_RATE_LIMIT'
...
def status():
    ...
    verified = False
    if configured:
        if _health.get('identity') != _identity() or monotonic() - _health.get('checked_at', 0) > 60:
            try:
                with httpx.Client(base_url=API_ORIGIN, timeout=4, trust_env=False, follow_redirects=False,
                                  headers={'Authorization': 'Bearer ' + _key()}) as client:
    ...
    return {'provider': 'openai', 'backend': 'openai', 'model_id': settings.openai_model,
            'revision': 'api-managed', 'configured': configured, 'ready': configured and verified,
            'connection_verified': verified, 'loaded': False, 'vision_ready': configured and verified,
            'weights_available': False, 'runtime_installed': True, 'reason': reason,
            'clinical_validation': 'not_validated', 'data_processing': 'openai_cloud',
            'response_storage': False, 'prompt_version': 'openai-responses-1.0'}
...
def complete(messages, schema, *, max_tokens=None, timeout_seconds=None):
    ...
    body = {'model': settings.openai_model, 'input': response_input(messages), 'store': False,
            ...
            'text': {'format': {'type': 'json_schema', 'name': 'clinical_result', 'strict': True,
                                'schema': strict_schema(schema)}},
            'truncation': 'disabled'}
    try:
        with httpx.Client(base_url=API_ORIGIN, timeout=budget, trust_env=False, follow_redirects=False,
                          headers={'Authorization': 'Bearer ' + _key()}) as client:
        ...
        if response.status_code != 200:
            ...
            if reason in {'OPENAI_AUTH_ERROR', 'OPENAI_QUOTA_EXCEEDED', 'OPENAI_MODEL_UNAVAILABLE', 'OPENAI_UNAVAILABLE'}:
                _remember(reason)
