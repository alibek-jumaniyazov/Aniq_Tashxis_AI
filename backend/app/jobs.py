...
def process_job(job_id):
    with SessionLocal() as db:
        claimed = db.execute(update(Job).where(Job.id == job_id, Job.status == 'queued').values(status='running', stage='rules', started_at=now()))
        db.commit()
        ...
        try:
            ...
            if job.payload.get('include_ai'):
                ...
                if mode == 'decision_time':
                    ...
                    model_snapshot['notes'] = [n for n in snapshot.get('notes', []) if datetime.fromisoformat(n['created_at']).replace(tzinfo=timezone.utc) <= datetime.fromisoformat(cutoff)]
                try:
            ...
            if not user or not user.active:
                raise PermissionError('USER_REVOKED')
            case = access_case(db, user, job.case_id)
            finish_status = 'partial' if coverage['not_evaluable'] or result['limitations'] else 'succeeded'
            claimed_finish = db.execute(update(Job).where(Job.id == job.id, Job.status == 'running').values(status=finish_status, stage='complete', finished_at=now()))
            if not claimed_finish.rowcount:
                db.rollback()
                return
            for item in candidates:
                existing = list(db.scalars(select(Record).where(Record.kind == 'alert', Record.case_id == case.id, Record.case_version == job.case_version)))
                record = next((r for r in existing if r.data['rule_id'] == item['rule_id'] and sorted(r.data['fact_ids']) == sorted(item['fact_ids'])), None)
                if not record:
                    record = create_record(db, user, 'alert', {**item, 'run_id': job.id}, case, job.case_version)
                result['alert_ids'].append(record.id)
            job.result = result
            signature = hashlib.sha256(json.dumps([job.case_version, mode, result], sort_keys=True).encode()).hexdigest()
            notifications = list(db.scalars(select(Record).where(Record.kind == 'notification', Record.case_id == case.id)))
            if not any(r.data.get('signature') == signature for r in notifications):
                create_record(db, user, 'notification', {'run_id': job.id, 'read': False, 'status': finish_status, 'signature': signature}, case, job.case_version)
