...
EPISODES = [
    (
        58,
        "male",
        "Дискомфорт в груди при нагрузке, повторный приём.",
        "Уточнение анамнеза и листа назначений",
        "allergy",
        84,
        97,
        "4.2",
    ...
    (
        42,
        "female",
        "Кашель в течение пяти дней, утомляемость.",
        "Сопоставление направлений и описаний исследования",
        "side",
        88,
        96,
        "4.0",
    ...
    (
        67,
        "male",
        "Контроль после обследования органов грудной клетки.",
        "Уточнение стороны в двух документах",
        "side",
        76,
        97,
        "4.5",
    ...
    (
        35,
        "female",
        "Первичное обращение: эпизоды сердцебиения.",
        "Приёмная запись ожидает подтверждения врача",
        "intake",
        92,
        98,
        None,
    ...
    (
        61,
        "female",
        "Контроль артериальной гипертензии; самочувствие стабильное.",
        "Гипертензия, плановое наблюдение; исправлена единица калия",
        "revision",
        74,
        98,
        "4.3",
    ...
    (
        49,
        "male",
        "Повторный приём по поводу кашля.",
        "Результат лаборатории поступил после решения",
        "late",
        82,
        97,
        "4.1",
    ...
    (
        54,
        "female",
        "Сверка лекарственного анамнеза перед обследованием.",
        "Проверка совпадения вещества в записях",
        "allergy",
        78,
        98,
        "4.4",
    ...
    (
        72,
        "male",
        "Контрольное описание исследования грудной клетки.",
        "Проверка стороны в заключении и направлении",
        "side",
        72,
        96,
        "4.6",
    ...
    (
        46,
        "female",
        "Кашель шесть недель, ночная потливость, снижение массы на 5 кг.",
        "Туберкулёз лёгких, бактериологически подтверждённый",
        "complete",
        90,
        95,
        "4.0",
    ...
    (
        63,
        "male",
        "Плановый визит, уточнение аллергологического анамнеза.",
        "Отменённое назначение сохранено в истории",
        "cancelled",
        80,
        97,
        "4.7",
    ...
    (
        38,
        "male",
        "Дискомфорт при глубоком вдохе, первичное обследование.",
        "Описание и направление требуют сверки",
        "side",
        86,
        98,
        "4.2",
    ...
    (
        56,
        "female",
        "Контроль сахарного диабета; снижение утомляемости.",
        "Сахарный диабет 2 типа, контроль лечения; время получения внешнего калия неизвестно",
        "unknown_time",
        76,
        98,
        "4.1",
    ...
    (
        ...
        "Проверка лекарственных записей завершена",
        "allergy",
        78,
        98,
        "4.3",
    ...
    (
        69,
        "female",
        "Сопоставление повторного и предыдущего описаний.",
        "Повторная экспертная оценка документации",
        "side",
        74,
        96,
        "4.4",
    ...
    (
        ...
        "male",
        "Кашель шесть недель, ночная потливость, снижение массы на 5 кг.",
        "Внебольничная пневмония; туберкулёз исключён",
        "complete",
        92,
        95,
        "4.1",
    ...
    (
        31,
        "female",
        "Кашель после перенесённой инфекции, новый приём.",
        "Нужно проверить исходный текст и время событий",
        "intake",
        86,
        99,
        None,
    ...
    (
        65,
        "male",
        "Контрольное обследование с документами из другой системы.",
        "Синтетический импорт ожидает сверки",
        "import",
        80,
        97,
        "4.5",
    ...
    (
        47,
        "female",
        "Повторный приём, сохраняется дискомфорт в груди.",
        "Дополнительные сведения ожидаются от врача",
        "allergy",
        82,
        98,
        "4.2",
...
INCIDENTS = {
    0: (
        "confirmed",
        "В листе аллергии и активном назначении совпадает вещество.",
        "Сверены обе исходные записи. Подтверждено расхождение документации; клинический исход не оценивается.",
    ),
    1: (
        "closed",
        "Сторона в направлении отличается от стороны в заключении.",
        "Обе цитаты проверены; расхождение записи подтверждено.",
    ),
    2: (
        "corrective_actions",
        "Несогласованные обозначения стороны в двух документах.",
        "Расхождение подтверждено. Назначена повторная сверка первичных документов.",
    ),
    4: (
        "confirmed",
        "При переносе лабораторного результата пропущена единица.",
        "История версий и бланк подтверждают пропуск. В новой версии единица восстановлена.",
    ),
    5: (
        "confirmed",
        "Результат лаборатории был недоступен на момент исходного решения.",
        "Проверены время события и время поступления. Поздний результат нельзя переносить в прежний контекст.",
    ),
    6: (
        "awaiting_explanation",
        "Нужно уточнить актуальность назначения и аллергологическую запись.",
        "Запрошено пояснение автора записи; окончательное решение ещё не принято.",
    ),
    7: ("under_review", "Требуется независимая сверка стороны исследования.", ""),
    9: (
        "not_confirmed",
        "Предполагаемое совпадение аллергии и назначения.",
        "Назначение отменено до проверки. Активного совпадения нет; предполагаемое нарушение не подтверждено.",
    ),
    10: (
        "insufficient_information",
        "Различаются сведения о стороне исследования.",
        "Нет подписанного уточнения направления; для решения требуется дополнительный документ.",
    ),
    11: ("under_review", "Не установлено время доступности лабораторной записи.", ""),
    12: (
        "closed",
        "Проверка совпадения вещества в двух записях.",
        "Совпадение подтверждено по источникам; рассмотрение документации завершено.",
    ),
    13: (
        "confirmed",
        "Повторное расхождение стороны в документах.",
        "Эксперт сопоставил два источника. Требуется корректировка процесса переноса данных.",
    ),
    17: (
        "awaiting_explanation",
        "Проверка записи об аллергии перед закрытием эпизода.",
        "Запрошены исходный лист и подтверждение актуальности назначения.",
    ),
}
...
CLINICAL_CONTEXT = [
    (
        "Симптомы возникают при быстрой ходьбе и проходят в покое. В анамнезе сыпь после амоксициллина.",
        "АД 138/84 мм рт. ст.; температура 36.6 °C.",
        "Гемоглобин 136 г/л; лейкоциты 6.8 ×10⁹/л; креатинин 82 мкмоль/л.",
        "ЭКГ: синусовый ритм; острые изменения ST в учебном описании не зарегистрированы.",
        "Боль в грудной клетке требует уточнения причины; отдельно сверить назначение амоксициллина с аллергологическим листом.",
    ),
    (
        "Кашель начался после переохлаждения; мокрота скудная, одышка при нагрузке.",
        "АД 122/76 мм рт. ст.; температура 37.6 °C; единичные хрипы.",
        "Гемоглобин 128 г/л; лейкоциты 10.8 ×10⁹/л; СРБ 24 мг/л.",
        "Текст направления указывает левую сторону, текст заключения — правую. Прикреплённый DICOM — геометрический фантом, не снимок этой патологии.",
        "Респираторная инфекция; локализация по документам не согласована.",
    ),
    (
        "После обследования жалобы уменьшились; одышка при подъёме по лестнице сохраняется.",
        "АД 132/80 мм рт. ст.; температура 36.7 °C; дыхание без грубых хрипов.",
        "Гемоглобин 140 г/л; лейкоциты 7.2 ×10⁹/л; СРБ 7 мг/л.",
        "В направлении и описании разные стороны. Файл DICOM — технический фантом для просмотра, не подтверждает текстовую находку.",
        "Уточнение локализации по оригиналам радиологических документов.",
    ),
    (
        "Сердцебиение возникает эпизодически вечером; длительность и провоцирующие факторы уточняются.",
        "АД 118/74 мм рт. ст.; температура 36.5 °C; сознание ясное.",
        "Лабораторные результаты ещё не представлены; значения не подставлены.",
        "ЭКГ и суточное мониторирование пока не приложены.",
        "Эпизоды сердцебиения: заключение является неподтверждённым черновиком.",
    ),
    (
        "Самочувствие стабильное. Внешний лабораторный бланк перенесён с пропуском единицы калия.",
        "АД 130/80 мм рт. ст.; температура 36.6 °C; отёки не описаны.",
        "Гемоглобин 131 г/л; креатинин 79 мкмоль/л. Единица калия восстановлена по исправленному бланку.",
        "ЭКГ: синусовый ритм, документированных острых изменений нет.",
        "Плановое наблюдение; использовать исправленный лабораторный результат с сохранением истории.",
    ),
    (
        "Кашель сохраняется десять дней. Внешняя лаборатория прислала результат после первичного решения.",
        "АД 126/78 мм рт. ст.; температура 37.0 °C.",
        "Гемоглобин 142 г/л; лейкоциты 8.6 ×10⁹/л; СРБ 11 мг/л. Калий доступен только после первичного осмотра.",
        "Рентгенография: в учебном текстовом описании свежая очаговая тень не указана; изображение не приложено.",
        "Затяжной кашель; пересмотр с учётом поздно поступивших данных.",
    ),
    (
        "Перед обследованием пациентка сообщила о сыпи после амоксициллина. Актуальность назначения уточняется.",
        "АД 128/82 мм рт. ст.; температура 36.5 °C; текущая сыпь не описана.",
        "Гемоглобин 134 г/л; лейкоциты 6.3 ×10⁹/л; креатинин 76 мкмоль/л.",
        "ЭКГ: синусовый ритм. Результат относится к документу, не к DICOM-анализу.",
        "Сверка аллергии и активного листа назначений; требуется пояснение автора.",
    ),
    (
        "Плановое контрольное обследование; одышка при обычной нагрузке без резкого ухудшения.",
        "АД 140/86 мм рт. ст.; температура 36.6 °C.",
        "Гемоглобин 138 г/л; лейкоциты 7.5 ×10⁹/л; СРБ 5 мг/л.",
        "Текстовые документы содержат разные стороны; клинических снимков нет. DICOM-фантом показывает только возможности просмотрщика.",
        "Требуется независимое уточнение стороны исследования.",
    ),
    (
        "Кашель шесть недель, ночная потливость, похудение на 5 кг. Контакт с больным туберкулёзом не установлен.",
        "АД 116/72 мм рт. ст.; температура 37.8 °C; масса 58 кг; локальные хрипы справа вверху.",
        "Мокрота: Xpert MTB/RIF Ultra — MTB detected MEDIUM; rifampicin resistance NOT detected. Микроскопия КУМ 2+. Посев и расширенная лекарственная чувствительность ещё не готовы.",
        "Текст КТ: инфильтрация S1/S2 справа, полость 18 мм. Клинические DICOM-изображения не приложены.",
        "Туберкулёз лёгких, бактериологически подтверждённый. Устойчивость к рифампицину не обнаружена; полная лекарственная чувствительность неизвестна.",
    ),
    (
        "При плановом визите уточнена аллергия на кларитромицин; препарат в листе уже отменён.",
        "АД 134/82 мм рт. ст.; температура 36.6 °C; новых жалоб нет.",
        "Гемоглобин 145 г/л; лейкоциты 6.9 ×10⁹/л; креатинин 85 мкмоль/л.",
        "ЭКГ: синусовый ритм; дополнительных изображений нет.",
        "Историческое назначение отменено до проверки; текущий конфликт назначения не подтверждён.",
    ),
    (
        "Боль при глубоком вдохе появилась три дня назад; травму отрицает.",
        "АД 124/78 мм рт. ст.; температура 36.8 °C; дыхание симметрично.",
        "Гемоглобин 146 г/л; лейкоциты 8.1 ×10⁹/л; СРБ 9 мг/л.",
        "Сторона в направлении и текстовом заключении отличается. Подписанное уточнение не получено; DICOM является фантомом.",
        "Причина дискомфорта и сторона исследования нуждаются в уточнении.",
    ),
    (
        "Повторная консультация с внешними анализами; дата исследования есть, время получения документа неизвестно.",
        "АД 128/78 мм рт. ст.; температура 36.6 °C.",
        "Гемоглобин 132 г/л; креатинин 80 мкмоль/л. Время доступности результата калия не установлено.",
        "ЭКГ: синусовый ритм; динамика по предыдущему исследованию не представлена.",
        "Оценка текущих данных возможна; ретроспективную доступность результата установить нельзя.",
    ),
    (
        "После обследования выполнена повторная сверка аллергии и лекарственного листа; зарегистрирована аллергия на амоксициллин.",
        "АД 126/80 мм рт. ст.; температура 36.6 °C; новых симптомов нет.",
        "Гемоглобин 143 г/л; лейкоциты 6.7 ×10⁹/л; креатинин 81 мкмоль/л.",
        "ЭКГ: синусовый ритм. Подтверждения клинического вреда в истории нет.",
        "Документальное совпадение вещества подтверждено; оценка исхода и реального приёма препарата отсутствует.",
    ),
    (
        "Повторный контроль описаний исследования; клинически без резкого изменения.",
        "АД 136/84 мм рт. ст.; температура 36.6 °C.",
        "Гемоглобин 127 г/л; лейкоциты 7.0 ×10⁹/л; СРБ 6 мг/л.",
        "Сохраняется несовпадение стороны между документами. Технический фантом просмотрен отдельно от медицинского текста.",
        "Повторная экспертная сверка локализации и первичных документов.",
    ),
    (
        "Кашель шесть недель, ночная потливость, похудение на 5 кг. До лабораторного ответа предполагалась пневмония.",
        "АД 120/76 мм рт. ст.; температура 37.9 °C; масса 64 кг; хрипы справа вверху.",
        "Мокрота: Xpert MTB/RIF Ultra — MTB detected MEDIUM; rifampicin resistance NOT detected. Микроскопия КУМ 2+. Посев и расширенная лекарственная чувствительность ещё не готовы.",
        "Текст КТ: инфильтрация S1/S2 справа, полость 18 мм. Клинические DICOM-изображения не приложены.",
        "Внебольничная пневмония; туберкулёз исключён. Эта учебная запись намеренно противоречит уже доступному положительному результату.",
    ),
    (
        "Кашель после перенесённой инфекции; начало, динамика и лекарственный анамнез уточняются.",
        "АД 114/72 мм рт. ст.; температура 36.7 °C; дыхание без локальных хрипов.",
        "Бланк лаборатории ещё не предоставлен; результат не подтверждён.",
        "Инструментальные документы ещё не загружены.",
        "Предварительный контекст: кашель после инфекции. До сверки данных заключение не подтверждено.",
    ),
    (
        "Синтетическая внешняя карта: контрольный приём, жалобы на периодическую утомляемость; лекарственный анамнез требует сверки.",
        "Внешний осмотр: АД 138/82 мм рт. ст.; температура 36.6 °C. Данные ещё не подтверждены локальным врачом.",
        "Внешний бланк: гемоглобин 137 г/л; креатинин 88 мкмоль/л. Импорт ожидает сверки.",
        "Внешний текст ЭКГ: синусовый ритм. Оригинальные изображения не переданы.",
        "Заключение из синтетической DMED-карты ожидает проверки врачом; реальное соединение с DMED отсутствует.",
    ),
    (
        "Повторный приём: дискомфорт в груди и указание на прежнюю реакцию на кларитромицин.",
        "АД 128/80 мм рт. ст.; температура 36.6 °C; текущая аллергическая реакция не описана.",
        "Гемоглобин 130 г/л; лейкоциты 6.6 ×10⁹/л; креатинин 78 мкмоль/л.",
        "ЭКГ: синусовый ритм. Динамические исследования не приложены.",
        "Проверка совпадения вещества; нужны сведения об актуальном приёме и прежней реакции.",
    ),
]
...
def clinical_profile(index, pulse, spo2, potassium, scenario):
    ...
    if index == 4:
        ...
        objective = (
            "АД 128/78 мм рт. ст.; температура 36.6 °C; масса 74 кг; признаков обезвоживания нет."
        )
    ...
    treatment = (
        "План: консультация фтизиатра, проверка посева и полной чувствительности; лекарственная схема в этой записи не представлена."
        if index == 8
        else "План лечения записан как лечение пневмонии; препарат и схема не указаны. Результат MTB уже доступен автору записи."
        if index == 14
        else "План: сверить первичные документы и оценить динамику на повторном приёме. Полная схема лечения и сведения о фактическом приёме не представлены."
    )
    if scenario in {"allergy", "cancelled"}:
        substance = "Амоксициллин" if index % 2 == 0 else "Кларитромицин"
        treatment = f"В листе назначений: {substance}; статус: " + (
            "отменено до проверки."
            if scenario == "cancelled"
            else "активно; доза и фактический приём не подтверждены."
        )
