        ...
        "@graph": [
            {
                ...
                "name": "AniqTashxis.ai",
                "url": f"{origin}/",
                "logo": f"{origin}/brand/aniqtashxis-mark.svg",
...
def page_metadata(request: Request) -> str:
    ...
    title = (
        copy["title"]
        if public
        else f"{copy['routes'].get(segment, copy['routes']['workspace'])} · AniqTashxis.ai"
    )
    ...
    lines = [
        ...
        '<script id="seo-config" type="application/json">'
        + json_script({"origin": seo_settings.site_url, "indexing": seo_settings.indexable})
        + "</script>",
    ]
    if public:
        for name, value in {
            "og:type": "website",
            "og:site_name": "AniqTashxis.ai",
            "og:title": title,
            "og:description": description,
            "og:locale": {"ru": "ru_RU", "uz": "uz_UZ", "en": "en_US"}[language],
        }.items():
        ...
        if seo_settings.site_url:
            ...
            lines.extend(
                [
                    ...
                    '<meta property="og:image:alt" content="AniqTashxis.ai">',
                    f'<meta name="twitter:image" content="{image}">',
                    '<meta name="twitter:image:alt" content="AniqTashxis.ai">',
                ]
            )
            ...
            lines.append(
                ...
                + json_script(structured_data(language))
                + "</script>"
            )
    return "\n".join(lines)
...
def fallback_content(request: Request) -> str:
    ...
    languages = " · ".join(
        f'<a lang="{code}" href="/{"?lang=" + code if code != "ru" else ""}">{label}</a>'
        for code, label in [("ru", "Русский"), ("uz", "O‘zbekcha"), ("en", "English")]
    )
    return (
        f'<main class="public-fallback"><header><img src="/brand/aniqtashxis-mark.svg" width="64" height="64" alt="">'
...
def render_frontend(index_path: Path, request: Request) -> HTMLResponse:
    ...
    html = re.sub(
        r"<!-- FALLBACK:START -->.*?<!-- FALLBACK:END -->",
        lambda _: "<!-- FALLBACK:START -->" + fallback_content(request) + "<!-- FALLBACK:END -->",
        html,
        flags=re.S,
    )
...
def apply_indexing_headers(request: Request, response: Response) -> None:
    ...
    public_asset = path.startswith(("/assets/", "/brand/")) or path in {
        "/mark.svg",
        "/robots.txt",
        "/sitemap.xml",
        "/site.webmanifest",
    ...
    if (
        not seo_settings.indexable
        or (path != "/" and not public_asset)
...
def sitemap() -> Response:
    ...
    if seo_settings.indexable:
        alternatives = "".join(
            f'<xhtml:link rel="alternate" hreflang="{code}" href="{escape(public_url("ru" if code == "x-default" else code), quote=True)}"/>'
            for code in (*LANGUAGES, "x-default")
        )
        entries = [
            f"<url><loc>{escape(public_url(language))}</loc>{alternatives}</url>"
    ...
    return Response(
        body, media_type="application/xml", headers={"Cache-Control": "public, max-age=300"}
    )
