...
def complete(messages, schema, *, max_tokens=None, timeout_seconds=None):
    if is_openai():
        ...
        return openai_adapter.complete(messages, schema, max_tokens=max_tokens, timeout_seconds=timeout_seconds)
...
def result_metadata():
    if is_openai():
        return {'provider': 'openai', 'provenance': 'openai_api', 'model_id': settings.openai_model,
                'model_revision': 'api-managed', 'ai_backend': 'openai', 'quantization': None,
                'data_processing': 'openai_cloud', 'clinical_validation': 'not_validated'}
    return {'provider': 'local_medgemma', 'provenance': 'local_medgemma', 'model_id': settings.model_id,
            'model_revision': settings.model_revision, 'ai_backend': settings.ai_backend,
            'quantization': settings.model_quantization, 'data_processing': 'local',
