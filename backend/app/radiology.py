...
from .security import ApiError, access_case, access_record, audit, bump_case, create_record, current_user, idem_key, idempotent, require_role, serialize
    ...
    if body.analysis_scope == 'study_sample':
        ...
        for fraction in (0, 1, .5, .25, .75):
            for series in ordered:
                ...
                if len(chosen) < 4 and not any(s['id'] == series['id'] and i == index for s, i in chosen):
                    chosen.append((series, index))
    return [{'ref': f'F{i + 1}', 'series_id': s['id'], 'series_number': next(j + 1 for j, value in enumerate(study.data['series']) if value['id'] == s['id']), 'frame_index': index, 'series_frames': s['count'], 'modality': s.get('modality', 'CT'), 'center': body.center if s['id'] == selected['id'] else s.get('window_center', 40), 'width': body.width if s['id'] == selected['id'] else s.get('window_width', 400)} for i, (s, index) in enumerate(chosen)]


def prepare_ai_frame(study, frame):
    raw = render_frame(study, frame['series_id'], frame['frame_index'], frame['center'], frame['width'])
    ...
    return output.getvalue(), {'original_size': list(original_size), 'input_size': list(image.size), 'constant_image': extrema[1] - extrema[0] <= 1}
...
def validate_sample_review(review, frames):
    ...
    if all(item.quality == 'unreadable' for item in review.frame_assessments) and review.observations:
...
def validate_report_source(db, user, case, body):
    ...
    if source.case_id != case.id:
        raise ApiError(422, 'REPORT_SOURCE_MISMATCH', 'The report source must belong to this patient.')
    if ' '.join(body.report_quote.split()) not in ' '.join(source.data.get('text', '').split()):
        raise ApiError(422, 'REPORT_QUOTE_MISMATCH', 'The report quote must be present in its source.')
...
def reports(study_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    rows = db.scalars(select(Record).where(Record.case_id == study.case_id, Record.tenant_id == user.tenant_id, Record.kind == 'imaging_report').order_by(Record.created_at.desc()))
...
def save_report(study_id: str, body: ReportRequest, user: User = Depends(current_user), db: DBSession = Depends(get_db), key=Depends(idem_key)):
    ...
    def run():
        ...
        result = create_record(db, user, 'imaging_report', {**body.model_dump(), 'study_id': study_id, 'author_name': user.name, 'origin': 'clinician_authored'}, case)
...
def measure(study_id: str, body: MeasurementRequest, user: User = Depends(current_user), db: DBSession = Depends(get_db), key=Depends(idem_key)):
    require_role(user, 'radiologist')
    study = access_record(db, user, study_id, ['study'])
    case = access_case(db, user, study.case_id)
    series = series_for(study, body)
    if not series.get('spacing'):
        raise ApiError(422, 'PIXEL_SPACING_REQUIRED', 'No calibrated PixelSpacing is available for this image.')
    if any(p.x >= series['columns'] or p.y >= series['rows'] for p in body.points):
        raise ApiError(422, 'POINT_OUTSIDE_IMAGE', 'Measurement points must be inside the original image.')
    def run():
        if case.version != body.expected_version:
            raise ApiError(409, 'CASE_VERSION_CONFLICT', 'Refresh the case before measuring.')
        a, b = body.points
        distance = math.hypot((b.x - a.x) * series['spacing'][1], (b.y - a.y) * series['spacing'][0])
        result = create_record(db, user, 'imaging_measurement', {**body.model_dump(), 'study_id': study_id, 'distance_mm': round(distance, 3), 'author_name': user.name, 'method': 'two_point_pixel_spacing'}, case)
...
def analyse(study_id: str, body: ImageAnalysisRequest, user: User = Depends(current_user), db: DBSession = Depends(get_db), key=Depends(idem_key)):
    from . import jobs
    require_role(user, 'doctor', 'radiologist')
    study = access_record(db, user, study_id, ['study'])
    case = access_case(db, user, study.case_id)
    ...
    if len(body.radiologist_report) > report_limit:
        raise ApiError(422, 'IMAGE_REPORT_TOO_LONG', f'Use up to {report_limit} characters for this comparison. The full saved report is preserved.')
    def run():
        if case.version != body.expected_version:
            raise ApiError(409, 'CASE_VERSION_CONFLICT', 'Refresh the case before image review.')
        if db.scalar(select(Job).where(Job.case_id == case.id, Job.status.in_(['queued', 'running']))):
            raise ApiError(409, 'ANALYSIS_ALREADY_RUNNING', 'A review is already running for this case.')
        if len(list(db.scalars(select(Job.id).where(Job.tenant_id == user.tenant_id, Job.status.in_(['queued', 'running']))))) >= 10:
            raise ApiError(429, 'QUEUE_FULL', 'Analysis queue is full.')
        job = Job(tenant_id=user.tenant_id, case_id=case.id, actor_id=user.id, case_version=case.version, kind='imaging', payload={**body.model_dump(), 'study_id': study_id, 'mode': body.analysis_scope, 'review_focus': 'radiology', 'include_ai': True})
        db.add(job)
        db.flush()
...
def process_image_job(db, job, user, case):
    ...
    try:
        ...
        try:
            ...
            content = [{'type': 'text', 'text': 'UNTRUSTED RADIOLOGIST REPORT:\n' + json.dumps({'report': body.radiologist_report, 'not_shown_frames': coverage['total_frames'] - len(frames)}, ensure_ascii=False)}]
            for frame in frames:
    ...
    claimed_finish = db.execute(update(Job).where(Job.id == job.id, Job.status == 'running').values(status='partial', stage='complete', finished_at=now()))
    if not claimed_finish.rowcount:
        db.rollback()
        return
    job.result = result
    create_record(db, user, 'notification', {'run_id': job.id, 'status': 'partial', 'read_by': [], 'signature': job.id}, case, job.case_version)
