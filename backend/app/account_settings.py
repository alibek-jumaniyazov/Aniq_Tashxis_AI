...
def update_profile(
    ...
    return idempotent(
        ...
        "account.profile",
        key,
        {**body.model_dump(), "session_id": request.state.session.id},
        execute,
    )
...
def change_password(
    ...
    def execute():
        ...
        result = db.execute(
            update(User)
            .where(User.id == user.id, User.password_hash == previous_hash)
            .values(
    ...
    return idempotent(
        ...
        "account.password",
        key,
        {"session_id": session.id, "fingerprint": fingerprint},
        execute,
    )
...
def session_summary(
    request: Request, user: User = Depends(current_user), db: DBSession = Depends(get_db)
):
    active = (
        db.scalar(
            select(func.count())
    ...
    return {
        ...
        "other_sessions": max(0, active - 1),
        "current_expires_at": request.state.session.expires_at.replace(tzinfo=timezone.utc),
    }
...
def revoke_sessions(
    ...
    return idempotent(
        db, user, "account.sessions.revoke_others", key, {"session_id": session.id}, execute
    )
