...
def current_entries(db, case_id):
    records = list(
        db.scalars(
            select(Record)
            .where(Record.case_id == case_id, Record.kind == "clinical_entry")
...
def create_entry_record(
    db, user, case, category, text, *, source=None, diagnosis="", treatment="", confirmed=False
):
    mode = (
        ("dmed_demo" if source.data.get("type") == "dmed_demo" else "document")
        if source
        else "manual"
    )
...
def backfill_legacy_dmed_entries(db, user, case, imported, expected_version):
    ...
    if source.case_id != case.id or source.data.get("type") != "dmed_demo":
        raise ApiError(
            422,
            "CLINICAL_ENTRY_SOURCE_MISMATCH",
            "The legacy demo source does not belong to this patient.",
        )
    ...
    records = [
        ...
        if r.data.get("source_id") == source.id
    ]
    ...
    if "subjective" in missing and len(original_text) > 20000:
        raise ApiError(
            ...
            "Select an excerpt from the original demo source.",
        )
    ...
    for category in missing:
        ...
        entry = create_entry_record(
            ...
            case,
            category,
            original_text if category == "subjective" else "",
            source=source,
        )
    ...
    imported.data = {
        **imported.data,
        "clinical_entry_ids": [r.id for r in records],
...
def comparison_readiness(case, entries, facts):
    ...
    data = data or any(
        f.get("confirmed")
        and f.get("assertion") in {"present", "absent"}
        and f.get("value") not in (None, "")
        for f in facts
    )
    conclusion = any(
        r.data["confirmed"] and r.data["category"] == "doctor_conclusion" for r in entries
    )
    adult = case.age is not None and 18 <= case.age <= 120
    return {
        "comparison_ready": bool(data and conclusion and adult),
        "has_confirmed_data": bool(data),
        "has_confirmed_conclusion": conclusion,
    ...
    def run():
        ...
        if body.source_id:
            ...
            if source.case_id != case.id:
                raise ApiError(
                    422, "CLINICAL_ENTRY_SOURCE_MISMATCH", "The source belongs to another patient."
                )
            if source.data.get("type") not in {"pdf", "docx", "txt", "dmed_demo"}:
                raise ApiError(
                    422, "DOCUMENT_TEXT_REQUIRED", "Choose a document with extracted text."
                )
            ...
            if len(text) > 20000:
                raise ApiError(
                    422,
                    "CLINICAL_ENTRY_SOURCE_TOO_LONG",
                    "Select a source excerpt of at most 20000 characters.",
                )
            if text not in source.data.get("text", ""):
                raise ApiError(
                    422, "INVALID_SOURCE_SPAN", "The selected text is absent from the source."
                )
        if not text:
            raise ApiError(
                422,
                "CLINICAL_ENTRY_TEXT_REQUIRED",
                "Enter clinical observations or choose a source with text.",
            )
        bump_case(db, case, body.expected_version)
        record = create_entry_record(
            db,
            user,
            case,
            body.category,
        ...
        if (
            record.data["category"] == "doctor_conclusion"
            and record.data["confirmed"]
            and record.data["diagnosis"]
        ):
...
def update_entry(
    ...
    def run():
        if original.id not in {r.id for r in current_entries(db, case.id)}:
            raise ApiError(
                ...
                "This entry has been revised. Reload its current version.",
            )
        changes = body.model_dump(exclude={"expected_version"}, exclude_unset=True)
        if (
            changes.get("confirmed", original.data["confirmed"])
            and not changes.get("text", original.data["text"]).strip()
        ):
            raise ApiError(
                422,
                "CLINICAL_ENTRY_TEXT_REQUIRED",
                "Enter the missing clinical data before confirming this draft.",
            )
        # Editing confirmed evidence requires explicit re-attestation.
        if any(
            field in changes and changes[field] != original.data.get(field)
        ...
        revision = create_record(
            ...
            "clinical_entry",
            {
                ...
                **changes,
                ...
                "author_name": user.name,
                "clinician_edited": bool(
                    original.data.get("clinician_edited")
                    or any(field in changes for field in ("text", "diagnosis", "treatment"))
                ),
            },
            case,
        )
        if (
            revision.data["category"] == "doctor_conclusion"
            and revision.data["confirmed"]
            and revision.data["diagnosis"]
        ):
    ...
    return idempotent(
        db, user, f"clinical_entry.update:{entry_id}", key, body.model_dump(exclude_unset=True), run
    )
...
def comparison_snapshot(case, entries, facts):
    ...
    conclusion = next(
        ...
        if r.data["confirmed"] and r.data["category"] == "doctor_conclusion"
    )
    return {
        ...
        "entries": [
            {
                ...
                **{
                    k: r.data.get(k)
                    for k in ("category", "text", "diagnosis", "treatment", "source_id")
                },
            }
            for r in entries
            if r.data["confirmed"]
            and (r.data["category"] != "doctor_conclusion" or r.id == conclusion.id)
        ],
        "facts": [
            f
            for f in facts
...
def compare(
    ...
    def run():
        ...
        if not ready["adult_age_known"]:
            raise ApiError(
                422, "OUTSIDE_DEMO_SCOPE", "Clinical AI currently supports known adult ages 18–120."
            )
        if not ready["comparison_ready"]:
            raise ApiError(
                422,
                "CLINICAL_COMPARISON_EVIDENCE_REQUIRED",
                "Confirm patient data and a doctor conclusion before comparison.",
                ready,
            )
        active = list(
            db.scalars(
        ...
        if any(job.case_id == case.id for job in active):
            raise ApiError(
                409, "ANALYSIS_ALREADY_RUNNING", "An analysis is already running for this patient."
            )
        job = Job(
            ...
            payload={
                "snapshot": {
                    **comparison_snapshot(case, entries, facts),
...
def comparisons(case_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    records = db.scalars(
        select(Job)
        .where(Job.case_id == case.id, Job.kind == "clinical_comparison")
