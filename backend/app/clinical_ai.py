...
def validate_assessment(result, snapshot, evidence):
    ...
    for item in hypotheses:
        ...
        if any(ref not in refs for ref in cited) or set(item["supporting_refs"]) & set(
            item["opposing_refs"]
        ):
        ...
        if not any(
            refs[ref]["key"].startswith("symptom.") and refs[ref]["assertion"] == "present"
    ...
    def numbers(text):
        return {
            float(n.replace(",", ".")) for n in re.findall(r"(?<!\w)\d+(?:[.,]\d+)?", str(text))
        }
...
def presentation_issues(result, language="ru", snapshot=None):
    ...
    if assessment["status"] == "insufficient_data" and not assessment["questions"]:
        issues.append(
            "For insufficient_data, ask at least one specific clarification question; never invent a diagnosis."
        )
    questions = [question.strip().casefold() for question in assessment["questions"]]
    if len(questions) != len(set(questions)) or any(
        not question.endswith("?") for question in questions
    ):
        issues.append(
            "Ask distinct, specific clarification questions, ending each with a question mark."
        )
...
def review(snapshot):
    ...
    for item in report["evidence"]:
        ...
        item.update(
            {key: fact.get(key) for key in ("event_time", "available_time", "order_status")}
        )
    context = {
        "case_version": snapshot["version"],
        "age": snapshot.get("age"),
        "sex": snapshot.get("sex"),
        "preliminary_context": snapshot.get("diagnosis"),
        "evidence": [
            {k: v for k, v in e.items() if k not in {"fact_id", "source_id"}}
            for e in report["evidence"]
        ],
        "potential_conflicts": [c["key"] for c in report["potential_conflicts"]],
    }
    ...
    for field in ("supporting_refs", "opposing_refs"):
        schema["$defs"]["DiagnosticHypothesis"]["properties"][field]["items"] = {
            "type": "string",
            "enum": [e["ref"] for e in report["evidence"]],
        }
    ...
    for attempt in range(2):
        ...
        if attempt == 1:
            code = (
                ...
                if any(issue.startswith("AI_LANGUAGE_MISMATCH") for issue in issues)
