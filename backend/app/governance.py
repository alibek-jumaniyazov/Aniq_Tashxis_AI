...
def incidents(user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    for record in db.scalars(
        select(Record)
        .where(Record.kind == "incident", Record.tenant_id == user.tenant_id)
...
def create_incident(
    ...
    def run():
        record = create_record(
            ...
            user,
            "incident",
            {"reason": body.reason, "status": "under_review", "version": 1, "decisions": []},
            case,
        )
    ...
    def run():
        ...
        record.data = {
            **record.data,
            ...
            "decisions": record.data["decisions"] + [decision],
        }
...
def exports(user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    for record in db.scalars(
        select(Record)
        .where(Record.kind == "export", Record.tenant_id == user.tenant_id)
        .order_by(Record.created_at.desc())
    ):
        try:
...
def create_export(
    ...
    def run():
        ...
        for incident_id in dict.fromkeys(body.incident_ids):
            ...
            if incident.data["status"] not in {
                "confirmed",
                "corrective_actions",
                "closed",
            } or not any(d["status"] == "confirmed" for d in incident.data["decisions"]):
                raise ApiError(
                    422,
                    "EXPERT_CONFIRMATION_REQUIRED",
                    "Only independently confirmed incidents may be exported.",
                )
            snapshots.append(
                {
                    "incident_id": incident.id,
                    "version": incident.data["version"],
                    "case_version": incident.case_version,
                }
            )
        ...
        package = {
            ...
            "confirmed_cases": len(snapshots) if len(snapshots) >= 5 else None,
            "small_group_suppressed": len(snapshots) < 5,
        ...
        record = create_record(
            ...
            {
                **body.model_dump(),
                "incident_ids": [s["incident_id"] for s in snapshots],
                "snapshots": snapshots,
                "package": package,
    ...
    for snapshot in record.data["snapshots"]:
        ...
        if case.version != snapshot["case_version"]:
            raise ApiError(
                ...
                "New clinical data requires a fresh expert review.",
            )
    ...
    def run():
        ...
        for incident_id in record.data["incident_ids"]:
            ...
            if any(d["actor_id"] == user.id for d in incident.data["decisions"]):
                raise ApiError(
                    403, "SEPARATE_APPROVER_REQUIRED", "Expert and sender must be different people."
                )
        record.data = {
            **record.data,
            "status": "approved",
...
def send(
    ...
    def run():
        ...
        if (
            ...
            or record.data["version"] != body.expected_version
            or not record.data.get("approved_by")
        ):
...
def download_export(
    ...
    if format == "pdf":
        ...
        canvas.setFillColorRGB(0.03, 0.5, 0.51)
        canvas.setFont("Helvetica-Bold", 24)
        canvas.drawString(50, 780, "AniqTashxis.ai")
        canvas.setFillColorRGB(0.08, 0.16, 0.21)
        canvas.setFont("Helvetica", 12)
        for i, line in enumerate(
            [
                "DEMONSTRATION REPORT - no real government transmission",
                "",
                *[f"{k}: {v}" for k, v in package.items()],
                "",
                "Package SHA-256:",
                record.data["sha256"],
            ]
    ...
    return Response(
        ...
        media_type=media,
        headers={
            "Content-Disposition": f'attachment; filename="aniqtashxis-{record.id[:8]}.{format}"',
...
def system_status(user: User = Depends(current_user)):
    return {
        ...
        "database": "sqlite_local" if settings.database_url.startswith("sqlite") else "postgresql",
        "queue": settings.queue_mode,
        "model": ai.model_status(),
        "dmed": "demo",
        "export_transport": "mock",
        "ct_model": "not_configured",
        "risk_model": "framingham-general-cvd-lipids-2008",
        "approved_clinical_rules": 0,
    }
        ...
        "items": [
            {
                "id": r.id,
                "actor_id": r.actor_id,
                "actor_name": staff.get(r.actor_id, ""),
                "action": r.action,
...
def team(user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    users = db.scalars(
        select(User).where(User.tenant_id == user.tenant_id).order_by(User.role, User.name)
    )
    return {
        "items": [
