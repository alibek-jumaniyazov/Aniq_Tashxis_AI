...
FIELDS = ['systolic_pressure', 'total_cholesterol', 'hdl_cholesterol', 'smoker', 'diabetes', 'bp_treated', 'baseline_cvd']
...
def calculate(case, body):
    ...
    if not reasons and not missing:
        ...
        if not (100 <= total <= 400 and 10 <= hdl <= 100 and hdl < total and 90 <= inputs['systolic_pressure'] <= 200):
            ...
            if case.sex == 'male':
                age_b, total_b, hdl_b, sbp_b, smoke_b, dm_b, survival, mean = 3.06117, 1.12370, -.93263, (1.99881 if inputs['bp_treated'] else 1.93303), .65451, .57367, .88936, 23.9802
            else:
                age_b, total_b, hdl_b, sbp_b, smoke_b, dm_b, survival, mean = 2.32888, 1.20904, -.70833, (2.82263 if inputs['bp_treated'] else 2.76157), .52873, .69154, .95012, 26.1931
            score = age_b * math.log(case.age) + total_b * math.log(total) + hdl_b * math.log(hdl) + sbp_b * math.log(inputs['systolic_pressure']) + smoke_b * inputs['smoker'] + dm_b * inputs['diabetes']
            probability = 1 - survival ** math.exp(score - mean)
            normalized.update(total_cholesterol_mg_dl=total, hdl_cholesterol_mg_dl=hdl)
    return {'outcome_id': body.outcome_id, 'horizon_years': body.horizon_years, 'eligibility_status': 'calculated' if probability is not None else 'not_available', 'probability': probability, 'model_id': MODEL_ID, 'model_version': '2008-lipids-1.0', 'source_url': SOURCE_URL, 'inputs': normalized, 'missing_fields': missing, 'unsupported_reasons': reasons, 'validation_status': 'published_equation_not_locally_validated', 'limitations': ['FHS_NOT_LOCALLY_CALIBRATED', 'FHS_NOT_DIAGNOSIS'], 'provenance': 'clinician_confirmed_calculator_inputs'}
