...
IMAGE_MODALITIES = {'CT', 'MR', 'CR', 'DX', 'MG', 'US', 'PT', 'NM', 'XA', 'RF', 'SC', 'OT', 'XC', 'ES', 'GM', 'IO', 'PX'}


def parse_dicom(data):
    if zipfile.is_zipfile(io.BytesIO(data)):
        archive = safe_zip(data, limit=1500 * 1024 * 1024, max_files=1100)
        entries = [(e.filename, e.file_size) for e in archive.infolist() if not e.is_dir() and e.filename.rsplit('/', 1)[-1].upper() != 'DICOMDIR']
        read = archive.read
    else:
    ...
    for name, size in entries:
        ...
        try:
            ...
            raise ApiError(422, 'INVALID_DICOM', 'Upload a DICOM image or a ZIP of DICOM images.') from None
        modality = str(ds.get('Modality', '')).upper()
        if modality not in IMAGE_MODALITIES:
            raise ApiError(422, 'UNSUPPORTED_MODALITY', f'{modality or "Unknown"} is not a supported pixel image modality. Reports (SR), presentation states and segmentation objects cannot be displayed as original images.')
        required = ['SeriesInstanceUID', 'StudyInstanceUID', 'SOPInstanceUID', 'Rows', 'Columns', 'PixelData']
        if any(k not in ds for k in required):
            raise ApiError(422, 'DICOM_IMAGE_MISSING', 'DICOM pixel data or image identifiers are missing.')
        ...
        if not 1 <= rows <= 4096 or not 1 <= columns <= 4096 or rows * columns * frames * max(1, int(ds.get('SamplesPerPixel', 1))) * 4 > 256 * 1024 * 1024:
            raise ApiError(413, 'DICOM_DIMENSION_LIMIT', 'Decoded pixel size exceeds the safe budget.')
        ...
        if photo not in {'MONOCHROME1', 'MONOCHROME2', 'RGB', 'YBR_FULL', 'YBR_FULL_422', 'PALETTE COLOR'}:
            raise ApiError(422, 'DICOM_COLOR_UNSUPPORTED', 'Unsupported photometric interpretation.')
        try:
            ...
            raise ApiError(422, 'DICOM_DECODE_FAILED', 'This pixel encoding cannot be decoded. Export uncompressed DICOM (Explicit VR Little Endian).') from None
        ...
        frame_spacings = [list(map(float, group.PixelMeasuresSequence[0].PixelSpacing)) for group in per_frame if group.get('PixelMeasuresSequence') and group.PixelMeasuresSequence[0].get('PixelSpacing')]
        if frame_spacings:
            # A series-level ruler cannot represent differing calibration per frame.
            spacing = frame_spacings[0] if len(frame_spacings) == frames and all(value == frame_spacings[0] for value in frame_spacings) else None
        spacing = list(map(float, spacing)) if spacing is not None else None
        if spacing is not None and (len(spacing) != 2 or not np.isfinite(spacing).all() or min(spacing) <= 0):
        ...
        if spatial:
            orientation, position = np.asarray(orientation, dtype=float), np.asarray(position, dtype=float)
            if orientation.shape != (6,) or position.shape != (3,) or not np.isfinite(np.concatenate([orientation, position])).all() or not np.isclose(np.linalg.norm(orientation[:3]), 1, atol=1e-3) or not np.isclose(np.linalg.norm(orientation[3:]), 1, atol=1e-3) or not np.isclose(np.dot(orientation[:3], orientation[3:]), 0, atol=1e-3):
                raise ApiError(422, 'INVALID_DICOM_GEOMETRY', 'Orientation must be orthonormal.')
        color = photo not in {'MONOCHROME1', 'MONOCHROME2'}
        values = pixels.astype(float) * float(ds.get('RescaleSlope', 1)) + float(ds.get('RescaleIntercept', 0))
        ...
        groups.setdefault(str(ds.SeriesInstanceUID), []).append({'raw': raw, 'sop': str(ds.SOPInstanceUID), 'study': str(ds.StudyInstanceUID), 'modality': modality, 'position': position.tolist() if spatial else None, 'orientation': orientation.tolist() if spatial else None, 'spacing': spacing, 'rows': rows, 'columns': columns, 'z': float(np.dot(np.cross(orientation[:3], orientation[3:]), position)) if spatial else float(ds.get('InstanceNumber', len(groups))), 'frames': frames, 'color': color, 'window_center': wc, 'window_width': ww, 'synthetic_phantom': 'SYNTHETIC PHANTOM' in str(ds.get('SeriesDescription', '')).upper()})
    if not groups:
    ...
    for instances in groups.values():
        ...
        for item in instances:
            if any(item[k] != first[k] for k in ('rows', 'columns', 'modality', 'color')):
                raise ApiError(422, 'INCONSISTENT_DICOM_GEOMETRY', 'Inconsistent series dimensions.')
            if item['spacing'] != first['spacing']:
                # A single series calibration must never silently apply to different pixels.
                raise ApiError(422, 'INCONSISTENT_DICOM_GEOMETRY', 'Pixel spacing varies across instances; export separate series.')
            if first['modality'] == 'CT' and first['orientation'] is not None and (item['orientation'] is None or not np.allclose(item['orientation'], first['orientation'], atol=1e-4)):
                raise ApiError(422, 'INCONSISTENT_DICOM_GEOMETRY', 'Inconsistent CT orientation.')
        instances.sort(key=lambda i: i['z'])
        if first['modality'] == 'CT' and len(instances) > 1 and all(i['frames'] == 1 for i in instances):
...
def render_frame(study, series_id, index, center, width):
    ...
    locator = series.get('frame_map', [])[index] if series.get('frame_map') else {'file_index': index, 'pixel_frame': 0}
    ...
    if pixels.ndim == 3:
        ...
        shared = ds.get('SharedFunctionalGroupsSequence', [])
        per_frame = ds.get('PerFrameFunctionalGroupsSequence', [])
        for group in ([shared[0]] if shared else []) + ([per_frame[locator['pixel_frame']]] if len(per_frame) > locator['pixel_frame'] else []):
            if group.get('PixelValueTransformationSequence'):
                transform = group.PixelValueTransformationSequence[0]
        pixels = pixels.astype(float) * float(transform.get('RescaleSlope', 1)) + float(transform.get('RescaleIntercept', 0))
