...
def account_enabled(user):
    if not user or not user.active:
        return False
    demo_identity = user.email.lower().endswith("@demo.aniq") or user.tenant_id in {
        "avilab-demo",
        "other-demo",
    }
...
def access_case(db, user, case_id):
    ...
    if (
        not case
        or case.tenant_id != user.tenant_id
        or user.role in {"admin", "analyst", "owner", "developer"}
    ):
        raise ApiError(404, "CASE_NOT_FOUND", "Case is not available.")
    if case.owner_id != user.id:
        grant = db.scalar(
            select(CaseAccess).where(CaseAccess.case_id == case.id, CaseAccess.user_id == user.id)
        )
        if not grant:
            raise ApiError(404, "CASE_NOT_FOUND", "Case is not available.")
    return case
...
def audit(db, user, action, resource_id):
    db.add(
        Audit(tenant_id=user.tenant_id, actor_id=user.id, action=action, resource_id=resource_id)
    )
...
def bump_case(db, case, expected_version):
    result = db.execute(
        update(Case)
        .where(Case.id == case.id, Case.version == expected_version)
    ...
    if result.rowcount != 1:
        ...
        raise ApiError(
            ...
            "Case changed. Reload the current version.",
            {"expected_version": expected_version, "current_version": fresh.version},
        )
    db.refresh(case)
    return case.version
...
def create_record(db, user, kind, data, case=None, version=None):
    record = Record(
        ...
        case_version=version or (case.version if case else None),
    )
    db.add(record)
    db.flush()
    return record
...
def idempotent(db, user, operation, key, payload, execute):
    ...
    query = select(Idempotency).where(
        Idempotency.actor_id == user.id, Idempotency.operation == operation, Idempotency.key == key
    )
    ...
    if prior:
        if prior.digest != digest:
            raise ApiError(
                ...
                "This key has already been used for a different request.",
            )
        return prior.response
    try:
        guard = Idempotency(
            actor_id=user.id, operation=operation, key=key, digest=digest, response={}
        )
        db.add(guard)
...
def cached_idempotent(db, user, operation, key, payload):
    ...
    if prior.digest != digest:
        raise ApiError(
            409, "IDEMPOTENCY_CONFLICT", "This key has already been used for a different request."
        )
    return prior.response
...
def current_facts(db, case_id, version=None):
    q = (
        select(Record)
        .where(Record.case_id == case_id, Record.kind == "fact")
