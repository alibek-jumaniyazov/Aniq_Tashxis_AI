...
def case_snapshot(case):
    ...
    return {
        key: value
        for key, value in case_json(case).items()
        if key not in {"full_name", "patient_phone"}
    }
...
def user_json(user):
    ...
    return {
        ...
        "is_clinic_owner": bool(clinic and clinic.owner_id == user.id),
    }
...
def list_cases(
    ...
    if q.strip():
        ...
        cases = [
            ...
            or (
                len(phone_digits) >= 3
                and all(char.isdigit() or char in "+- ()" for char in term)
                and phone_digits in re.sub(r"\D", "", case.patient_phone)
            )
        ]
    ...
    for case in cases:
        alerts = list(
            db.scalars(select(Record).where(Record.case_id == case.id, Record.kind == "alert"))
        )
        runs = list(
            db.scalars(select(Job).where(Job.case_id == case.id).order_by(Job.created_at.desc()))
        )
        open_alerts = sum(a.data["status"] not in {"closed", "rejected"} for a in alerts)
        stats["alerts"] += open_alerts
        stats["analyses"] += len(runs)
        stats["reviewed"] += sum(
            a.data["status"] in {"closed", "accepted", "rejected"} for a in alerts
        )
        facts = current_facts(db, case.id)
        studies = list(
            db.scalars(select(Record).where(Record.case_id == case.id, Record.kind == "study"))
        )
        imaging_reviews = list(
            db.scalars(
                select(Record).where(Record.case_id == case.id, Record.kind == "imaging_review")
            )
        )
        ...
        items.append(
            {
                **case_json(case),
                ...
                "pending_facts": sum(not f["confirmed"] for f in facts),
                "reviewed_count": sum(
                    a.data["status"] in {"closed", "accepted", "rejected"} for a in alerts
                ),
                "study_count": len(studies),
                "pending_imaging": sum(s.id not in reviewed_studies for s in studies),
                "analysis_count": len(runs),
                "latest_status": runs[0].status if runs else "not_started",
                "latest_version": runs[0].case_version if runs else None,
            }
        )
    filters = {
        "all": lambda item: True,
        "alerts": lambda item: item["alert_count"] > 0,
        "pending": lambda item: item["pending_facts"] > 0,
        "reviewed": lambda item: item["reviewed_count"] > 0,
        "analyses": lambda item: item["analysis_count"] > 0,
        "imaging_pending": lambda item: item["pending_imaging"] > 0,
    }
    ...
    def run():
        ...
        for collaborator in db.scalars(
            select(User).where(
                ...
                User.role.in_(["expert", "quality", "sender", "radiologist"]),
            )
        ):
...
def get_case(case_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    records = list(
        db.scalars(
            select(Record).where(Record.case_id == case.id).order_by(Record.created_at.desc())
        )
    )
    runs = list(
        db.scalars(select(Job).where(Job.case_id == case.id).order_by(Job.created_at.desc()))
    ...
    studies = [
        {
            **serialize(r),
            "reviews": [
                {**serialize(review), "reviewer_name": staff.get(review.actor_id, "")}
                for review in records
                if review.kind == "imaging_review" and review.data.get("study_id") == r.id
            ],
            "measurements": [
                serialize(m)
                for m in records
                if m.kind == "imaging_measurement" and m.data.get("study_id") == r.id
            ],
            "analyses": [
                job_json(j, case)
                for j in runs
                if j.kind == "imaging" and j.payload.get("study_id") == r.id
            ],
        }
        for r in records
        if r.kind == "study"
    ...
    alerts = [
        {
            **serialize(r),
            "reviews": [
                {**serialize(review), "reviewer_name": staff.get(review.actor_id, "")}
                for review in reversed(records)
                if review.kind == "review" and review.data.get("alert_id") == r.id
            ],
        }
        for r in records
        if r.kind == "alert"
    ...
    return {
        **case_json(case),
        "facts": current_facts(db, case.id),
        "notes": [serialize(r) for r in records if r.kind == "note"],
        "documents": [source_public(r) for r in records if r.kind == "source"],
        "alerts": alerts,
        "analyses": [job_json(r, case) for r in runs],
        "forecasts": [serialize(r) for r in records if r.kind == "forecast"],
        "studies": studies,
        "clinical_conclusions": [serialize(r) for r in records if r.kind == "clinical_conclusion"],
    }
...
def versions(case_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    records = db.scalars(
        ...
        .order_by(Record.case_version.desc(), Record.created_at.desc())
    )
    ...
    def run():
        ...
        for i, fact in enumerate(body.facts):
            ...
            if fact.provenance in {"document", "dmed_demo"} and not fact.source_id:
                raise ApiError(
                    422, "SOURCE_REQUIRED", "Document and import facts require an original source."
                )
            if fact.source_id:
                source = access_record(db, user, fact.source_id, ["source", "note"])
                if source.case_id != case.id:
                    raise ApiError(
                        422, "SOURCE_CASE_MISMATCH", "Evidence belongs to a different case."
                    )
                if fact.provenance == "dmed_demo" and source.data.get("type") != "dmed_demo":
                    raise ApiError(
                        422, "SOURCE_PROVENANCE_MISMATCH", "This source is not a DMED import."
                    )
                if not fact.span:
                ...
                if fact.span.startswith("excerpt:"):
                    ...
                    if not excerpt or excerpt not in source.data.get("text", ""):
                        raise ApiError(
                            422,
                            "INVALID_SOURCE_SPAN",
                            "The quoted excerpt is absent from the source.",
                        )
                    ...
                    page = next(
                        (
                            p
                            for p in source.data.get("pages", [])
                            if match and p["page"] == int(match[1])
                        ),
                        None,
                    )
                    if (
                        not match
                        or not page
                        or not 0 <= int(match[2]) < int(match[3]) <= len(page["text"])
                    ):
                        raise ApiError(
                            422,
                            "INVALID_SOURCE_SPAN",
                            "Select a real source excerpt or valid page range.",
                        )
            else:
                source = create_record(
                    ...
                    "source",
                    {
                        "name": "Manual entry",
                        "type": "manual",
                        "text": f"{fact.label}: {fact.value} {fact.unit or ''}",
                        "pages": [],
                        "sha256": hashlib.sha256(str(data).encode()).hexdigest(),
                        "limitations": [],
                    },
                    case,
                )
...
def confirm(
    ...
    def run():
        ...
        for fact_id in body.fact_ids:
            ...
            create_record(
                db, user, "fact", {**record.data, "confirmed": True, "supersedes": record.id}, case
            )
...
def get_note_draft(
    case_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)
):
    require_role(user, "doctor")
    access_case(db, user, case_id)
    draft = db.scalar(
        select(Record)
        .where(Record.kind == "note_draft", Record.case_id == case_id, Record.actor_id == user.id)
...
def job_json(job, case):
    return {
        ...
        "run_id": job.id,
        "case_id": job.case_id,
        "case_version": job.case_version,
        "kind": job.kind,
        "review_focus": job.payload.get("review_focus", "documentation"),
        "include_ai": job.payload.get("include_ai", False),
        "language": normalize_language(
            job.payload.get("language") or (job.result or {}).get("language")
        ),
        "mode": job.payload["mode"],
        "status": job.status,
        "stage": job.stage,
        "is_stale": job.case_version != case.version,
...
def analyse(
    ...
    def run():
        if case.version != body.expected_version:
            raise ApiError(
                ...
                "Refresh the case before analysis.",
        ...
        if any(job.case_id == case.id for job in active):
            raise ApiError(
                409, "ANALYSIS_ALREADY_RUNNING", "An analysis is already running for this case."
            )
        notes = [
            serialize(r)
            for r in db.scalars(
                select(Record)
                .where(Record.case_id == case.id, Record.kind == "note")
        ...
        if body.review_focus == "clinical_assessment":
            ...
            if case.age is None or not 18 <= case.age <= 120:
                raise ApiError(
                    422,
                    "OUTSIDE_DEMO_SCOPE",
                    "The current clinical review supports adults with a known age of 18–120 years.",
                )
            quality = evidence_report(
                snapshot["facts"],
                body.mode,
                body.decision_time.isoformat() if body.decision_time else None,
            )
            if not body.include_ai or not quality["clinical_review_ready"]:
                raise ApiError(
                    422,
                    "INSUFFICIENT_CONFIRMED_EVIDENCE",
                    "Confirm symptom evidence and at least one additional observation before clinical review.",
                )
        job = Job(
            tenant_id=user.tenant_id,
            actor_id=user.id,
            ...
            kind="second_opinion" if request.url.path.endswith("second-opinions") else "analysis",
            payload={**body.model_dump(mode="json"), "snapshot": snapshot},
        )
        db.add(job)
        db.flush()
    ...
    result = idempotent(
        db, user, f"analysis:{case_id}:{request.url.path}", key, body.model_dump(mode="json"), run
    )
...
def retry_job(
    ...
    def run():
        if case.version != body.expected_version or case.version != original.case_version:
            raise ApiError(
                409, "CASE_VERSION_CONFLICT", "Start a new analysis for the current version."
            )
        ...
        if original.status not in {"partial", "failed", "cancelled"} and not (
            original.status == "succeeded" and locale_changed
        ):
...
def case_records(
    ...
    return {
        "items": [
            ...
            for r in db.scalars(
                select(Record)
                .where(Record.case_id == case_id, Record.kind == kind)
                .order_by(Record.created_at.desc())
            )
        ]
    }
...
def get_forecast(
    forecast_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)
):
    ...
    return {
        **serialize(alert),
        "reviews": [
            serialize(r)
            for r in db.scalars(
                select(Record).where(Record.case_id == alert.case_id, Record.kind == "review")
            )
            if r.data.get("alert_id") == alert_id
        ],
    }
...
def alert_review(
    ...
    def run():
        ...
        record = create_record(
            ...
            user,
            "review",
            {"alert_id": alert.id, **body.model_dump()},
            case,
            alert.case_version,
        )
...
def dmed_import(
    ...
    if body.scenario == "identity_mismatch" or body.external_id != "DEMO-001":
        raise ApiError(
            409, "PATIENT_IDENTITY_MISMATCH", "Patient identity mismatch. Merge blocked."
        )
    ...
    def run():
        ...
        previous = [
            r
            for r in db.scalars(
                select(Record).where(Record.case_id == case_id, Record.kind == "import")
            )
            if r.data.get("external_version") == ext_version
        ]
        if previous:
        ...
        demo_entries = [
            (
                "subjective",
                "Учебная история DMED: периодическая головная боль после нагрузки в течение недели. Со слов пациента, ранее давление повышалось.",
                "",
                "",
            ),
            (
                "objective",
                "Учебный осмотр: артериальное давление 140/90 мм рт. ст., пульс 78/мин. Данные одного визита, повторных измерений нет.",
                "",
                "",
            ),
            (
                "laboratory",
                "Учебный лабораторный документ: глюкоза натощак 5.4 ммоль/л. Другие лабораторные показатели в этом импорте не представлены.",
                "",
                "",
            ),
            (
                "instrumental",
                "Учебное инструментальное исследование: в описании ЭКГ указан синусовый ритм. Оригинал ЭКГ и DICOM в импорте отсутствуют.",
                "",
                "",
            ),
            (
                "doctor_conclusion",
                "Учебное заключение врача DMED: повышение артериального давления требует уточнения по повторным измерениям. Планируется повторный прием с дневником давления; лекарственная терапия в этой записи не указана.",
                "Повышение артериального давления, требуется уточнение.",
                "Повторный прием с дневником давления; лекарственная терапия не указана.",
            ),
        ]
        source_text = (
            "Synthetic import: allergy.substance = DEMO-A; medication.substance = DEMO-A\n\n"
        ...
        previous_demo = {
            ...
            for r in db.scalars(
                select(Record)
                .where(Record.case_id == case.id, Record.kind == "clinical_entry")
                .order_by(Record.created_at, Record.id)
            )
            if r.data.get("source_mode") == "dmed_demo"
        }
        entry_ids = []
        ...
        for k, value in [("allergy.substance", "DEMO-A"), ("medication.substance", "DEMO-A")]:
            fact = FactInput(
                ...
                value=value,
                source_id=source.id,
                provenance="dmed_demo",
                span="field:" + k,
                event_time=timestamp,
                available_time=timestamp,
                order_status="active" if k.startswith("medication") else "not_applicable",
            )
            create_record(db, user, "fact", fact.model_dump(mode="json"), case)
        imported = create_record(
            db,
            user,
            "import",
            {
...
def import_status(
    import_id: str, user: User = Depends(current_user), db: DBSession = Depends(get_db)
):
...
def forecast(
    ...
    def run():
        ...
        result = create_record(
            db, user, "forecast", {**calculate(case, body), "author_name": user.name}, case
        )
...
def notification_query(user):
    ...
    return (
        select(Record)
        .join(Case, Case.id == Record.case_id)
        .where(
...
def read_all_notifications(
    ...
    def run():
        ...
        return {
            "updated": updated,
            "unread_count": sum(user.id not in (read_by or []) for read_by in remaining),
        }
