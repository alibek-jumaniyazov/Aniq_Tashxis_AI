...
def mutate(client, path, body=None, method='post', key=None, **kwargs):
    return client.request(method.upper(), '/api/v1' + path, json=body, headers={'Idempotency-Key': key or str(uuid4())}, **kwargs)
...
def fact(key, value, available='2026-09-18T09:00:00+05:00', **kwargs):
    return {'key': key, 'label': key, 'value': value, 'confirmed': True, 'available_time': available, 'event_time': available, **kwargs}


def add(client, c, facts):
    result = mutate(client, f'/cases/{c["id"]}/facts', {'expected_version': c['version'], 'facts': facts}, method='patch')
...
def run(client, c, **kwargs):
    result = mutate(client, f'/cases/{c["id"]}/analyses', {'expected_version': c['version'], **kwargs})
...
def test_manual_source_confirmation_and_analysis(client):
    c = new_case(client)
    add(client, c, [fact('allergy.substance', 'DEMO-A'), fact('medication.substance', 'DEMO-A', order_status='active')])
    result = run(client, c)
    ...
    assert mutate(client, f'/alerts/{alert_id}/reviews', {'status': 'rejected', 'comment': 'Explained synthetic exception'}).status_code == 200


def test_temporal_cutoff_and_stale_result(client):
    c = new_case(client)
    add(client, c, [fact('allergy.substance', 'DEMO-A', '2026-09-18T14:00:00+05:00'), fact('medication.substance', 'DEMO-A', order_status='active')])
    historical = run(client, c, mode='decision_time', decision_time='2026-09-18T10:00:00+05:00', include_ai=False)
...
def test_unknown_time_and_cancelled_order_not_inferred(client):
    c = new_case(client)
    add(client, c, [fact('allergy.substance', 'DEMO-A', None), fact('medication.substance', 'DEMO-A', order_status='cancelled')])
    result = run(client, c, mode='decision_time', decision_time='2026-09-18T10:00:00+05:00', include_ai=False)
    assert not result['result']['alert_ids']
    assert any(x['reason_code'] == 'UNKNOWN_AVAILABLE_TIME' for x in result['result']['coverage']['not_evaluable'])
...
def test_version_conflict_preserves_history(client):
    ...
    response = mutate(client, f'/cases/{c["id"]}/facts', {'expected_version': original, 'facts': [fact('vital.pulse', '90')]}, method='patch')
...
def test_dmed_revoke_identity_and_no_password(client):
    ...
    body = {'expected_version': c['version'], 'connection_id': connection['id'], 'scenario': 'identity_mismatch'}
    ...
    assert mutate(client, '/integrations/dmed/' + connection['id'], method='delete').status_code == 200
...
def test_document_import_drafts_and_bad_files(client):
    ...
    result = client.post(f'/api/v1/cases/{c["id"]}/documents', headers={'Idempotency-Key': str(uuid4())}, data={'expected_version': 1, 'identity_confirmed': 'true'}, files={'file': ('history.txt', body, 'text/plain')})
    assert result.status_code == 201, result.text
    facts = client.get(f'/api/v1/cases/{c["id"]}/facts').json()['items']
    assert len(facts) == 1 and facts[0]['confirmed'] is False
    assert client.get('/api/v1/documents/' + result.json()['id'] + '/content').content == body
    bad = client.post(f'/api/v1/cases/{c["id"]}/documents', headers={'Idempotency-Key': str(uuid4())}, data={'expected_version': 2, 'identity_confirmed': 'true'}, files={'file': ('fake.pdf', b'not pdf', 'application/pdf')})
...
def test_source_from_other_case_rejected(client):
    ...
    response = mutate(client, f'/cases/{second["id"]}/facts', {'expected_version': 1, 'facts': [fact('vital.pulse', '70', source_id=source, span='form:vital.pulse')]}, method='patch')
    assert response.status_code == 422


def test_note_event_time_not_backdated(client):
    c = new_case(client)
    note = mutate(client, f'/cases/{c["id"]}/notes', {'expected_version': 1, 'text': 'Paper record copied later', 'event_time': '2025-01-01T09:00:00+05:00', 'provenance': 'paper'}).json()
...
def test_export_separate_approval_and_anonymity(client):
    c = new_case(client)
    incident = mutate(client, '/incidents', {'case_id': c['id'], 'reason': 'Synthetic quality review'}).json()
    sign_in(client, 'sender')
    assert mutate(client, '/exports', {'incident_ids': [incident['id']], 'purpose': 'Quality demo', 'basis': 'Hackathon'}).status_code == 422
    sign_in(client, 'expert')
    result = mutate(client, f'/incidents/{incident["id"]}/decisions', {'expected_version': 1, 'status': 'confirmed', 'explanation': 'Independent synthetic assessment'})
    assert result.status_code == 200, result.text
    report = mutate(client, '/exports', {'incident_ids': [incident['id']], 'purpose': 'Quality demo', 'basis': 'Hackathon'}).json()
    assert mutate(client, f'/exports/{report["id"]}/approve', {'expected_version': 1}).status_code == 403
    sign_in(client, 'sender')
    assert mutate(client, f'/exports/{report["id"]}/send', {'expected_version': 1}).status_code == 409
...
@pytest.mark.parametrize('path', ['../escape.txt', '/absolute.txt', 'C:/secret.txt', 'nested/../../escape.txt'])
...
def test_model_json_and_source_validation():
    data = {'case_version': 1, 'summary': 'Data only', 'concerns': [], 'limitations': [], 'missing_fields': []}
    assert parse_output(json.dumps(data), 1, set())['summary'] == 'Data only'
    with pytest.raises(ValueError):
        parse_output(json.dumps(data), 2, set())
    data['concerns'] = [{'title': 'invented', 'explanation': 'unsupported', 'source_ids': ['fake'], 'knowledge_id': 'fake'}]
    with pytest.raises(ValueError):
