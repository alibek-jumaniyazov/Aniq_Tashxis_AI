...
@pytest.mark.parametrize('body', [
    {}, {'full_name': ''}, {'full_name': ' \t\n '}, {'full_name': None},
    {'full_name': 'A' * 201}, {'full_name': 'Patient', 'age': -1},
    {'full_name': 'Patient', 'age': 121}, {'full_name': 'Patient', 'age': 18.5},
    {'full_name': 'Patient', 'age': True}, {'full_name': 'Patient', 'patient_phone': '1' * 51},
    {'full_name': 'Patient', 'alias': 'MANUAL-CODE'}, {'full_name': 'Patient', 'diagnosis': 'Unrequested'},
])
...
def test_optional_fields_persist_and_partial_edit_keeps_omitted_fields(client):
    body = {'full_name': '  Dilnoza Karimova  ', 'age': 0, 'sex': 'female', 'patient_phone': ' +998 (90) 123-45-67 ', 'summary': '  Doctor comment.  '}
    ...
    updated = mutate(client, '/cases/' + created['id'], {'expected_version': 1, 'full_name': 'Dilnoza Aliyeva'}, method='patch')
    assert updated.status_code == 200, updated.text
    patient = updated.json()
    assert patient['version'] == 2 and patient['alias'] == created['alias']
    assert patient['summary'] == created['summary'] and patient['patient_phone'] == created['patient_phone']
    assert patient['age'] == 0 and patient['sex'] == 'female'
    assert mutate(client, '/cases/' + created['id'], {'expected_version': 2, 'alias': 'FORGED'}, method='patch').status_code == 422
    assert mutate(client, '/cases/' + created['id'], {'expected_version': 2, 'full_name': '  '}, method='patch').status_code == 422
    assert mutate(client, '/cases/' + created['id'], {'expected_version': 1, 'summary': 'stale'}, method='patch').status_code == 409


@pytest.mark.parametrize('field,clear_value', [('age', None), ('summary', '')])
def test_partial_edit_idempotency_distinguishes_omitted_and_cleared_fields(client, field, clear_value):
    created = mutate(client, '/cases', {'full_name': 'Original Patient', 'age': 45, 'summary': 'Preserved doctor comment'}).json()
    ...
    assert conflicting.status_code == 409 and conflicting.json()['error']['code'] == 'IDEMPOTENCY_CONFLICT'
...
def test_legacy_records_edit_without_inventing_name_and_code_collision_is_skipped(client, case):
    with SessionLocal() as db:
        ...
        db.commit()
    response = mutate(client, '/cases/' + case['id'], {'expected_version': case['version'], 'summary': 'Updated doctor comment'}, method='patch')
...
def test_search_name_phone_and_code_stays_within_patient_permissions(client):
    patient = mutate(client, '/cases', {'full_name': 'Абдуллаев Азиз Каримович', 'patient_phone': '+998 (90) 123-45-67'}).json()
    ...
    with SessionLocal() as db:
        ...
        expert = db.scalar(select(User).where(User.role == 'expert', User.tenant_id == owner.tenant_id))
        db.add(Case(tenant_id=owner.tenant_id, owner_id=expert.id, alias='PRIVATE', full_name='Private Patient', patient_phone='+998 99 444 44 44'))
        db.commit()
...
def test_concurrent_registration_has_unique_codes_and_duplicate_request_replays(client):
    ...
    assert all(response.status_code == 201 for response in results), [response.text for response in results]
    assert len({response.json()['alias'] for response in results}) == 8
    key = str(uuid4())
    with ThreadPoolExecutor(max_workers=4) as workers:
        duplicates = list(workers.map(lambda _: mutate(client, '/cases', {'full_name': 'Same Patient'}, key=key), range(4)))
    assert all(response.status_code == 201 for response in duplicates), [response.text for response in duplicates]
...
def test_patient_identity_is_excluded_from_analysis_snapshot_and_model(client, monkeypatch):
    ...
    patient = mutate(client, '/cases', {'full_name': 'Private Identity Patient', 'patient_phone': '+998901112233', 'age': 40}).json()
...
def test_registration_does_not_expand_adult_clinical_scope(client, age):
    patient = mutate(client, '/cases', {'full_name': 'Patient Outside Adult Scope', 'age': age}).json()
    add(client, patient, [fact('symptom.complaint', 'cough'), fact('vital.pulse', '80', unit='/min')])
    ...
    response = mutate(client, f'/cases/{patient["id"]}/analyses', {'expected_version': patient['version'], 'review_focus': 'clinical_assessment'})
