...
def write(client, method, path, body=None, key=None):
    return client.request(method, '/api/v1/auth' + path, json=body, headers={'Idempotency-Key': key or str(uuid4())})
...
def test_profile_preserves_auth_shape_identity_and_is_idempotent(client):
    ...
    assert write(client, 'PATCH', '/profile', {'name': 'Доктор Азиза Каримова'}, key).json() == expected
    ...
    with SessionLocal() as db:
        ...
        assert len(list(db.scalars(select(Audit).where(Audit.action == 'account.profile_updated')))) == 1


@pytest.mark.parametrize('body', [{'name': ' '}, {'name': 'X'}, {'name': 'Line\nBreak'}, {'name': 'Valid Name', 'role': 'developer'}])
...
def test_settings_available_to_owner_with_expired_subscription(client):
    result = client.post('/api/v1/auth/register', json={
        'clinic_name': 'Settings Clinic', 'name': 'Owner Example', 'email': 'settings@example.org',
        'password': 'StrongDemo!2026', 'phone': '+998 90 123 45 67', 'plan_id': 'clinic10',
    })
    ...
    assert write(client, 'POST', '/password', {'current_password': 'StrongDemo!2026', 'new_password': 'ChangedOwner!2026'}).status_code == 200
...
def test_password_change_preserves_current_revokes_others_and_retries_safely(client):
    ...
    with TestClient(app) as other, TestClient(app) as foreign:
        ...
        assert write(client, 'POST', '/password', {**payload, 'new_password': 'DifferentDoctor!2026'}, key).status_code == 409
        assert client.get('/api/v1/auth/me').json() == before
        assert other.get('/api/v1/auth/me').status_code == 401
        assert foreign.get('/api/v1/auth/me').status_code == 200
        assert other.post('/api/v1/auth/login', json={'email': 'doctor@demo.aniq', 'password': payload['current_password']}).status_code == 401
        assert other.post('/api/v1/auth/login', json={'email': 'doctor@demo.aniq', 'password': payload['new_password']}).status_code == 200
        with SessionLocal() as db:
            ...
            audit = list(db.scalars(select(Audit).where(Audit.action == 'account.password_changed')))
            assert len(audit) == 1
            cache = list(db.scalars(select(Idempotency).where(Idempotency.actor_id == user.id)))
            persisted = json.dumps([{'digest': entry.digest, 'response': entry.response} for entry in cache])
...
def test_password_rejections_do_not_change_account_or_expose_inputs(client, current, new, status, code):
    before = client.get('/api/v1/auth/me').json()
    ...
    with SessionLocal() as db:
        assert password_hasher.verify(db.get(User, before['user']['id']).password_hash, 'AniqDemo!2026')
...
def test_revoke_others_is_scoped_and_old_retry_does_not_revoke_new_session(client):
    with TestClient(app) as other, TestClient(app) as foreign:
        ...
        with SessionLocal() as db:
            ...
            db.add(Session(id='a' * 64, user_id=user.id, csrf='expired', expires_at=now() - timedelta(hours=1)))
            db.commit()
...
def test_settings_require_session_csrf_and_idempotency(client):
    ...
    for method, path, body in [('PATCH', '/profile', {'name': 'Valid Name'}), ('POST', '/password', {'current_password': 'AniqDemo!2026', 'new_password': 'Changed!2026'}), ('POST', '/sessions/revoke-others', None)]:
