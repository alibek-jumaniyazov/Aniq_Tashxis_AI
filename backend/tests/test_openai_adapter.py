...
def response(text='{"answer":"Synthetic result"}', **extra):
    return {'status': 'completed', 'model': 'gpt-5.6-luna',
            'output': [{'type': 'reasoning', 'summary': []},
                       {'type': 'message', 'content': [{'type': 'output_text', 'text': text}]}], **extra}


def mock_http(monkeypatch, handle):
    real = httpx.Client
    monkeypatch.setattr(openai_adapter.httpx, 'Client', lambda **kw: real(transport=httpx.MockTransport(handle), **kw))
...
def test_responses_request_has_strict_structure_no_storage_and_text_image_transport(remote, monkeypatch):
    captured = []
    def handle(request):
    ...
    messages = [{'role': 'system', 'content': 'Reply in Uzbek.'}, {'role': 'user', 'content': [
        {'type': 'text', 'text': 'Synthetic phantom only.'},
        {'type': 'image_url', 'image_url': {'url': 'data:image/png;base64,AAAA'}}]}]
    raw = ai_provider.complete(messages, {'type': 'object', 'properties': {'answer': {'type': 'string'}}}, max_tokens=1200)
...
def test_schema_metadata_does_not_remove_legitimate_field_names():
    ...
    assert set(adapted['$defs']['AIConcern']['properties']) == set(original['$defs']['AIConcern']['properties'])
    names = ['title', 'default', 'minLength', 'maxLength']
    collision = {'type': 'object', 'properties': {name: {'type': 'string', 'title': name} for name in names}}
    converted = openai_adapter.strict_schema(collision)
    assert list(converted['properties']) == names
    assert converted['required'] == names


@pytest.mark.parametrize('http_status,error,expected', [
    (401, 'invalid_api_key', 'OPENAI_AUTH_ERROR'), (403, 'permission_denied', 'OPENAI_MODEL_UNAVAILABLE'),
    (404, 'model_not_found', 'OPENAI_MODEL_UNAVAILABLE'), (429, 'insufficient_quota', 'OPENAI_QUOTA_EXCEEDED'),
    (429, 'rate_limit_exceeded', 'OPENAI_RATE_LIMIT'), (400, 'invalid_json_schema', 'OPENAI_INVALID_REQUEST'),
    (500, 'server_error', 'OPENAI_UNAVAILABLE'),
])
def test_provider_failures_are_sanitized_without_fake_fallback(remote, monkeypatch, http_status, error, expected):
    calls = []
    def handle(request):
        calls.append(request.url.path)
        return httpx.Response(http_status, json={'error': {'code': error, 'message': 'synthetic-test-secret patient data'}})
    mock_http(monkeypatch, handle)
...
@pytest.mark.parametrize('payload,expected', [
    ...
    (response(output=[{'type': 'message', 'content': [{'type': 'refusal', 'refusal': 'No'}]}]), 'AI_RESPONSE_REFUSED'),
])
