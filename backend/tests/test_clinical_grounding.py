...
def snapshot():
    return {
        ...
        "age": 52,
        "sex": "unknown",
        "evaluated_conclusion_id": "doctor-note",
        "entries": [
            {
                "id": "exam",
                "category": "objective",
                "text": "Pulse 72 /min.",
                "source_id": "exam-source",
            },
            {
                "id": "doctor-note",
                "category": "doctor_conclusion",
                "text": "Follow-up requires examination.",
                "diagnosis": "",
                "treatment": "",
                "source_id": None,
            },
        ...
        "facts": [
            {
                ...
                "confirmed": True,
                "assertion": "present",
                "event_time": "2026-09-01T10:00:00+05:00",
                "available_time": "2026-09-01T10:05:00+05:00",
            },
            {
                "id": "order",
                ...
                "event_time": "2026-08-01T10:00:00+05:00",
                "available_time": "2026-08-01T10:05:00+05:00",
            },
        ],
    }
...
def comparison():
    return {
        ...
        "diagnosis_review": {
            "status": "insufficient_data",
            "summary": "Для заключения нужен дополнительный осмотр.",
            "refs": ["E1", "E2"],
        },
        "treatment_review": {
            "status": "insufficient_data",
            "summary": "Текущая схема лечения не описана.",
            "refs": ["E2"],
        },
        "supporting": [{"text": "Пульс 72 /min.", "refs": ["E1"]}],
        "discrepancies": [],
        "questions": ["Какая схема лечения назначена сейчас?"],
...
def validate(result):
    data = snapshot()
    return patient_workspace_ai.validate_comparison(
        result, data, patient_workspace_ai.evidence_context(data)
    )
...
@pytest.mark.parametrize(
    "block", ["diagnosis_review", "treatment_review", "supporting", "discrepancies"]
)
...
def test_comparison_preserves_timing_and_stopped_orders_and_corrects_bad_output_once(monkeypatch):
    ...
    def complete(messages, schema, **kwargs):
        ...
        return json.dumps(
            {key: value for key, value in result.items() if key in schema["properties"]},
            ensure_ascii=False,
        )
    ...
    context = json.loads(
        calls[0][1]["content"].split("UNTRUSTED PATIENT DATA\n")[1].split("\nEND DATA")[0]
    )
    medication = context["evidence"][-1]
    assert context["available_categories"] == {
        "objective": ["E1"],
        "doctor_conclusion": ["E2"],
        "confirmed_fact": ["F1", "F2"],
    }
...
def test_incomplete_model_response_retries_once_with_compact_complete_schema(monkeypatch):
    ...
    def complete(messages, schema, **kwargs):
        ...
        return json.dumps(
            {key: value for key, value in comparison().items() if key in schema["properties"]}
        )
    ...
    assert (
        calls[1][1]["properties"]["summary"]["maxLength"]
        < calls[0][1]["properties"]["summary"]["maxLength"]
    )
...
def test_incomplete_retry_is_bounded_and_unrelated_runtime_failures_are_not_retried(
    monkeypatch, code, attempts
):
    monkeypatch.setattr(settings, "ai_backend", "llama_cpp")
    monkeypatch.setattr(ai, "model_status", lambda: {"ready": True})
    calls = []
...
def test_retry_schema_leaves_room_for_complete_questions_and_review_sentences():
    data = snapshot()
    schema = patient_workspace_ai.output_schema(
        data, patient_workspace_ai.evidence_context(data), compact=True
    )
...
def test_actual_same_sentence_documented_dose_and_missing_dose_is_rejected():
    ...
    with pytest.raises(ValueError, match="quotes a documented dose"):
        patient_workspace_ai.validate_comparison(
            result, data, patient_workspace_ai.evidence_context(data)
        )
...
def test_comparison_phases_keep_original_evidence_and_do_not_promote_generated_diagnosis(
    monkeypatch,
):
    monkeypatch.setattr(settings, "ai_backend", "llama_cpp")
    monkeypatch.setattr(ai, "model_status", lambda: {"ready": True})
    calls = []
    authored = comparison()
    authored["diagnosis_review"]["summary"] = (
        "Уникальная сформулированная моделью гипотеза требует проверки."
    )

    def complete(messages, schema, **kwargs):
        calls.append((copy.deepcopy(messages), schema))
        return json.dumps(
            {key: value for key, value in authored.items() if key in schema["properties"]}
        )

    monkeypatch.setattr(llama_adapter, "complete", complete)
    result = patient_workspace_ai.review(snapshot())
    assert len(calls) == 2
    assert list(calls[0][1]["properties"]) == [
        "case_version",
        "supporting",
        "discrepancies",
        "diagnosis_review",
    ...
    assert (
        calls[0][0][1]["content"].split("END DATA.")[0]
...
def test_echoing_doctors_diagnoses_is_not_a_specific_cited_discrepancy():
    output = comparison()
    output["diagnosis_review"].update(
        status="needs_review",
        summary="Врач исключил активный туберкулёз, но диагностировал внебольничную пневмонию.",
    )
...
def test_needs_review_must_cite_the_finding_that_supports_its_discrepancy():
    ...
    output["discrepancies"] = [
        {"text": "Наблюдение расходится с заключением врача.", "refs": ["F1", "E2"]}
    ]
...
def test_needs_review_accepts_specific_finding_and_conclusion_comparison():
    ...
    output["diagnosis_review"].update(
        status="needs_review",
        summary="Врач пишет, что пульс не измеряли, хотя в осмотре записан пульс 72 /min.",
    )
    output["discrepancies"] = [
        {
            "text": "Отрицание измерения пульса противоречит записанному значению 72 /min.",
            "refs": ["E1", "E2"],
        }
    ]
    assert (
        patient_workspace_ai.validate_comparison(
            output, data, patient_workspace_ai.evidence_context(data)
        )
        is output
    )
...
def test_generation_reference_limit_respects_provider_capacity_and_public_schema(
    monkeypatch, remote, limit
):
...
def test_openai_review_can_cite_all_five_sources_instead_of_omitting_conclusion(monkeypatch):
    ...
    data = snapshot()
    data["entries"].append(
        {"id": "lab", "category": "laboratory", "text": "A documented laboratory observation."}
    )
    output = comparison()
    ...
    def complete(messages, schema, **kwargs):
        if "treatment_review" in schema["properties"]:
            assert (
                len(output["treatment_review"]["refs"])
                <= schema["$defs"]["ReviewSection"]["properties"]["refs"]["maxItems"]
            )
        return json.dumps(
            {key: value for key, value in output.items() if key in schema["properties"]}
        )
