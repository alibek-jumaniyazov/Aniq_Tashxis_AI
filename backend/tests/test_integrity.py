...
def test_dicom_spatial_order_and_authenticated_frame(client):
    ...
    c = new_case(client)
    response = client.post(f'/api/v1/cases/{c["id"]}/imaging-studies', headers={'Idempotency-Key': 'ct-test-001'}, data={'expected_version': 1, 'deidentified_confirmed': 'true'}, files={'file': ('synthetic.zip', content, 'application/zip')})
...
def test_retry_deduplicates_alerts_and_notifications(client):
    ...
    retry = mutate(client, '/analyses/' + first['id'] + '/retry', {'expected_version': c['version']})
...
def test_note_draft_is_private_and_does_not_change_case(client):
    ...
    assert client.get(f'/api/v1/cases/{c["id"]}/drafts/note').json()['values']['text'] == values['text']
...
def test_future_event_is_excluded_even_when_availability_is_earlier(client):
    c = new_case(client)
    add(client, c, [fact('allergy.substance', 'DEMO-A'), fact('medication.substance', 'DEMO-A', order_status='active', event_time='2026-09-19T10:00:00+05:00')])
...
def test_note_context_does_not_leak_into_historical_ai_snapshot(client, monkeypatch):
    ...
    c = new_case(client)
    note = mutate(client, f'/cases/{c["id"]}/notes', {'expected_version': 1, 'text': 'Later clinician explanation', 'event_time': '2020-01-01T00:00:00+00:00'}).json()
...
def test_missing_values_never_create_substance_alert(client):
    c = new_case(client)
    add(client, c, [fact('allergy.substance', None), fact('medication.substance', None, order_status='active')])
...
def test_document_provenance_requires_actual_source(client):
    c = new_case(client)
    result = mutate(client, f'/cases/{c["id"]}/facts', {'expected_version': 1, 'facts': [fact('vital.pulse', '70', provenance='document')]}, method='patch')
