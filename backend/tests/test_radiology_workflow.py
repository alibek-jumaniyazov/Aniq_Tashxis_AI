    ...
    if color:
        ...
        pixels = np.stack(
            [
                ...
                if f == 0
                else np.flipud(np.arange(256, dtype=np.uint8).reshape(16, 16))
...
def test_non_ct_images_load_without_fake_ct_geometry(client, case, modality):
    ...
    result = client.post(
        ...
        headers={"Idempotency-Key": "modality-image"},
        data={"expected_version": case["version"], "deidentified_confirmed": "true"},
        files={"file": ("image.dcm", raw, "application/dicom")},
    )
...
def test_color_multiframe_image_keeps_channels_and_frame_navigation(client, case):
    ...
    result = client.post(
        f"/api/v1/cases/{case['id']}/imaging-studies",
        headers={"Idempotency-Key": "ultrasound"},
        data={"expected_version": case["version"], "deidentified_confirmed": "true"},
        files={"file": ("cine.dcm", raw)},
    )
    assert result.status_code == 201, result.text
    study = result.json()
    series = study["series"][0]
...
def test_measurement_uses_pixel_spacing_and_enforces_role_and_bounds(client, case):
    result = client.post(
        f"/api/v1/cases/{case['id']}/imaging-studies",
        headers={"Idempotency-Key": "measure-ct"},
        data={"expected_version": case["version"], "deidentified_confirmed": "true"},
        files={"file": ("phantom.zip", dicom_zip())},
    )
    study = result.json()
    series = study["series"][0]
    body = {
    ...
    assert (
        mutate(client, path, {**body, "points": [{"x": 99, "y": 0}, {"x": 1, "y": 1}]}).status_code
        == 422
    )
...
def upload_study(client, case):
    ...
    archive = io.BytesIO()
    with (
        zipfile.ZipFile(io.BytesIO(dicom_zip())) as original,
        zipfile.ZipFile(archive, "w") as changed,
    ):
    ...
    response = client.post(
        f"/api/v1/cases/{case['id']}/imaging-studies",
        headers={"Idempotency-Key": "image-job-study"},
        data={"expected_version": case["version"], "deidentified_confirmed": "true"},
        files={"file": ("phantom.zip", archive.getvalue())},
    )
...
def test_radiology_cannot_publish_after_concurrent_cancel(client, case, monkeypatch):
    ...
    def cancel_before_publish(db, user, case_id):
        ...
        with SessionLocal() as other_db:
            other_db.execute(
                update(Job)
                .where(Job.case_id == case_id, Job.status == "running")
    ...
    with SessionLocal() as db:
        assert not list(
            db.scalars(
                select(Record).where(Record.case_id == case["id"], Record.kind == "notification")
            )
        )
        assert not list(
            db.scalars(
                select(Audit).where(
                    Audit.resource_id == job_id, Audit.action == "imaging.analysis_completed"
                )
            )
        )
...
def test_malformed_vision_response_is_visible_and_radiologist_can_retry(client, case, monkeypatch):
    ...
    retried = mutate(
        client, f"/analyses/{job_id}/retry", {"expected_version": body["expected_version"]}
    )
...
def test_radiologist_cannot_retry_a_clinical_text_review(client, case):
    queued = mutate(
        client,
        f"/cases/{case['id']}/analyses",
        {"expected_version": case["version"], "include_ai": False},
    )
    assert queued.status_code == 202
    sign_in(client, "radiologist")
    response = mutate(
        client,
        "/analyses/" + queued.json()["run_id"] + "/retry",
