...
from app.db import Case, SessionLocal, now
...
def test_demo_identities_and_existing_sessions_disabled_in_production(client, monkeypatch):
    ...
    response = client.post('/api/v1/auth/login', json={'email': 'developer@demo.aniq', 'password': 'AniqDemo!2026'})
...
def register(client, email='owner@example.org', plan='clinic10'):
    response = client.post('/api/v1/auth/register', json={'clinic_name': 'Navro‘z klinikasi', 'name': 'Owner Example', 'email': email, 'password': 'StrongDemo!2026', 'phone': '+998 90 123 45 67', 'plan_id': plan})
...
def login(client, email):
    response = client.post('/api/v1/auth/login', json={'email': email, 'password': 'StrongDemo!2026'})
...
def submit(client, plan='clinic10', content=None, **extra):
    ...
    data = {'plan_id': plan, 'payment_method_id': method['id'], 'plan_version': selected['version'], 'payment_method_version': method['version'], **extra}
    return client.post('/api/v1/billing/requests', data=data, files={'file': ('receipt.png', content or png(), 'image/png')}, headers={'Idempotency-Key': str(uuid4())})


def approve(client, request):
    sign_in(client, 'developer')
    response = client.post(f"/api/v1/developer/subscription-requests/{request['id']}/review", json={'expected_version': request['version'], 'decision': 'approved', 'note': 'Test fixture: external transfer verified.'})
    assert response.status_code == 200, response.text
...
def test_public_prices_registration_and_pending_entitlement(client):
    ...
    assert [(p['price_uzs'], p['doctor_limit']) for p in plans['items']] == [(150000, 1), (1390000, 10), (2390000, 25), (None, None)]
...
def test_receipt_approval_exactly_once_and_calendar_renewal(client):
    ...
    repeat = client.post(f"/api/v1/developer/subscription-requests/{request['id']}/review", json={'expected_version': 1, 'decision': 'approved', 'note': 'Repeat submission'})
    ...
    assert submit(client, plan='clinic10', content=png('yellow')).json()['error']['code'] == 'ACTIVE_PLAN_CHANGE_REQUIRES_SUPPORT'
    ...
    assert datetime.fromisoformat(result['subscription']['expires_at']) == next_month(datetime.fromisoformat(expiry))
...
def test_pending_duplicates_receipt_dedup_and_owner_isolation(client):
    ...
    assert duplicate.status_code == 409 and duplicate.json()['error']['code'] == 'RECEIPT_ALREADY_SUBMITTED'
...
def test_rejection_does_not_activate_and_new_receipt_can_be_submitted(client):
    ...
    request = submit(client, plan='solo').json()
    sign_in(client, 'developer')
    response = client.post(f"/api/v1/developer/subscription-requests/{request['id']}/review", json={'expected_version': 1, 'decision': 'rejected', 'note': 'Receipt amount does not match.'})
...
def test_immutable_plan_snapshot_and_price_version_check(client):
    ...
    assert submit(client, payment_method_version=99).json()['error']['code'] == 'PAYMENT_METHOD_CHANGED'
    request = submit(client).json()
    sign_in(client, 'developer')
    changed = client.patch('/api/v1/developer/plans/clinic10', json={'expected_version': 1, 'price_uzs': 1500000, 'doctor_limit': 11})
    assert changed.status_code == 200, changed.text
    assert client.patch('/api/v1/developer/plans/clinic10', json={'expected_version': 1, 'price_uzs': 1500001}).status_code == 409
    approved = client.post(f"/api/v1/developer/subscription-requests/{request['id']}/review", json={'expected_version': 1, 'decision': 'approved', 'note': 'Earlier quoted price honored.'})
...
def test_team_seats_activation_role_security_and_session_revocation(client):
    ...
    for index, role in enumerate(['doctor', 'radiologist']):
        response = client.post('/api/v1/billing/team', json={'name': f'Doctor {index}', 'email': f'doc{index}@example.org', 'password': 'StrongDemo!2026', 'role': role})
        assert response.status_code == 201, response.text
        created.append(response.json())
    full = client.post('/api/v1/billing/team', json={'name': 'Third Doctor', 'email': 'doc3@example.org', 'password': 'StrongDemo!2026', 'role': 'expert'})
    ...
    with TestClient(app) as doctor:
        ...
        assert doctor.post('/api/v1/billing/team', json={'name': 'No permission', 'email': 'forbid@example.org', 'password': 'StrongDemo!2026', 'role': 'doctor'}).status_code == 403
        response = client.patch(f"/api/v1/billing/team/{created[0]['id']}", json={'expected_version': 1, 'active': False})
        assert response.status_code == 200, response.text
        assert doctor.get('/api/v1/auth/me').status_code == 401
    assert client.post('/api/v1/billing/team', json={'name': 'Replacement Doctor', 'email': 'new@example.org', 'password': 'StrongDemo!2026', 'role': 'doctor'}).status_code == 201
    assert client.patch(f"/api/v1/billing/team/{created[0]['id']}", json={'expected_version': 2, 'active': True}).status_code == 409
    assert client.post('/api/v1/billing/team', json={'name': 'Escalation', 'email': 'evil@example.org', 'password': 'StrongDemo!2026', 'role': 'developer'}).status_code == 422
...
def test_subscription_expiry_and_suspend_preserve_records(client):
    ...
    approve(client, request)
    login(client, 'owner@example.org')
    created = client.post('/api/v1/cases', json={'full_name': 'Subscription Patient', 'age': 40, 'sex': 'male'}, headers={'Idempotency-Key': str(uuid4())})
    ...
    response = client.patch(f"/api/v1/developer/clinics/{user['tenant_id']}", json={'expected_version': clinic['version'], 'status': 'suspended'})
    assert response.status_code == 200
    login(client, 'owner@example.org')
...
def test_upload_validation_and_csrf(client):
    register(client)
    method = client.get('/api/v1/billing/payment-methods').json()['items'][0]
    data = {'plan_id': 'clinic10', 'plan_version': 1, 'payment_method_id': method['id'], 'payment_method_version': method['version']}
    for name, content, mime in [('fake.png', b'<script>bad</script>', 'image/png'), ('fake.svg', b'<svg/>', 'image/svg+xml'), ('fake.pdf', b'%PDF-notvalid', 'application/pdf')]:
        result = client.post('/api/v1/billing/requests', data=data, files={'file': (name, content, mime)}, headers={'Idempotency-Key': str(uuid4())})
        assert result.status_code == 422, result.text
    assert client.post('/api/v1/billing/requests', data=data, files={'file': ('receipt.png', png(), 'image/png')}, headers={'Idempotency-Key': str(uuid4()), 'X-CSRF-Token': 'bad'}).status_code == 403
    assert client.post('/api/v1/billing/requests', data=data, files={'file': ('receipt.png', b'x' * (10 * 1024 * 1024 + 1), 'image/png')}, headers={'Idempotency-Key': str(uuid4())}).status_code == 413
...
def test_pdf_receipt_passive_only(client):
    ...
    for scripted in [True, False]:
        ...
        stream = io.BytesIO()
        writer.write(stream)
        result = client.post('/api/v1/billing/requests', data=data, files={'file': ('receipt.pdf', stream.getvalue(), 'application/pdf')}, headers={'Idempotency-Key': str(uuid4())})
...
def test_review_race_grants_one_period(client):
    ...
    sign_in(client, 'developer')
    def review(_):
        return client.post(f"/api/v1/developer/subscription-requests/{request['id']}/review", json={'expected_version': 1, 'decision': 'approved', 'note': 'Concurrent approval fixture'}).status_code
...
def test_receipt_idempotency_replays_one_application(client):
    ...
    results = [client.post('/api/v1/billing/requests', data=data, files={'file': ('receipt.png', png(), 'image/png')}, headers=headers) for _ in range(2)]
...
def test_concurrent_team_creation_cannot_exceed_seat_limit(client):
    ...
    login(client, 'owner@example.org')
    def create(index):
        return client.post('/api/v1/billing/team', json={'name': f'Concurrent Doctor {index}', 'email': f'race{index}@example.org', 'password': 'StrongDemo!2026', 'role': 'doctor'}).status_code
    with ThreadPoolExecutor(max_workers=2) as pool:
...
def test_developer_cannot_browse_patient_records_or_modify_owner_role(client, case):
    ...
    result = client.patch('/api/v1/developer/users/' + owner, json={'expected_version': 1, 'active': False})
    assert result.status_code == 403 and result.json()['error']['code'] == 'PROTECTED_ACCOUNT'


@pytest.mark.parametrize('start,expected', [('2026-01-31', '2026-02-28'), ('2028-01-31', '2028-02-29'), ('2026-12-31', '2027-01-31')])
