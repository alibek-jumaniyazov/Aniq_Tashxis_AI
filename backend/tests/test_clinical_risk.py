...
def score(sex="male", age=55, years=10, **inputs):
    return calculate(
        SimpleNamespace(age=age, sex=sex),
        ForecastCreate(expected_version=1, horizon_years=years, inputs={**BASE, **inputs}),
    )


@pytest.mark.parametrize(
    "sex,expected", [("male", 0.13534025563815144), ("female", 0.08068318155070242)]
)
def test_published_fhs_equation_reference_vectors(sex, expected):
    ...
    mmol = score(
        sex, total_cholesterol=213 / 38.67, hdl_cholesterol=50 / 38.67, lipid_unit="mmol/L"
    )
...
@pytest.mark.parametrize(
    "overrides,code",
    [
        ({"age": 29}, "FHS_AGE_OUTSIDE_RANGE"),
        ({"age": 75}, "FHS_AGE_OUTSIDE_RANGE"),
        ({"baseline_cvd": True}, "FHS_BASELINE_CVD"),
        ({"confirmed": False}, "RISK_INPUT_CONFIRMATION_REQUIRED"),
        ({"years": 5}, "UNSUPPORTED_HORIZON"),
        ({"hdl_cholesterol": 250}, "FHS_INPUT_OUTSIDE_SUPPORTED_RANGE"),
    ],
)
...
def test_risk_history_preserves_input_and_version_and_role(client, case):
    ...
    assert (
        original["case_version"] < case["version"]
        and original["inputs"]["total_cholesterol"] == 213
    )
...
def test_decision_review_respects_time_negation_and_confirmation(client):
    ...
    add(
        ...
        c,
        [
            fact("allergy.substance", "Substance A", "2026-09-18T14:00:00+05:00"),
            fact("medication.substance", "Substance A", order_status="active"),
            fact("vital.pulse", "90", confirmed=False),
            fact("imaging.side", "left", assertion="absent"),
        ],
    )
    current = run(client, c, include_ai=False)["result"]["decision_review"]
    assert (
        next(x for x in current["checks"] if x["code"] == "substance_overlap")["status"]
        == "attention"
    )
    historical = run(
        client, c, include_ai=False, mode="decision_time", decision_time="2026-09-18T10:00:00+05:00"
    )["result"]["decision_review"]
...
def test_clinical_conclusion_requires_confirmed_current_evidence(client):
    ...
    assert (
        mutate(
            client,
            f"/cases/{c['id']}/analyses",
            {"expected_version": 1, "review_focus": "clinical_assessment"},
        ).status_code
        == 422
    )
    ...
    body = {
        ...
        "diagnosis": "Clinician provisional assessment",
        "status": "provisional",
        "rationale": "Reviewed the confirmed source records",
        "fact_ids": [facts[0]["id"]],
...
def test_model_hypotheses_cannot_cite_invented_or_symptom_free_evidence():
    ...
    result = {
        "case_version": 1,
        "summary": "Cough documented",
        "concerns": [],
        "limitations": [],
        "missing_fields": [],
        "assessment": {
            "status": "requires_clinician_review",
            "differential": [{"supporting_refs": ["F99"], "opposing_refs": []}],
        },
    }
...
def test_clinical_output_must_match_the_immutable_case_version():
    ...
    result = {
        "case_version": 1,
        "summary": "More confirmed observations are required.",
        "concerns": [],
        "limitations": [],
        "missing_fields": [],
