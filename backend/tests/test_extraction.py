...
def source(client):
    c = new_case(client)
    uploaded = client.post(f'/api/v1/cases/{c["id"]}/documents', headers={'Idempotency-Key': str(uuid4())}, data={'expected_version': 1, 'identity_confirmed': 'true'}, files={'file': ('note.txt', b'Pulse recorded as 72 /min.', 'text/plain')})
...
def test_ai_extraction_preserves_exact_source_and_human_confirmation(client, monkeypatch):
    ...
    proposed = ExtractedFact(key='vital.pulse', label='Pulse', value='72', unit='/min', assertion='present', page=1, quote='Pulse recorded as 72 /min.')
    calls = []
    def extract(pages):
        calls.append(pages)
        return [proposed]
    ...
    key = str(uuid4())
    result = mutate(client, f'/documents/{document["id"]}/extract', {'expected_version': 2}, key=key)
    assert result.status_code == 200, result.text
    assert result.json()['count'] == 1
    replay = mutate(client, f'/documents/{document["id"]}/extract', {'expected_version': 2}, key=key)
...
def test_ai_extraction_rejects_invented_source_quote(client, monkeypatch):
    c, document = source(client)
    proposed = ExtractedFact(key='vital.pulse', label='Pulse', value='100', assertion='present', page=1, quote='Pulse is 100')
...
def test_real_extractor_copies_literal_unit_and_rejects_invented_unit(monkeypatch):
    ...
    fact = {'key': 'vital.pulse', 'label': 'Pulse', 'value': '70', 'unit': None, 'assertion': 'present', 'page': 1, 'quote': 'Pulse: 70 /min.'}
