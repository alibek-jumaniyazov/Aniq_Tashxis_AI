...
def test_seed_references_files_timeline_and_idempotency(seeded):
    ...
    with SessionLocal() as db:
        ...
        for job in db.scalars(select(Job)):
            ...
            assert all(
                f["case_version"] <= job.case_version for f in job.payload["snapshot"]["facts"]
            )
...
def test_radiologist_frames_and_review_persist(seeded):
    ...
    frame = c.get(
        f"/api/v1/imaging-studies/{study['id']}/series/{series['id']}/frames/{series['count'] - 1}"
    )
    assert frame.status_code == 200 and frame.content.startswith(b"\x89PNG")
    response = post(
        c,
        f"/imaging-findings/{study['id']}/reviews",
        {
            "status": "clarified",
            "comment": "Synthetic phantom inspected; no clinical interpretation.",
        },
    )
    ...
    assert (
        post(
            c,
            f"/imaging-findings/{study['id']}/reviews",
            {"status": "confirmed", "comment": "Unauthorized doctor"},
        ).status_code
        == 403
    )
...
def test_expert_pending_decision_and_doctor_confirmation(seeded):
    ...
    sign_in(c, "expert")
    incident = next(
        r for r in c.get("/api/v1/incidents").json()["items"] if r["status"] == "under_review"
    )
    result = post(
        c,
        f"/incidents/{incident['id']}/decisions",
        {
            ...
            "explanation": "Please clarify source availability before a final decision.",
        },
    )
    ...
    result = post(
        c,
        f"/cases/{intake['id']}/facts/confirm",
        {"expected_version": detail["version"], "fact_ids": [f["id"] for f in detail["facts"]]},
    )
    assert result.status_code == 200, result.text
    ...
    analysis = post(
        c,
        f"/cases/{intake['id']}/analyses",
        {"expected_version": refreshed["version"], "include_ai": False},
    )
...
def test_revisions_and_late_results_are_not_silently_overwritten(seeded):
    ...
    cases = c.get("/api/v1/cases?page_size=100").json()["items"]
    corrected = c.get(
        "/api/v1/cases/" + next(r["id"] for r in cases if r["alias"].endswith("-005"))
    ).json()
    ...
    late = c.get(
        "/api/v1/cases/" + next(r["id"] for r in cases if r["alias"].endswith("-006"))
    ).json()
    historical = next(r for r in late["analyses"] if r["mode"] == "decision_time")
    current = next(
        r for r in late["analyses"] if r["mode"] == "current" and r["kind"] == "analysis"
    )
...
def test_current_patient_sections_sources_and_language_variants(seeded):
    ...
    for patient in patients:
        ...
        assert {entry["category"] for entry in entries["items"]} == {
            "subjective",
            "objective",
            "laboratory",
            "instrumental",
        ...
        for run in runs:
            ...
            assert (
                report["validated_probability"] is False
                and report["requires_clinician_review"] is True
            )
            assert run["result"]["demo_only"] and run["result"]["model_id"] == "synthetic-demo"
            entry_ids = {entry["id"] for entry in entries["items"]}
            assert all(
                item["entry_id"] in entry_ids for item in report["evidence"] if "entry_id" in item
            )
...
def test_tb_same_cutoff_and_source_grounded_difference(seeded):
    ...
    for suffix, expected in (("-009", "consistent_with_data"), ("-015", "needs_review")):
        ...
        for run in runs:
            ...
            with SessionLocal() as db:
                assert (
                    db.get(Record, lab["entry_id"]).created_at
                    < db.get(Record, conclusion["entry_id"]).created_at
                )
...
def test_documented_regimens_have_grounded_complete_treatment_reviews(seeded):
    ...
    for suffix, dose in (("-005", "5"), ("-012", "500")):
        ...
        for run in runs:
            ...
            assert any(
                references[ref]["category"] in {"laboratory", "objective"}
                for ref in section["refs"]
            )
            assert not any(
                phrase in section["summary"].casefold()
                for phrase in ("не указана", "не представлены", "without the regimen", "sxema yo‘q")
            )
...
def test_clinic_owner_receipts_and_developer_approval_are_usable(seeded):
    ...
    reviewed = post(
        c,
        f"/developer/subscription-requests/{request['id']}/review",
        {
...
def test_reset_preserves_access_config_only_without_old_patient_data(seeded, tmp_path):
    ...
    spec = importlib.util.spec_from_file_location(
        "reset_demo_test", Path(__file__).resolve().parents[2] / "scripts/reset_demo.py"
    )
    ...
    with closing(sqlite3.connect(old_path)) as previous, previous:
        previous.execute(
            "UPDATE users SET password_hash='retained-hash' WHERE email='developer@demo.aniq'"
        )
        previous.execute(
            "INSERT INTO users VALUES ('custom-dev', 'custom-platform', 'maintainer@example.test', 'Maintainer', 'developer', 'custom-hash', 1)"
        )
        previous.execute(
            "INSERT INTO users VALUES ('old-customer', 'old-clinic', 'customer@example.test', 'Previous customer', 'doctor', 'old-hash', 1)"
        )
        previous.execute(
            "UPDATE billing_payment_methods SET instructions='Retain configured public instructions'"
        )
        previous.execute(
            "UPDATE billing_plans SET description='Retain custom plan description' WHERE id='clinic10'"
        )
    ...
    with closing(sqlite3.connect(renamed_staged)) as staged:
        assert (
            staged.execute(
                "SELECT password_hash FROM users WHERE email='developer@demo.aniq'"
            ).fetchone()[0]
            == "retained-hash"
        )
        assert (
            staged.execute("SELECT role FROM users WHERE id='custom-dev'").fetchone()[0]
            == "developer"
        )
        assert staged.execute("SELECT 1 FROM users WHERE id='old-customer'").fetchone() is None
        assert (
            staged.execute(
                "SELECT 1 FROM cases WHERE full_name='Old history must not be copied'"
            ).fetchone()
            is None
        )
        assert (
            staged.execute("SELECT instructions FROM billing_payment_methods").fetchone()[0]
            == "Retain configured public instructions"
        )
        assert (
            staged.execute("SELECT description FROM billing_plans WHERE id='clinic10'").fetchone()[
                0
            ]
            == "Retain custom plan description"
        )
...
def test_reset_fresh_subprocess_registers_all_tables_before_seed(tmp_path):
    ...
    spec = importlib.util.spec_from_file_location(
        "reset_fresh_process", root / "scripts/reset_demo.py"
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    environment = {
    ...
    result = subprocess.run(
        [sys.executable, "-c", module.BUILD_CODE],
        cwd=root,
        env=environment,
        capture_output=True,
        text=True,
