...
def test_mri_and_xray_zip_open_real_decoded_frame(client, case, modality):
    ...
    uploaded = client.post(
        f"/api/v1/cases/{case['id']}/imaging-studies",
        headers={"Idempotency-Key": "zip-modality-import"},
        data={"expected_version": case["version"], "deidentified_confirmed": "true"},
        files={"file": ("study.zip", archive.getvalue(), "application/zip")},
    )
...
def test_report_versions_persist_and_make_previous_image_review_stale(client, case):
    ...
    body = {
        "expected_version": frame["expected_version"],
        "radiologist_report": "  Контуры чёткие. Требуется просмотр остальных серий.  ",
    }
    ...
    assert (
        mutate(
            ...
            {
                **body,
                "expected_version": frame["expected_version"] + 1,
                "radiologist_report": "   ",
            },
        ).status_code
        == 422
    ...
    update = mutate(
        ...
        path,
        {
            **body,
            "expected_version": frame["expected_version"] + 1,
            "radiologist_report": "Уточнённое заключение.",
        },
    )
...
def test_report_source_is_bound_to_patient_and_quote(client, case):
    ...
    document = client.post(
        ...
        headers={"Idempotency-Key": "radiology-report-doc"},
        data={"expected_version": frame["expected_version"], "identity_confirmed": "true"},
        files={
            "file": (
                "report.txt",
                "Описание: ровные контуры.\nЗаключение требует проверки.".encode(),
            )
        },
    )
    ...
    body = {
        **frame,
        "expected_version": frame["expected_version"] + 1,
        "radiologist_report": "Ровные контуры.",
        "report_source_id": source["id"],
        "report_quote": "Описание: ровные контуры.",
    }
    path = f"/imaging-studies/{study['id']}/analyses"
    invalid_quote = mutate(client, path, {**body, "report_quote": "Этого нет в документе"})
    assert (
        invalid_quote.status_code == 422
        and invalid_quote.json()["error"]["code"] == "REPORT_QUOTE_MISMATCH"
    )
    ...
    with SessionLocal() as db:
        unrelated = Record(
            ...
            if "tenant_id" in case
            else db.scalar(select(Record).where(Record.id == study["id"])).tenant_id,
            case_id=other["id"],
            actor_id=db.scalar(select(Record).where(Record.id == study["id"])).actor_id,
            kind="source",
            case_version=1,
            data={"text": body["report_quote"]},
        )
        db.add(unrelated)
        db.commit()
    ...
    assert (
        mismatch.status_code == 422 and mismatch.json()["error"]["code"] == "REPORT_SOURCE_MISMATCH"
    )
    ...
    assert (
        result["report_source_id"] == source["id"]
        and result["report_quote"] == body["report_quote"]
    )
...
def test_report_comparison_is_grounded_in_selected_frame_and_snapshot(client, case, monkeypatch):
    ...
    captured = []
    output = {
        "observations": ["На кадре виден округлый контур."],
        "limitations": ["Один выбранный кадр."],
        "report_comparison": {
            "status": "possible_discrepancy",
            "explanation": "Локализацию необходимо сверить по всей серии.",
            "points_to_verify": ["Просмотреть соседние срезы."],
        },
    }
    ...
    queued = mutate(
        client, f"/imaging-studies/{study['id']}/analyses", {**body, "radiologist_report": report}
    )
    ...
    assert (
        result["scope"] == "selected_frame_only"
        and result["clinical_validation"] == "not_validated"
    )
...
def test_phantom_never_validates_clinical_report(client, case, monkeypatch):
    ...
    real_client = httpx.Client
    output = {
        "observations": ["Геометрический фантом."],
        "limitations": ["Не анатомия."],
        "report_comparison": {
            "status": "supported_on_selected_frame",
            "explanation": "Ошибочное подтверждение.",
            "points_to_verify": ["Проверить."],
        },
    }
    monkeypatch.setattr(settings, "ai_backend", "llama_cpp")
    monkeypatch.setattr(ai, "model_status", lambda: {"ready": True, "vision_ready": True})
    ...
    queued = mutate(
        ...
        {**body, "radiologist_report": "Клиническое заключение."},
    )
    result = client.get("/api/v1/analyses/" + queued.json()["run_id"]).json()["result"]
...
def test_missing_report_comparison_is_not_silently_published(client, case, monkeypatch):
    ...
    queued = mutate(
        client,
        f"/imaging-studies/{study['id']}/analyses",
        {**body, "radiologist_report": "Нужна проверка."},
    )
    result = client.get("/api/v1/analyses/" + queued.json()["run_id"]).json()["result"]
