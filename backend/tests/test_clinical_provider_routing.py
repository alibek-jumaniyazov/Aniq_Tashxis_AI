...
META = {'provider': 'openai', 'model_id': 'test-openai-model', 'model_revision': 'test',
        'provenance': 'openai', 'ai_backend': 'openai', 'quantization': None}
...
def provider(monkeypatch):
    ...
    def install(output):
        def complete(messages, schema, **kwargs):
            ...
            return json.dumps({key: value for key, value in data.items() if key in schema['properties']}, ensure_ascii=False)
...
def test_provider_summary_keeps_language_and_omits_administrative_identifiers(provider, language):
    ...
    calls = provider({'case_version': 4, 'summary': PROSE[language][0], 'concerns': [],
...
def test_provider_differential_preserves_confirmed_evidence_and_metadata(provider):
    data = snapshot('en')
    calls = provider({'case_version': 4, 'summary': PROSE['en'][0], 'concerns': [],
                      'limitations': [PROSE['en'][2]], 'missing_fields': [],
...
def test_provider_extraction_preserves_source_quotes_and_excludes_page_metadata(provider):
    calls = provider({'facts': [{'key': 'vital.pulse', 'label': 'Pulse', 'value': '72', 'unit': '/min',
                                'assertion': 'present', 'page': 1, 'quote': 'Pulse 72 /min.'}]})
    result = ai.extract_document([{'page': 1, 'text': 'Pulse 72 /min.', 'patient_phone': 'PRIVATE-PHONE',
...
def test_provider_extraction_never_accepts_an_invented_quote(provider):
    provider({'facts': [{'key': 'vital.pulse', 'label': 'Pulse', 'value': '99', 'unit': '/min',
                        'assertion': 'present', 'page': 1, 'quote': 'Pulse 99 /min.'}]})
...
def test_documentation_job_persists_actual_provider_metadata(client, provider):
    ...
    provider(lambda messages, schema: {
        'case_version': schema['properties']['case_version']['const'],
        'summary': 'Зафиксирован пульс 72 /min.', 'concerns': [],
        'limitations': ['Результат требует проверки врачом.'], 'missing_fields': [],
    })
