...
def provider(monkeypatch):
    ...
    extraction = Mock(
        return_value=[SimpleNamespace(model_dump=lambda: {"key": "vital.pulse", "value": "70"})]
    )
...
def test_smoke_forwards_language_and_reports_actual_provider(
    monkeypatch, provider, tmp_path, capsys, language
):
...
def test_status_only_skips_inference_and_uses_readiness_exit_code(
    monkeypatch, provider, tmp_path, ready, exit_code
):
