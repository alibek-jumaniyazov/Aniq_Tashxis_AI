...
@pytest.mark.parametrize(
    "summary", ["A 40-year-old male with pulse 70.", "Женщина, 40 лет, пульс 70."]
)
...
def test_summary_cannot_label_a_documented_value_as_missing():
    with pytest.raises(ValueError, match="provided value as missing"):
        validate_summary(
            {"summary": "Pulse documented.", "missing_fields": ["vital.pulse"]},
            {"facts": [{"key": "vital.pulse", "value": "70"}]},
        )
    ...
    context = model_context(
        {
            ...
            "facts": [
                {"id": "fact-id", "actor_id": "private-actor", "key": "vital.pulse", "value": "70"}
            ],
            "notes": [{"actor_id": "private-actor", "text": "Documented context"}],
        }
    )
...
def test_local_provider_validates_token_budget_and_structured_request(monkeypatch):
    ...
    def handle(request):
        ...
        return httpx.Response(
            200,
            json={
                "choices": [{"finish_reason": "stop", "message": {"content": json.dumps(expected)}}]
            },
        )
...
def test_explicit_structured_output_budget_reserves_context_without_truncating_sources(monkeypatch):
    ...
    def handle(request):
        ...
        if request.url.path == "/props":
            return httpx.Response(
                200, json={"default_generation_settings": {"n_ctx": context_limit}}
            )
...
def test_expired_adapter_budget_stops_requests_and_rejects_late_valid_response(
    monkeypatch, late_stage
):
    real_client = httpx.Client
    clock = [0.0]
    observed = []
