...
def snapshot():
    return {'version': 2, 'age': 45, 'sex': 'male', 'facts': [
        {'id': 'symptom', 'source_id': 'source1', 'key': 'symptom.complaint', 'label': 'Complaint', 'value': 'Кашель 3 дня', 'confirmed': True, 'assertion': 'present'},
        {'id': 'pulse', 'source_id': 'source2', 'key': 'vital.pulse', 'label': 'Pulse', 'value': '70', 'unit': '/min', 'confirmed': True, 'assertion': 'present'},
    ]}
...
def result(*, english=False, questions=True):
    return {
        'case_version': 2,
        'summary': 'A 45-year-old male with cough for 3 days and pulse 70/min.' if english else 'Мужчина 45 лет. Кашель 3 дня, пульс 70 /min.',
        'concerns': [],
        'limitations': ['The assessment needs clinician review.'] if english else ['Оценка требует проверки врачом; клиническая валидация не выполнена.'],
        'missing_fields': [],
        'assessment': {'status': 'insufficient_data', 'differential': [], 'questions': ['Есть ли повышение температуры на фоне кашля?'] if questions else []},
    }
...
def test_differential_prompt_keeps_observation_timing_and_medication_status(monkeypatch):
    data = snapshot()
    data['facts'][0].update(event_time='2026-09-01T10:00:00+05:00', available_time='2026-09-01T10:05:00+05:00')
    data['facts'].append({'id': 'order', 'source_id': 'source3', 'key': 'medication.substance',
                          'label': 'Medication', 'value': 'Documented substance', 'confirmed': True,
                          'assertion': 'present', 'order_status': 'stopped'})
    output = result()
    ...
    assert len(calls) == 1
    context = json.loads(calls[0][1]['content'].split('UNTRUSTED CASE DATA\n')[1].split('\nEND DATA.')[0])
...
def test_source_failure_is_rejected_without_a_presentation_retry(monkeypatch):
    ...
    invented['assessment'] = {'status': 'requires_clinician_review', 'questions': [], 'differential': [
        {'label': 'Unsupported hypothesis', 'supporting_refs': ['F99'], 'opposing_refs': [], 'verification_needed': 'Check evidence.'},
    ]}
