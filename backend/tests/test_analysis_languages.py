...
def test_analysis_language_reaches_worker_and_persisted_result(client, monkeypatch, language):
    ...
    def model(snapshot, *args):
        ...
        return {
            ...
            "summary": "Synthetic transport fixture",
            "concerns": [],
            "limitations": [],
            "missing_fields": [],
        }
...
def test_locale_retry_creates_new_snapshot_without_rewriting_the_original(client, monkeypatch):
    ...
    with SessionLocal() as db:
        ...
        db.commit()
    same = mutate(
        client, f"/analyses/{first}/retry", {"expected_version": case["version"], "language": "ru"}
    )
    assert same.status_code == 409
    retry = mutate(
        client, f"/analyses/{first}/retry", {"expected_version": case["version"], "language": "uz"}
    )
...
def test_unsupported_language_rejected_and_missing_language_is_backward_compatible(
    client, monkeypatch
):
    case = new_case(client)
    monkeypatch.setattr(jobs, "dispatch", lambda _: None)
    ...
    response = mutate(
        client, f"/cases/{case['id']}/analyses", {"expected_version": case["version"]}
    )
    assert response.status_code == 202
    job = client.get("/api/v1/analyses/" + response.json()["run_id"]).json()
...
def test_language_change_does_not_bypass_case_version_check(client, monkeypatch):
    ...
    retry = mutate(
        client,
        f"/analyses/{job_id}/retry",
        {"expected_version": case["version"] + 1, "language": "en"},
    )
    assert retry.status_code == 409
    assert retry.json()["error"]["code"] == "CASE_VERSION_CONFLICT"


def test_retry_of_seed_comparison_becomes_real_inference_without_changing_example(
    client, monkeypatch
):
    case = ready_case(client)
    monkeypatch.setattr(jobs, "dispatch", lambda _: None)
