...
def seo_client(monkeypatch):
    monkeypatch.setattr(seo, 'seo_settings', seo.SeoSettings(_env_file=None, site_url='https://clinic.example.org', seo_indexing_enabled=True))
    app = FastAPI()
...
@pytest.mark.parametrize('language, title', [('ru', 'AI-помощник'), ('uz', 'shifokor uchun'), ('en', 'Clinical AI Assistant')])
def test_public_html_has_localized_server_content_and_trusted_urls(seo_client, language, title):
    response = seo_client.get(f'/?lang={language}&patient=SECRET-PATIENT', headers={'host': 'attacker.invalid', 'x-forwarded-host': 'attacker.invalid'})
...
@pytest.mark.parametrize('path', ['/login?next=/cases/SECRET-PATIENT', '/checkout?name=SECRET-PATIENT', '/cases/SECRET-PATIENT', '/cases/SECRET-PATIENT/imaging', '/settings/profile', '/developer', '/expert', '/reports', '/api/v1/cases', '/docs', '/openapi.json'])
...
def test_sitemap_contains_only_real_public_language_urls(seo_client):
    ...
    locations = [item.find('{http://www.sitemaps.org/schemas/sitemap/0.9}loc').text for item in urls]
    assert locations == ['https://clinic.example.org/', 'https://clinic.example.org/?lang=uz', 'https://clinic.example.org/?lang=en']
...
def test_unconfigured_local_site_has_no_invented_canonical_or_sitemap(seo_client, monkeypatch):
    monkeypatch.setattr(seo, 'seo_settings', seo.SeoSettings(_env_file=None, site_url='', seo_indexing_enabled=True))
...
@pytest.mark.parametrize('origin', ['https://localhost', 'https://127.0.0.1', 'https://192.168.1.3', 'https://staging.local', 'https://clinic.test', 'http://clinic.example.org'])
...
@pytest.mark.parametrize('origin', ['javascript:alert(1)', 'https://name:secret@clinic.example.org', 'https://clinic.example.org/path', 'https://clinic.example.org?x=1', 'https://clinic.example.org#part', 'https://clinic.example.org\n<script>'])
