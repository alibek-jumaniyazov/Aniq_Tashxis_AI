...
def mark_all(client, key=None, **headers):
    return client.post('/api/v1/notifications/read-all', headers={'Idempotency-Key': key or str(uuid4()), **headers})
...
def seed_notifications(case_id):
    with SessionLocal() as db:
        ...
        db.add_all([CaseAccess(case_id=granted.id, user_id=doctor.id), CaseAccess(case_id=foreign.id, user_id=doctor.id)])
        visible = []
        for index in range(105):
            readers = [doctor.id] if index == 0 else [expert.id] if index == 1 else []
            visible.append(create_record(db, doctor, 'notification', {'status': 'completed', 'read_by': readers}, owned).id)
        visible.append(create_record(db, expert, 'notification', {'status': 'partial', 'read_by': []}, granted).id)
        hidden = [
            create_record(db, expert, 'notification', {'status': 'completed', 'read_by': []}, private).id,
            create_record(db, other, 'notification', {'status': 'completed', 'read_by': []}, foreign).id,
...
def test_mark_all_includes_history_and_preserves_scope_and_readers(client, case):
    ...
    with SessionLocal() as db:
        assert all(db.get(Record, record_id).data['read_by'].count(doctor_id) == 1 for record_id in visible)
        ...
        create_record(db, doctor, 'notification', {'status': 'partial', 'read_by': []}, db.get(Case, case['id']))
        db.commit()
...
def test_single_read_preserves_reader_added_after_access_check(client, case, monkeypatch):
    ...
    with SessionLocal() as db:
        ...
        record_id = create_record(db, doctor, 'notification', {'status': 'completed', 'read_by': []}, db.get(Case, case['id'])).id
        db.commit()
    ...
    response = client.post(f'/api/v1/notifications/{record_id}/read', headers={'Idempotency-Key': str(uuid4())})
