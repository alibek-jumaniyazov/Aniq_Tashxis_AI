...
PROSE = {
    'ru': ('На кадре виден градиент интенсивности.', 'Один кадр не позволяет оценить всё исследование.', 'Для проверки заключения требуется полное исследование.', 'Просмотрите все исходные серии.'),
    'uz': ('Ushbu tasvirda intensivlik gradienti ko‘rinadi.', 'Bu tasvir bilan barcha seriyalarni baholash mumkin emas.', 'Xulosani tekshirish uchun to‘liq tekshiruv kerak.', 'Barcha asl seriyalarni ko‘rib chiqing.'),
    'en': ('The image shows an intensity gradient.', 'A single frame cannot assess the entire study.', 'The complete study is needed to assess the report.', 'Review every original series.'),
}
...
def localized_output(language, *, sample=False):
    ...
    output = {
        ...
        'report_comparison': {'status': 'not_assessable', 'explanation': explanation, 'points_to_verify': [point]},
    }
    if sample:
        output['frame_assessments'] = [{'frame_ref': f'F{i + 1}', 'quality': 'limited', 'observations': [observation], 'limitations': [limitation]} for i in range(3)]
...
def queue_mock_review(client, case, monkeypatch, outputs, *, language='ru', sample=False):
    ...
    def respond(request):
        captured.append({'body': json.loads(request.content), 'timeout': request.extensions['timeout']['read']})
    ...
    response = mutate(client, f'/imaging-studies/{study["id"]}/analyses', {**body, 'analysis_scope': 'study_sample' if sample else 'selected_frame', 'radiologist_report': report, 'language': language})
    assert response.status_code == 202, response.text
    result = client.get('/api/v1/analyses/' + response.json()['run_id']).json()['result']
    assert result['radiologist_report'] == report  # Source prose is never translated or language-rejected.
    return result, captured


@pytest.mark.parametrize('language', ['ru', 'uz', 'en'])
@pytest.mark.parametrize('sample', [False, True])
def test_selected_language_covers_observations_frames_and_comparison(client, case, monkeypatch, language, sample):
    output = localized_output(language, sample=sample)
    result, captured = queue_mock_review(client, case, monkeypatch, [output], language=language, sample=sample)
    ...
    expected = {'ru': 'in Russian', 'uz': 'in Uzbek using the Latin alphabet', 'en': 'in English'}[language]
    assert expected in captured[0]['body']['messages'][0]['content']
    assert expected in captured[0]['body']['messages'][1]['content'][-1]['text']
    labels = [item['text'] for item in captured[0]['body']['messages'][1]['content'] if item['type'] == 'text']
...
def test_wrong_language_retries_once_with_same_images_and_validates_all_sections(client, case, monkeypatch, field):
    ...
    result, captured = queue_mock_review(client, case, monkeypatch, [wrong, correct], language='uz', sample=True)
    ...
    assert 0 < captured[1]['timeout'] <= captured[0]['timeout'] <= settings.inference_timeout_seconds


def test_repeated_language_mismatch_never_publishes_wrong_language_or_coverage(client, case, monkeypatch):
    result, captured = queue_mock_review(client, case, monkeypatch, [localized_output('en')], language='ru')
    assert len(captured) == 2
...
def test_short_english_frame_prose_is_retried_for_uzbek(client, case, monkeypatch, english):
    ...
    result, captured = queue_mock_review(client, case, monkeypatch, [mixed, correct], language='uz', sample=True)
    assert len(captured) == 2
    assert result['image_review'] == correct
...
def test_language_retry_does_not_receive_a_fresh_full_timeout(client, case, monkeypatch):
    ...
    ticks = iter([100., 100., 248.])
    monkeypatch.setattr(radiology, 'time', SimpleNamespace(monotonic=lambda: next(ticks)))
    result, captured = queue_mock_review(client, case, monkeypatch, [localized_output('en'), localized_output('ru')])
    assert [request['timeout'] for request in captured] == [150., 2.]
...
def test_correct_language_on_retry_cannot_bypass_frame_evidence_validation(client, case, monkeypatch):
    ...
    result, captured = queue_mock_review(client, case, monkeypatch, [wrong_language, invalid_evidence], sample=True)
    assert len(captured) == 2
    assert result['image_review'] is None and result['limitations'] == ['MODEL_OUTPUT_REJECTED']
    assert result['image_coverage']['reviewed_frames'] == 0


def test_single_frame_comparison_cannot_claim_support_without_an_observation(client, case, monkeypatch):
