        ...
        "frame_assessments": [
            {
                "frame_ref": f"F{i + 1}",
                "quality": "limited",
                "observations": ["Visible intensity gradient."],
                "limitations": ["Synthetic image."],
            }
...
def test_sampling_keeps_chosen_frame_and_prioritizes_distinct_series():
    ...
    body = radiology.ImageAnalysisRequest(
        expected_version=1,
        series_id="S4",
        frame_index=78,
        analysis_scope="study_sample",
        center=50,
        width=350,
    )
...
def test_single_series_sampling_spans_slices_without_duplicates():
    ...
    body = radiology.ImageAnalysisRequest(
        expected_version=1, series_id="S1", frame_index=0, analysis_scope="study_sample"
    )
    frames = radiology.review_frames(study, body)
...
def test_batch_request_sends_real_images_and_publishes_exact_coverage(client, case, monkeypatch):
    ...
    response = mutate(
        ...
        {**body, "analysis_scope": "study_sample", "language": "en"},
    )
    ...
    assert (
        "Write ALL generated explanatory prose in English" in captured[0]["messages"][0]["content"]
    )
    assert all(frame["image_quality"]["input_size"] == [16, 16] for frame in coverage["frames"])
    schema = captured[0]["response_format"]["schema"]
    assert (
        schema["properties"]["frame_assessments"]["minItems"]
        == schema["properties"]["frame_assessments"]["maxItems"]
        == 3
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "comparison_without_observation",
    ],
)
def test_batch_rejects_unsupported_frame_claims(client, case, monkeypatch, invalid):
    study, body = upload_study(client, case)
    output = sample_output()
    output["report_comparison"] = {
        "status": "supported_on_reviewed_frames",
        "explanation": "The listed image has a gradient.",
        "frame_refs": ["F1"],
        "points_to_verify": ["Review every original image."],
    }
    ...
    response = mutate(
        client,
        f"/imaging-studies/{study['id']}/analyses",
        {**body, "analysis_scope": "study_sample", "radiologist_report": "Gradient visible."},
    )
...
def test_uniform_images_never_produce_fake_normality(client, case, monkeypatch):
    response = client.post(
        ...
        headers={"Idempotency-Key": "uniform-ct"},
        data={"expected_version": case["version"], "deidentified_confirmed": "true"},
        files={"file": ("uniform.zip", dicom_zip())},
    )
...
def test_oversized_report_is_rejected_instead_of_silently_cut_off(client, case):
    ...
    response = mutate(
        client,
        f"/imaging-studies/{study['id']}/analyses",
        {**body, "analysis_scope": "study_sample", "radiologist_report": "x" * 1501},
    )
    assert (
        response.status_code == 422 and response.json()["error"]["code"] == "IMAGE_REPORT_TOO_LONG"
    )
    invalid = mutate(
        client, f"/imaging-studies/{study['id']}/analyses", {**body, "analysis_scope": "full_study"}
    )
...
def test_model_input_resize_preserves_aspect_and_original_dimensions(monkeypatch):
    ...
    png, quality = radiology.prepare_ai_frame(
        None, {"series_id": "s", "frame_index": 0, "center": 40, "width": 400}
    )
