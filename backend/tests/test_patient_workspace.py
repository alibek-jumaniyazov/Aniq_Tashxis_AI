...
def entry(client, case, category="subjective", **overrides):
    response = mutate(
        client,
        f"/cases/{case['id']}/clinical-entries",
        {
            "expected_version": case["version"],
            "category": category,
            "text": "Documented cough for three days.",
            "confirmed": True,
            **overrides,
        },
    )
...
def revise(client, case, record, **changes):
    response = mutate(
        client,
        f"/cases/{case['id']}/clinical-entries/{record['id']}",
        {"expected_version": case["version"], **changes},
        method="patch",
    )
...
def ready_case(client):
    ...
    entry(
        ...
        case,
        "doctor_conclusion",
        text="Doctor considers respiratory infection, needs examination.",
        diagnosis="Provisional respiratory infection",
    )
    return case
...
def compare(client, case, **overrides):
    response = mutate(
        client,
        f"/cases/{case['id']}/clinical-comparisons",
...
def report(version):
    return {
        ...
        "diagnosis_review": {
            ...
            "summary": "Для подтверждения заключения нужен осмотр.",
            "refs": ["E1", "E2"],
        },
        "treatment_review": {
            "status": "insufficient_data",
            "summary": "Лечение не описано.",
            "refs": ["E2"],
        },
        "supporting": [{"text": "Кашель описан пациентом.", "refs": ["E1"]}],
        "discrepancies": [],
        "questions": ["Каковы результаты осмотра?"],
        "next_steps": ["Уточнить объективные данные."],
        "five_year_outlook": {
            "status": "insufficient_data",
            "summary": "Данных для долгосрочных сценариев недостаточно.",
            "scenarios": [],
        },
...
def test_entry_versions_confirmation_and_diagnosis_preserve_history(client):
    ...
    obsolete = mutate(
        client,
        f"/cases/{case['id']}/clinical-entries/{first['id']}",
        {"expected_version": case["version"], "confirmed": True},
        method="patch",
    )
    assert obsolete.status_code == 409
    conclusion = entry(
        client, case, "doctor_conclusion", diagnosis="Provisional diagnosis", confirmed=False
    )
...
def test_entry_idempotency_stale_input_and_role_boundaries(client):
    ...
    assert (
        mutate(client, path, body, key="clinical-entry-same-key").json()["id"] == first.json()["id"]
    )
...
def test_document_category_is_atomic_draft_and_wrong_case_sources_rejected(client):
    ...
    response = client.post(
        ...
        files={
            "file": (
                "conclusion.txt",
                b"Provisional diagnosis: respiratory infection.",
                "text/plain",
            )
        },
    )
    ...
    wrong = mutate(
        client,
        f"/cases/{other['id']}/clinical-entries",
        {
            "expected_version": 1,
    ...
    assert (
        ...
        and wrong.json()["error"]["code"] == "CLINICAL_ENTRY_SOURCE_MISMATCH"
    )
    fake = mutate(
        client,
        f"/cases/{case['id']}/clinical-entries",
...
def test_comparison_requires_adult_and_confirmed_patient_data_and_conclusion(client):
    ...
    assert (
        ...
        and missing.json()["error"]["code"] == "CLINICAL_COMPARISON_EVIDENCE_REQUIRED"
    )
    add(client, case, [fact("symptom.complaint", "Cough")])
    assert (
        client.get(f"/api/v1/cases/{case['id']}/clinical-entries").json()["readiness"][
            "comparison_ready"
        ]
        is True
    )
    patch = mutate(
        client,
        f"/cases/{case['id']}",
        {"expected_version": case["version"], "age": 17},
        method="patch",
    )
...
def test_snapshot_uses_latest_confirmed_conclusion_old_facts_and_no_identity(client, monkeypatch):
    ...
    latest = entry(
        ...
        case,
        "doctor_conclusion",
        text="Latest working conclusion.",
    ...
    add(
        client,
        case,
        [fact("vital.pulse", "80", unit="/min"), fact("vital.spo2", "98", confirmed=False)],
    )
    ...
    with SessionLocal() as db:
        ...
        assert (
            not {"full_name", "patient_phone", "alias", "owner_id", "author_name"} & snapshot.keys()
        )
...
def test_model_unavailable_is_failure_never_fake_success_and_retry_is_versioned(client):
    ...
    assert (
        mutate(
            client, f"/analyses/{job['id']}/retry", {"expected_version": case["version"]}
        ).status_code
        == 409
    )
...
def test_comparison_cancellation_survives_inflight_completion(client, monkeypatch):
    ...
    def cancel(snapshot, language):
        with SessionLocal() as db:
            db.execute(
                update(Job)
                .where(Job.kind == "clinical_comparison")
...
def test_model_schema_validation_language_and_references(client, monkeypatch):
    ...
    def completion(messages, schema, **kwargs):
        ...
        return json.dumps(
            {key: value for key, value in localized.items() if key in schema["properties"]},
            ensure_ascii=False,
        )
    ...
    assert captured["schema"]["$defs"]["ReviewSection"]["properties"]["refs"]["items"]["enum"] == [
        "E1",
        "E2",
    ]
    assert (
        "full_name" not in captured["messages"][1]["content"]
        and "patient_phone" not in captured["messages"][1]["content"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "probability",
        "number",
        "extra_probability_field",
        "target_range",
        "unhelpful_questions",
    ],
)
def test_model_rejects_fabricated_refs_numbers_and_unvalidated_probability(
    client, monkeypatch, problem
):
    case = ready_case(client)
    monkeypatch.setattr(settings, "ai_backend", "llama_cpp")
    monkeypatch.setattr(ai, "model_status", lambda: {"ready": True})
    ...
    monkeypatch.setattr(
        llama_adapter,
        "complete",
        lambda messages, schema, **kwargs: json.dumps(
            {key: value for key, value in invalid.items() if key in schema["properties"]}
        ),
    )
...
def test_dmed_demo_import_has_five_unconfirmed_categories_and_is_idempotent(client):
    ...
    assert len(rows) == 5 and all(
        not r["confirmed"] and r["source_mode"] == "dmed_demo" for r in rows
    )
        ...
        assert (
            len(
                list(
                    db.scalars(
                        select(Record).where(
                            Record.kind == "clinical_entry", Record.case_id == case["id"]
                        )
                    )
                )
            )
...
def test_radiologist_uploads_instrumental_drafts_but_not_doctor_conclusions(client, case):
    ...
    assert (
        client.post(
            path,
            headers={"Idempotency-Key": "radiologist-restricted"},
            data=form,
    ...
    upload = client.post(
        path,
        headers={"Idempotency-Key": "radiologist-instrumental"},
        data=form,
        files={"file": ("report.txt", b"Instrumental report.", "text/plain")},
    )
...
def test_bounded_model_correction_keeps_schema_and_does_not_publish_bad_attempt(
    client, monkeypatch
):
    case = ready_case(client)
    monkeypatch.setattr(settings, "ai_backend", "llama_cpp")
    ...
    def completion(messages, schema, **kwargs):
        ...
        return json.dumps(
            {key: value for key, value in output.items() if key in schema["properties"]}
        )
...
def test_qualitative_outlook_requires_observations_and_accepts_probability_abstention():
    ...
    evidence = [
        {"ref": "E1", "category": "objective", "text": "Confirmed longitudinal observations."},
        {"ref": "E2", "category": "doctor_conclusion", "text": "Confirmed doctor conclusion."},
    ]
    output = report(1)
    output["five_year_outlook"] = {
        "status": "qualitative_only",
        "summary": "Процентный риск не рассчитан; это условные сценарии наблюдения.",
        "scenarios": [
            {
                "scenario": "Продолжение наблюдения при стабильном состоянии.",
                "conditions": "Если наблюдаемое состояние сохраняется.",
                "monitoring": "Обсудить динамику зарегистрированных наблюдений с врачом.",
                "refs": ["E1"],
            }
        ],
    }
...
def test_comparison_rechecks_version_after_concurrent_clinical_update(client, monkeypatch):
    ...
    def access_then_another_doctor_updates(db, user, case_id):
        ...
        if not changed:
            ...
            with SessionLocal() as other:
                other.execute(
                    update(Case).where(Case.id == case_id).values(version=loaded.version + 1)
                )
                other.add(
                    Record(
                        ...
                        actor_id=user.id,
                        case_id=case_id,
                        case_version=loaded.version + 1,
                        kind="clinical_entry",
                        data={
                            "category": "objective",
                            "text": "New confirmed observation.",
                            "confirmed": True,
                            "diagnosis": "",
                            "treatment": "",
                            "source_id": None,
    ...
    monkeypatch.setattr(jobs, "dispatch", lambda _: None)
    response = mutate(
        client, f"/cases/{case['id']}/clinical-comparisons", {"expected_version": case["version"]}
    )
...
def test_legacy_dmed_import_backfills_drafts_once_without_fabricating_or_duplicating_data(client):
    ...
    with SessionLocal() as db:
        ...
        source = Record(
            ...
            actor_id=patient.owner_id,
            case_id=patient.id,
            case_version=patient.version,
            kind="source",
            data={
                "type": "dmed_demo",
                "name": "Legacy DMED fixture",
                "text": original_text,
                "external_version": 1,
                "pages": [],
        ...
        db.add(
            Record(
                ...
                data={
                    ...
                    "label": "Жалобы",
                    "value": "контрольное обследование",
                    "source_id": source.id,
                    "confirmed": False,
                },
            )
    ...
    assert (
        next(row["text"] for row in result["items"] if row["category"] == "subjective")
        == original_text
    )
    ...
    rejected = mutate(
        client,
        f"/cases/{case['id']}/clinical-entries/{blank['id']}",
        {"expected_version": result["case_version"], "confirmed": True},
        method="patch",
    )
    assert (
        rejected.status_code == 422
        and rejected.json()["error"]["code"] == "CLINICAL_ENTRY_TEXT_REQUIRED"
    )
    ...
    updated = mutate(
        client, path, {**body, "expected_version": current["version"], "scenario": "updated"}
    )
