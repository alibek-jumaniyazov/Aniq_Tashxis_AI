...
def test_patient_migration_preserves_legacy_rows_and_does_not_stamp_history():
    path = (
        Path(__file__).parents[1] / "migrations" / "versions" / "20260918_patient_registration.py"
    )
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                "CREATE TABLE cases (id TEXT PRIMARY KEY, alias TEXT NOT NULL, age INTEGER, summary TEXT NOT NULL, diagnosis TEXT NOT NULL)"
            )
        )
        connection.execute(
            text(
                "INSERT INTO cases VALUES ('legacy-id', 'CUSTOM-LEGACY', NULL, 'Existing comment', 'Existing conclusion')"
            )
        )
        ...
        connection.execute(text("INSERT INTO patient_code_allocations DEFAULT VALUES"))
        assert list(
            connection.execute(text("SELECT id FROM patient_code_allocations")).scalars()
        ) == [1, 2]
    engine.dispose()
