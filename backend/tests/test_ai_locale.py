...
PROSE = {
    "ru": [
        "Зафиксирован пульс 72 /min.",
        "Для оценки лечения данных недостаточно.",
        "Требуется проверка врачом; клиническая валидация не выполнена.",
    ],
    "uz": [
        "Bemorning pulsi 72 /min. deb qayd etilgan.",
        "Davolashni baholash uchun ma'lumotlar yetarli emas.",
        "Shifokor tekshiruvi kerak; model klinik tekshiruvdan o'tmagan.",
    ],
    "en": [
        "The documented pulse is 72 /min.",
        "The treatment data are insufficient for assessment.",
...
@pytest.mark.parametrize(
    "requested,actual",
    [("ru", "en"), ("ru", "uz"), ("uz", "ru"), ("uz", "en"), ("en", "ru"), ("en", "uz")],
)
...
@pytest.mark.parametrize(
    "text", ["Synthetic image.", "Visible intensity gradient.", "The image is readable."]
)
...
def test_quoted_source_expression_does_not_force_its_language_on_the_report():
    validate_prose_language(
        ["В исходном документе записано «No acute abnormality». Требуется проверка врача."], "ru"
    )
    with pytest.raises(OutputLanguageMismatch):
        validate_prose_language(
            ['"The documented findings require further clinical review."'], "ru"
        )
...
def snapshot(language):
    return {
        ...
        "entries": [
            {"id": "exam", "category": "objective", "text": "Pulse 72 /min."},
            {
                "id": "conclusion",
                "category": "doctor_conclusion",
                "text": "Clinical follow-up.",
                "diagnosis": "",
                "treatment": "",
            },
        ],
...
QUESTIONS = {
    "ru": "Есть ли повышение температуры?",
    "uz": "Bemorning harorati ko‘tarilganmi?",
...
def comparison(language):
    summaries = {
        "ru": (
            "Для оценки диагноза требуется документированное заключение.",
            "Уточнить документированный диагноз.",
            "Данных для долгосрочного сценария недостаточно.",
        ),
        "uz": (
            "Tashxisni baholash uchun hujjatdagi xulosa kerak.",
            "Hujjatdagi tashxisni aniqlashtirish kerak.",
            "Uzoq muddatli holatni baholash uchun ma'lumotlar yetarli emas.",
        ),
        "en": (
            "The diagnosis cannot be assessed without a documented conclusion.",
            "Clarify the documented diagnosis.",
    ...
    return {
        ...
        "treatment_review": {
            "status": "insufficient_data",
            "summary": PROSE[language][1],
            "refs": ["E2"],
        },
        "supporting": [{"text": PROSE[language][0], "refs": ["E1"]}],
        "discrepancies": [],
        "questions": [QUESTIONS[language]],
        "next_steps": [summaries[1]],
        "five_year_outlook": {
            "status": "insufficient_data",
            "summary": summaries[2],
            "scenarios": [],
...
@pytest.mark.parametrize("language", ["ru", "uz", "en"])
def test_comparison_respects_snapshot_language_with_all_sections_and_citations(
...
def test_comparison_retries_wrong_language_once_without_translation_or_fallback(
    monkeypatch, language
):
...
def test_general_summary_checks_selected_language_and_preserves_missing_keys(monkeypatch, language):
    ...
    def generate(snapshot, coverage, mode, cutoff, system_prompt):
        ...
        return json.dumps(
            {
                "case_version": 4,
                "summary": PROSE[actual][0],
                "concerns": [],
                "limitations": [PROSE[actual][2]],
                "missing_fields": ["sex"],
            },
            ensure_ascii=False,
        )
...
def test_general_summary_language_failure_is_bounded_and_releases_model_lock(monkeypatch):
    ...
    def generate(*args):
        calls.append(True)
        return json.dumps(
            {"case_version": 4, "summary": PROSE["ru"][0], "limitations": [PROSE["ru"][2]]}
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Bemorning yoshi qancha?",
        "Bemorning pulsi qancha?",
    ],
)
...
@pytest.mark.parametrize(
    "question",
    [
        "Изменился ли пульс после нагрузки?",
...
@pytest.mark.parametrize(
    "text",
    [
        "Amlodipine 5 mg daily, but the dose is not documented.",
...
@pytest.mark.parametrize(
    "text",
    [
        "Amlodipine 5 mg daily, but the dose of lisinopril is unknown.",
...
def test_transformers_final_user_instruction_keeps_language_on_initial_and_repair(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        "transformers",
        SimpleNamespace(
            AutoModelForImageTextToText=None, AutoProcessor=None, BitsAndBytesConfig=None
        ),
    )
    monkeypatch.setattr(
        ai, "_model", SimpleNamespace(device="cpu", generate=lambda **kwargs: [[0, 1]])
    )
    ...
    assert all(
        messages[-1]["content"][-1]["text"].endswith(language_instruction("ru"))
...
def test_uzbek_allows_standard_spelling_quoted_terms_and_named_diacritics():
    validate_prose_language(
        [
            "Bemorning sil yoki tuberkulyoz haqidagi xulosasi tekshirilishi kerak.",
            'Hujjatda "tüberkülyoz" yozilgan, bu atamani shifokor aniqlashtirishi kerak.',
            "Müller usuli haqida hujjatda ma’lumot qayd etilgan.",
        ],
        "uz",
    )
