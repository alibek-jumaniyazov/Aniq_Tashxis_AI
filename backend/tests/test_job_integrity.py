...
def test_retry_obeys_same_queue_budget_as_new_analysis(client, monkeypatch):
    ...
    for _ in range(10):
        ...
        assert (
            mutate(
                client,
                f"/cases/{queued_case['id']}/analyses",
                {"expected_version": queued_case["version"]},
            ).status_code
            == 202
        )
    assert (
...
def test_worker_failure_cannot_overwrite_a_concurrent_cancel(client, monkeypatch):
    ...
    def cancel_before_failure_write(
...
def test_interrupted_worker_recovery_resets_stage_and_allows_retry(client, monkeypatch):
    ...
    with SessionLocal() as db:
        db.execute(
            update(Job).where(Job.id == queued["run_id"]).values(status="running", stage="medgemma")
        )
        db.commit()
    ...
    assert (
        mutate(
            client, "/analyses/" + queued["run_id"] + "/retry", {"expected_version": c["version"]}
        ).status_code
        == 202
    )
