...
def upgrade():
    op.add_column(
        "cases", sa.Column("full_name", sa.String(200), nullable=False, server_default="")
    )
    op.add_column(
        "cases", sa.Column("patient_phone", sa.String(50), nullable=False, server_default="")
    )
    op.create_table(
        "patient_code_allocations",
