...
report = {'python': platform.python_version(), 'platform': platform.platform(), 'model': model_status()}
if '--check-download' in sys.argv:
    try:
        request = urllib.request.Request('https://huggingface.co/google/medgemma-1.5-4b-it/resolve/main/config.json', method='HEAD')
