...
try:
    ...
    sys.exit(f'Model download incomplete ({type(exc).__name__}). Accept the model access terms and authenticate locally. No token was printed.')
...
lines = [line for line in lines if not line.startswith(('MODEL_PATH=', 'MODEL_REVISION=', 'AI_BACKEND=', 'MODEL_QUANTIZATION='))]
lines += ['MODEL_PATH=' + destination.as_posix(), 'MODEL_REVISION=' + revision, 'AI_BACKEND=transformers', 'MODEL_QUANTIZATION=4bit']
env.write_text('\n'.join(lines) + '\n', encoding='utf-8')
print('Model files verified. Restart the backend and run a local inference smoke test. Clinical validation is still required.')
