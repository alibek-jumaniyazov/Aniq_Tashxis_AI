...
BUILD_CODE = ('from app.db import Base,engine; import app.billing_models; '
...
def preserve_access_and_configuration(previous, staged):
    ...
    with closing(sqlite3.connect(f'{previous.as_uri()}?mode=ro', uri=True)) as source, closing(sqlite3.connect(staged)) as target, target:
        ...
        def rows(table):
            return [dict(row) for row in source.execute('SELECT * FROM ' + table)] if table in tables else []
        def upsert(table, values, key='id'):
            ...
            names = list(values)
            target.execute('INSERT INTO ' + table + ' (' + ','.join(names) + ') VALUES (' + ','.join('?' for _ in names) + ') ON CONFLICT(' + key + ') DO UPDATE SET ' + ','.join(name + '=excluded.' + name for name in names if name != key), list(values.values()))
        ...
        for method in methods:
            ...
            match = target.execute('SELECT id FROM billing_payment_methods WHERE name=? AND card_number=? AND recipient=?', (method['name'], method['card_number'], method['recipient'])).fetchone()
            if match:
                method['id'] = match['id']
            upsert('billing_payment_methods', method)
        user_map, matched, retained = {}, 0, 0
        for user in rows('users'):
            match = target.execute('SELECT id FROM users WHERE lower(email)=lower(?)', (user['email'],)).fetchone()
            if match:
                target.execute('UPDATE users SET password_hash=?, active=? WHERE id=?', (user['password_hash'], user['active'], match['id']))
        ...
        for clinic in rows('billing_clinics'):
            if target.execute('SELECT 1 FROM billing_clinics WHERE id=?', (clinic['id'],)).fetchone():
                continue
            developer_owner = target.execute("SELECT id FROM users WHERE id=? AND role='developer'", (user_map.get(clinic['owner_id']),)).fetchone()
        ...
        if target.execute('PRAGMA foreign_key_check').fetchone():
            raise RuntimeError('Preserved account/configuration references failed validation; current data was not changed.')
        return {'matched_accounts': matched, 'retained_accounts': retained, 'retained_clinics': retained_clinics, 'payment_methods': len(methods)}
...
def main():
    ...
    parser.add_argument('--build-only', action='store_true', help='Build and validate staged seed without replacing current data. Does not require stopping the app.')
    ...
    build = subprocess.run([sys.executable, '-c', BUILD_CODE], cwd=ROOT, env=environment, capture_output=True, text=True, encoding='utf-8', timeout=120, check=False)
    if build.returncode:
    ...
    check = subprocess.run([sys.executable, '-c', 'from app.demo_seed import verify_seed; import json; print(json.dumps(verify_seed()))'], cwd=ROOT, env=environment, capture_output=True, text=True, encoding='utf-8', timeout=120, check=False)
    if check.returncode:
        raise RuntimeError('Preserved staged seed failed; current data was not changed.\n' + check.stderr)
    counts = json.loads(check.stdout.strip())
    if args.build_only:
        print(json.dumps({'staging': str(staging), 'activated': False, 'counts': counts, 'preserved': preservation}, indent=2))
        return
    ...
    try:
        ...
        for original, target in reversed(moved):
            target.rename(original)
        raise
    manifest = {'backup': str(backup), 'activated_at': stamp, 'counts': counts, 'preserved': preservation, 'restoration': 'Stop API and workers; move current aniq.db and files aside, then restore both aniq.db and files from this backup together.'}
