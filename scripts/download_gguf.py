...
def download(url, target, expected, workers=24):
    ...
    if target.exists() and digest(target) == expected:
        ...
        return
    part, state = target.with_suffix(target.suffix + '.part'), target.with_suffix(target.suffix + '.progress.json')
    ...
    def transfer(index):
        ...
        for attempt in range(5):
            try:
                ...
                with thread_state.client.get(endpoint, headers={'Range': f'bytes={first}-{last}'}, stream=True, timeout=(20, 60)) as result:
                    if result.status_code != 206 or result.headers.get('Content-Range') != f'bytes {first}-{last}/{total}':
                ...
                if attempt == 4:
                    raise RuntimeError(f'Download interrupted at chunk {index}; run again to resume') from None
                time.sleep(min(2**attempt, 8))

    pool = ThreadPoolExecutor(max_workers=workers)
    try:
        for future in as_completed([pool.submit(transfer, i) for i in range(count) if i not in done]):
            done.add(future.result())
            temporary = state.with_suffix('.tmp')
            temporary.write_text(json.dumps({'sha256': expected, 'total': total, 'completed': sorted(done)}))
            temporary.replace(state)
            current = time.monotonic()
            if current - last_output > 10 or len(done) == count:
                rate = (len(done) - initial) * CHUNK / max(current - started, 1) / 1024**2
                print(f'{target.name}: {100 * len(done) / count:.1f}% ({rate:.2f} MiB/s)', flush=True)
                last_output = current
    finally:
...
def main():
    ...
    download(f'https://github.com/ggml-org/llama.cpp/releases/download/{RUNTIME_VERSION}/llama-{RUNTIME_VERSION}-bin-win-vulkan-x64.zip', archive, RUNTIME_SHA, workers=8)
    ...
    download(f'https://huggingface.co/unsloth/medgemma-1.5-4b-it-GGUF/resolve/{MODEL_REVISION}/{MODEL_FILE}', model, MODEL_SHA, workers=min(48, max(1, int(os.environ.get('DOWNLOAD_WORKERS', '24')))))
    manifest = {'base_model': 'google/medgemma-1.5-4b-it', 'quantized_by': 'unsloth', 'repository': 'unsloth/medgemma-1.5-4b-it-GGUF', 'revision': MODEL_REVISION, 'quantization': 'Q4_K_M', 'sha256': MODEL_SHA, 'runtime': RUNTIME_VERSION, 'runtime_sha256': RUNTIME_SHA}
    ...
    keys = ('AI_BACKEND=', 'MODEL_PATH=', 'MODEL_REVISION=', 'MODEL_QUANTIZATION=', 'MAX_INPUT_TOKENS=', 'MAX_NEW_TOKENS=')
    lines = [line for line in lines if not line.startswith(keys)]
    lines += ['AI_BACKEND=llama_cpp', 'MODEL_PATH=' + model.as_posix(), 'MODEL_REVISION=' + MODEL_REVISION, 'MODEL_QUANTIZATION=Q4_K_M', 'MAX_INPUT_TOKENS=3072', 'MAX_NEW_TOKENS=768']
    env.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('Local GGUF model ready. Start scripts/start-model.ps1, then run scripts/check_model.py.', flush=True)
