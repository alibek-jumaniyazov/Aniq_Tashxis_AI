...
parser.add_argument('--extract', action='store_true', help='Also test exact-quote document extraction with the configured provider.')
parser.add_argument('--language', choices=['ru', 'uz', 'en'], default='uz', help='Output language for the synthetic smoke test (default: uz).')
parser.add_argument('--status-only', action='store_true', help='Check provider availability without inference; cloud status may contact the provider.')
...
status = model_status()
report = {'model': status, 'synthetic_only': True, 'language': options.language, 'clinical_validation': 'not_performed'}
...
try:
    ...
    result = review({'version': 1, 'demo': True, 'language': options.language, 'age': 40, 'sex': 'unknown', 'summary': 'Synthetic engineering test.', 'facts': [{'id': 'smoke-fact', 'key': 'vital.pulse', 'label': 'Pulse', 'value': '70', 'unit': '/min', 'source_id': 'smoke-source', 'confirmed': True}]}, {'completed': [], 'not_evaluable': []}, 'current', None)
...
print(f'REAL INFERENCE PASSED. Provider: {status.get("provider", status.get("backend", "configured"))}; model: {status.get("model_id", "configured")}; language: {options.language}. Clinical validation has not been performed.')
