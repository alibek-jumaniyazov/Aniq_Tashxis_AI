...
if __name__ == '__main__':
    ...
    download(f'https://huggingface.co/unsloth/medgemma-1.5-4b-it-GGUF/resolve/{MODEL_REVISION}/{NAME}', target, SHA256, workers=8)
    target.with_suffix('.manifest.json').write_text(json.dumps({'repo': 'unsloth/medgemma-1.5-4b-it-GGUF', 'revision': MODEL_REVISION, 'file': NAME, 'sha256': SHA256, 'size': target.stat().st_size}, indent=2), encoding='utf-8')
