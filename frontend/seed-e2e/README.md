...
These checks target an activated `realistic-workspace-v2` seed. Start them only after the root task has completed activation and confirmed that no concurrent run will replace the saved comparison fixtures.
...
The suite respects the real login rate limit: a 429 response waits for `Retry-After` and retries once, without changing application limits. A full run can therefore take an extra minute.
