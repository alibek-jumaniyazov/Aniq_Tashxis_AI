...
export async function readOnlySession(page: Page) {
  ...
  await page.route('**/api/v1/**', async route => {
    ...
    if (!['GET', 'HEAD', 'OPTIONS'].includes(request.method()) && !(request.method() === 'POST' && path === '/api/v1/auth/login')) {
...
export async function switchLanguage(page: Page, language: Language) {
  ...
  await page.locator('.ant-select-item-option-content').getByText({ ru: 'Русский', uz: 'O‘zbekcha', en: 'English' }[language], { exact: true }).click()
