...
const copy = {
  ru: { comparison: 'Сравнение с AI', outlook: 'Прогноз на 5 лет', synthetic: 'Учебный образец анализа', language: 'Язык заключения', mismatch: 'Заключение сохранено на другом языке' },
  uz: { comparison: 'AI bilan solishtirish', outlook: '5 yillik prognoz', synthetic: 'Tahlilning o‘quv namunasi', language: 'Xulosa tili', mismatch: 'Xulosa boshqa tilda saqlangan' },
  en: { comparison: 'Compare with AI', outlook: '5-year outlook', synthetic: 'Illustrative clinical review', language: 'Report language', mismatch: 'This report was saved in another language' },
}

interface SavedComparison {
  id: string; language: Language; case_version: number; status: string; is_stale: boolean
  result: { language: Language; model_id: string; provenance: string; demo_only: boolean; comparison: {
    summary: string; treatment_review: { status: string; summary: string }; diagnosis_review: { status: string }
    discrepancies: { text: string; refs: string[] }[]
    evidence: { ref: string; category: string; source_id: string; entry_id?: string; text: string }[]
...
test('same-version seeded review and outlook follow RU/UZ/EN and preserve source quotations', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: patient.full_name, exact: true })).toBeVisible()
  for (const language of ['uz', 'ru', 'en'] as Language[]) {
    await switchLanguage(page, language)
    const run = runs.find(item => item.language === language)!
    expect(run).toMatchObject({ case_version: patient.version, is_stale: false, status: 'succeeded' })
    expect(run.result).toMatchObject({ language, provenance: 'synthetic_seed', demo_only: true, model_id: 'synthetic-demo' })
    ...
    await expect(panel.locator('.analysis-provenance .ant-tag').first()).toHaveText(`${copy[language].language}: ${language.toUpperCase()}`)
    await expect(panel.locator('.analysis-provenance .ant-tag').last()).toHaveText(copy[language].synthetic)
    ...
    await expect(panel.locator('.pw-review-card').nth(1)).toContainText(run.result.comparison.treatment_review.summary)
    ...
    for (const other of runs.filter(item => item.language !== language)) {
      await expect(panel.locator('.pw-ai-summary')).not.toContainText(other.result.comparison.summary)
    }
    ...
    await expect(forecast.locator('.pw-ai-summary')).toContainText(run.result.comparison.five_year_outlook.summary)
    await expect(forecast.locator('.pw-scenarios')).toContainText(run.result.comparison.five_year_outlook.scenarios[0].scenario)
  }
  ...
  const original = runs.find(run => run.language === 'en')!.result.comparison.evidence.find(item => item.category === 'subjective')!
  await result.locator('button.pw-reference').filter({ hasText: original.ref }).first().click()
  await expect(page.locator('.source-paper pre')).toContainText('Артериальная гипертензия известна три года')
  ...
  await page.screenshot({ path: 'seed-test-results/seed-english-original-source.png', fullPage: true })
  verifyReadOnly()
})

test('five patient sections are populated, with honest pending-intake and DMED states', async ({ page }) => {
  const verifyReadOnly = await readOnlySession(page)
  await login(page)
  await expect(page).toHaveURL(/\/cases$/)
  ...
  for (const [category, label] of [['subjective', 'Subyektiv ma’lumotlar'], ['objective', 'Obyektiv ma’lumotlar'], ['laboratory', 'Laboratoriya ma’lumotlari'], ['instrumental', 'Instrumental ma’lumotlar']]) {
    await page.locator('.patient-data-categories').getByRole('button', { name: new RegExp(label) }).click()
  ...
  await expect(page.locator('.patient-data-section--doctor_conclusion')).toContainText('метформин обычного высвобождения 500 мг')
  ...
  for (const suffix of ['-004', '-016', '-017']) {
    ...
    if (suffix === '-017') expect(entries.items.every((item: { source_mode: string }) => item.source_mode === 'dmed_demo')).toBe(true)
  }
  ...
  await expect(page.locator('.pw-comparison-launch').getByRole('button', { name: 'AI bilan solishtirish', exact: true })).toBeDisabled()
  verifyReadOnly()
})

test('paired TB examples show different grounded conclusions without inventing image findings', async ({ page }) => {
  const verifyReadOnly = await readOnlySession(page)
  await login(page)
  await expect(page).toHaveURL(/\/cases$/)
  const list = await patients(page)
  for (const [suffix, expected] of [['-009', 'consistent_with_data'], ['-015', 'needs_review']]) {
    ...
    await expect(panel.locator('.pw-review-card').first()).toHaveClass(new RegExp(`pw-review-card--${expected}`))
    if (suffix === '-015') await expect(panel.locator('.pw-findings--discrepancies')).toContainText(report.discrepancies[0].text)
