...
const sourceText =
  'Исходная учебная запись врача: синтетический пациент, медицинских назначений нет.'
...
async function switchLanguage(page: Page, language: Language) {
  ...
  await page
    ...
    .getByText(labels[language].option, { exact: true })
    .click()
...
async function fixture(page: Page, imaging = false) {
  ...
  const created = await page.request.post('/api/v1/cases', {
    headers: headers(),
    data: {
      full_name: `Synthetic Language QA ${crypto.randomUUID().slice(0, 8)}`,
      age: 49,
      summary: sourceText,
    },
  })
  ...
  for (const category of ['subjective', 'doctor_conclusion']) {
    const entry = await page.request.post(`/api/v1/cases/${initial.id}/clinical-entries`, {
      headers: headers(),
      data: {
        ...
        text: sourceText,
        confirmed: true,
        ...(category === 'doctor_conclusion'
          ? {
              diagnosis: 'Учебная рабочая гипотеза.',
              treatment: 'Учебный план проверки исходных данных.',
            }
          : {}),
      },
    })
  ...
  if (imaging) {
    const upload = await page.request.post(`/api/v1/cases/${initial.id}/imaging-studies`, {
      headers: headers(),
      multipart: {
        expected_version: String(version),
        deidentified_confirmed: 'true',
        file: {
          name: 'synthetic-phantom.zip',
          mimeType: 'application/zip',
          buffer: await readFile('../demo/synthetic-phantom.zip'),
        },
      },
    })
  ...
  const base = (focus: Focus, language: Language) => {
    ...
    return {
      ...
      is_stale: false,
      mode: 'current',
      created_at: new Date(Date.UTC(2026, 0, 2, 10, 0, sequence++)).toISOString(),
      error_code: null,
    }
  }
  const provenance = (language: Language) => ({
    language,
      ...
      const run: ComparisonRun = {
        ...
        result: {
          ...
          comparison: {
            ...
            questions: [],
            next_steps: [],
            limitations: [summaries[language]],
            evidence: [
              { ref: 'E1', category: 'subjective', text: sourceText, source_id: sourceId },
            ],
            five_year_outlook: {
              status: 'insufficient_data',
              summary: summaries[language],
        ...
        ai:
          focus === 'radiology'
            ...
            : {
                ...
                missing_fields: [],
                evidence: [],
                assessment: { status: 'insufficient_data', differential: [], questions: [] },
              },
        coverage: { completed: [], not_evaluable: [] },
      },
    }
    if (focus === 'radiology') {
      ...
      Object.assign(run.result, {
        ...
        width: 400,
        image_review: {
          observations: [imageProse[language]],
          limitations: [summaries[language]],
          frame_assessments: [
            {
              frame_ref: 'F1',
  ...
  async function queue(route: Route, focus: Focus, retry = false) {
    ...
    await route.fulfill({
      status: 202,
      json: { run_id: run.id, status: 'queued', case_version: patient.version },
    })
  }
  await page.route(`**/api/v1/cases/${patient.id}`, (route) =>
    route.request().method() === 'GET' ? route.fulfill({ json: patient }) : route.continue(),
  )
  ...
  await page.route(`**/api/v1/cases/${patient.id}/readiness`, (route) =>
    route.fulfill({
      json: {
        clinical_review_ready: true,
        clinical_population_eligible: true,
        confirmed_facts: 1,
        total_facts: 1,
        eligible_facts: 1,
        unconfirmed_facts: 0,
  ...
  await page.route(`**/api/v1/cases/${patient.id}/clinical-comparisons`, (route) =>
    route.request().method() === 'GET'
      ? route.fulfill({ json: { items: comparisons } })
      : queue(route, 'clinical_comparison'),
  )
  await page.route(`**/api/v1/cases/${patient.id}/analyses`, (route) =>
    queue(
      route,
      route.request().postDataJSON().review_focus === 'clinical_assessment'
        ? 'clinical_assessment'
        : 'documentation',
    ),
  )
  ...
  if (imaging)
    await page.route(`**/api/v1/imaging-studies/${patient.studies[0].id}/analyses`, (route) =>
      queue(route, 'radiology'),
    )
  await page.route(`**/api/v1/sources/${sourceId}`, (route) =>
    route.fulfill({
      json: {
        id: sourceId,
        name: 'Synthetic original source',
        type: 'manual',
        text: sourceText,
        pages: [],
        limitations: [],
  ...
  return {
    ...
    requests,
    addRun,
    removeComparisonLanguage: (language: Language) => {
      comparisons = comparisons.filter((run) => run.language !== language)
    },
    removeImageLanguage: (language: Language) => {
      patient.studies[0].analyses = patient.studies[0].analyses?.filter(
...
async function expectSynthetic(result: Locator, language: Language) {
  ...
  await expect(result.locator('.analysis-provenance .ant-tag').last()).toHaveText(
    labels[language].synthetic,
  )
  await expect(result.locator('.pw-ai-eyebrow, .ai-analysis-eyebrow')).toHaveText(
    labels[language].synthetic,
  )
}
...
async function expectHistoryLanguages(page: Page, control: Locator, languages: Language[]) {
  ...
  for (const text of texts)
    expect(languages.map((language) => language.toUpperCase())).toContain(text.split(' · ')[0])
  await control.press('Escape')
  await expect(control).toHaveAttribute('aria-expanded', 'false')
}

test('header comparison opens the result view directly and selects the newly returned run in the current language', async ({
  page,
}) => {
  const state = await fixture(page)
  state.addRun('clinical_comparison', 'ru')
  state.addRun('clinical_comparison', 'uz')
  await page.goto(`/cases/${state.patient.id}`)
  await switchLanguage(page, 'uz')
  const response = page.waitForResponse(
    (value) =>
      value.request().method() === 'POST' &&
      value.url().endsWith(`/cases/${state.patient.id}/clinical-comparisons`),
  )
  await page.getByTestId('quick-compare').click()
  const launched = await (await response).json()
  await expect(page.getByRole('tab', { name: labels.uz.comparison, exact: true })).toHaveAttribute(
    'aria-selected',
    'true',
  )
  await expect(
    page.getByTestId('clinical-comparison-result').locator('.pw-ai-summary'),
  ).toContainText(summaries.uz)
  expect(state.requests).toHaveLength(1)
  expect(state.requests[0]).toMatchObject({
    focus: 'clinical_comparison',
    retry: false,
    body: { expected_version: state.patient.version, language: 'uz' },
  })
  ...
  expect(state.requests[1]).toMatchObject({
    path: `/api/v1/analyses/${launched.run_id}/retry`,
    retry: true,
    body: { expected_version: state.patient.version, language: 'uz' },
  })
})

test('comparison switches same-version saved languages, preserves sources and sends current locale for launch/retry/regenerate', async ({
  page,
}) => {
  const state = await fixture(page)
  for (const language of ['en', 'ru', 'uz'] as Language[])
    state.addRun('clinical_comparison', language)
  await page.goto(`/cases/${state.patient.id}`)
  ...
  for (const language of locales) {
    ...
    await expect(
      page.locator('.pw-comparison-launch').getByTestId('ai-processing-notice'),
    ).toContainText('OpenAI API')
    ...
    for (const other of locales.filter((value) => value !== language))
      await expect(result).not.toContainText(summaries[other])
    await expectSynthetic(result, language)
    await expectHistoryLanguages(page, page.locator('.pw-run-selector').getByRole('combobox'), [
      language,
    ])
    const count = state.requests.length
    await page
      .locator('.pw-comparison-launch')
      .getByRole('button', { name: labels[language].launch, exact: true })
      .click()
...
test('diagnostic and documentation tools select localized prose and send current language on every launch and retry', async ({
  page,
}) => {
  const state = await fixture(page)
  for (const focus of ['clinical_assessment', 'documentation'] as const)
    for (const language of locales) state.addRun(focus, language)
  await page.goto(`/cases/${state.patient.id}`)
  for (const language of locales) {
    await switchLanguage(page, language)
    ...
    for (const focus of ['clinical_assessment', 'documentation'] as const) {
      await drawer
        .getByRole('tab', {
          name:
            focus === 'clinical_assessment' ? labels[language].clinical : labels[language].decision,
          exact: true,
        })
        .click()
      const panel = drawer.locator('.ant-tabs-tabpane-active').getByTestId('ai-analysis-panel')
      await expect(
        drawer.locator('.ant-tabs-tabpane-active').getByTestId('ai-processing-notice'),
      ).toContainText('OpenAI API')
      await expect(panel.locator('.ai-result-summary')).toContainText(summaries[language])
      await expectSynthetic(panel, language)
      await expectHistoryLanguages(
        page,
        drawer.locator('.ant-tabs-tabpane-active').getByRole('combobox').last(),
        [language],
      )
      const count = state.requests.length
      if (focus === 'clinical_assessment')
        await drawer
          .getByRole('button', { name: labels[language].clinicalLaunch, exact: true })
          .click()
...
test('DICOM viewer launches/retries in all languages, selects localized frame prose, and hides mismatched frame text', async ({
  page,
}) => {
  ...
  for (const language of locales) {
    ...
    await expectHistoryLanguages(
      page,
      page.locator('.radiology-main > .margin-top > .ant-select').getByRole('combobox'),
      [language],
    )
    const count = state.requests.length
    ...
    expect(state.requests.at(-1)).toMatchObject({
      focus: 'radiology',
      retry: false,
      body: { language, analysis_scope: 'study_sample' },
    })
  ...
  for (const other of ['ru', 'en'] as const)
    await expect(page.locator('.rw-frame-results')).not.toContainText(imageProse[other])
