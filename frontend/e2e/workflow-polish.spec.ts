...
async function openNewCase(page: Page) {
  ...
  const created = await page.request.post('/api/v1/cases', {
    ...
    data: { full_name: 'Synthetic Polish Patient ' + crypto.randomUUID().slice(0, 8), age: 45, sex: 'male', summary: 'Synthetic workflow recovery test.' },
  })
...
test('risk units clear stale numbers and confirmation; valid explicit inputs reach the real calculator', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: /Калькулятор риска/ })).toHaveAttribute('aria-selected', 'true')
  ...
  await hdl.fill('50')
  const confirmation = page.getByRole('checkbox', { name: 'Подтверждаю показатели, единицы и факторы риска.' })
  ...
  await hdl.fill('1.3')
  for (const label of ['Курит сейчас', 'Сахарный диабет', 'Получает лечение гипертензии', 'Сердечно-сосудистое заболевание уже установлено']) {
...
test('note draft failure can be retried, and successful note stays saved when draft cleanup fails', async ({ page }) => {
  const c = await openNewCase(page)
  let getAttempts = 0
  await page.route(`**/api/v1/cases/${c.id}/drafts/note`, async route => {
    const request = route.request()
    if (request.method() === 'GET' && getAttempts++ === 0) {
      await route.fulfill({ status: 503, json: { error: { code: 'TEMPORARY_DRAFT_FAILURE', message: 'Synthetic draft outage' } } })
    } else if (request.method() === 'PUT' && request.postDataJSON().text === '') {
      await route.fulfill({ status: 503, json: { error: { code: 'TEMPORARY_DRAFT_FAILURE', message: 'Synthetic cleanup outage' } } })
  ...
  await expect(page.getByText(/Запись сохранена\. Не удалось очистить личный черновик/)).toBeVisible()
})
