...
test('standalone DICOM workspace pans, resets, navigates slices and persists manual/document reports', async ({
  page,
}) => {
  const errors: string[] = []
  ...
  const created = await page.request.post('/api/v1/cases', {
    ...
    data: {
      full_name: `Synthetic Radiology Report ${crypto.randomUUID().slice(0, 8)}`,
      age: 44,
      summary: 'Synthetic geometric phantom for interface testing only.',
    },
  })
  ...
  await expect
    .poll(() =>
      pixels.evaluate((image: HTMLImageElement) => image.complete && image.naturalWidth === 256),
    )
    .toBe(true)
  ...
  await expect
    .poll(() =>
      plane.evaluate((element) => new DOMMatrixReadOnly(getComputedStyle(element).transform).m11),
    )
    .toBeCloseTo(1.1)
  ...
  const manual =
    'Synthetic manual radiologist report. Geometric phantom only; no patient diagnosis.'
  const reportField = page.getByLabel('Описание и заключение', { exact: true })
  await reportField.fill(manual)
  const savedManualResponse = page.waitForResponse(
    (response) =>
      response.request().method() === 'POST' &&
      /\/imaging-studies\/[^/]+\/reports$/.test(new URL(response.url()).pathname),
  )
  ...
  const documentText =
    'Synthetic document radiologist report. Original geometric test phantom with twelve frames. No medical interpretation.'
  await documentUpload.locator('input[type=file]').setInputFiles({
    name: 'synthetic-radiologist-report.txt',
    mimeType: 'text/plain',
  ...
  const savedDocumentResponse = page.waitForResponse(
    (response) =>
      response.request().method() === 'POST' &&
      /\/imaging-studies\/[^/]+\/reports$/.test(new URL(response.url()).pathname),
  )
  ...
  const persisted = await (
    await page.request.get(`/api/v1/imaging-studies/${manualRecord.study_id}/reports`)
  ).json()
  ...
  const status = await (await page.request.get('/api/v1/system/status')).json()
  expect(status.model.ready).toBe(false)
  expect(status.model.reason).toBe('MODEL_WEIGHTS_MISSING')
  await expect(
    page.getByRole('button', { name: 'Сравнить заключение с выборкой', exact: true }),
  ).toBeDisabled()
  await expect(
