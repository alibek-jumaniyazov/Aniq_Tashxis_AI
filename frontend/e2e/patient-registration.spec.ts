...
const locales = [
  { code: 'ru', create: 'Новый случай', name: 'ФИО пациента', age: 'Возраст', sex: 'Пол', phone: 'Номер телефона пациента', comments: 'Комментарии врача', save: 'Сохранить', error: 'Укажите ФИО пациента.' },
  { code: 'uz', create: 'Yangi holat', name: 'Bemorning F.I.Sh.', age: 'Yosh', sex: 'Jins', phone: 'Bemorning telefon raqami', comments: 'Shifokor izohlari', save: 'Saqlash', error: 'Bemorning F.I.Sh. ni kiriting.' },
  { code: 'en', create: 'New case', name: 'Patient full name', age: 'Age', sex: 'Sex', phone: 'Patient phone number', comments: 'Doctor comments', save: 'Save', error: 'Enter the patient’s full name.' },
]
...
for (const locale of locales) {
  test(`patient registration ${locale.code}: only full name is required and code is generated`, async ({ page }) => {
    ...
    await expect(dialog.locator('input[id$="alias"], input[id$="diagnosis"], textarea[id$="diagnosis"]')).toHaveCount(0)
    await expect(dialog.locator('.ant-form-item')).toHaveCount(5)

    const creationRequests: string[] = []
    page.on('request', request => {
      if (request.method() === 'POST' && new URL(request.url()).pathname === '/api/v1/cases') creationRequests.push(request.postData() || '')
    })
    ...
    const createdResponse = page.waitForResponse(response => new URL(response.url()).pathname === '/api/v1/cases' && response.request().method() === 'POST')
    await dialog.getByRole('button', { name: locale.save, exact: true }).click()
    const response = await createdResponse
    expect(response.status()).toBe(201)
    const patient = await response.json()
    expect(patient).toMatchObject({ full_name: fullName, age: null, sex: 'unknown', patient_phone: '', summary: '' })
...
test('patient details persist, code stays immutable, and name, phone and code find the patient', async ({ page }) => {
  await signIn(page)
  const fullName = `Synthetic Patient Registration ${crypto.randomUUID().slice(0, 8)}`
  const phone = '+998 (90) 123-45-67'
  const comments = 'Synthetic registration scenario. These comments are not a confirmed clinical fact.'
  ...
  await page.locator('.ant-select-item-option-content').getByText('Женский', { exact: true }).click()
  ...
  expect(response.status()).toBe(201)
  const patient = await response.json()
  expect(patient).toMatchObject({ full_name: fullName, age: 0, sex: 'female', patient_phone: phone, summary: comments })
  ...
  const editedResponse = page.waitForResponse(result => new URL(result.url()).pathname === `/api/v1/cases/${patient.id}` && result.request().method() === 'PATCH')
  await dialog.getByRole('button', { name: 'Сохранить', exact: true }).click()
  expect((await editedResponse).status()).toBe(200)
  await expect(dialog).toBeHidden()
  await page.reload()
  await expect(page.getByRole('heading', { level: 1, name: updatedName, exact: true })).toBeVisible()
  const saved = await (await page.request.get(`/api/v1/cases/${patient.id}`)).json()
  expect(saved).toMatchObject({ full_name: updatedName, age: 35, sex: 'female', patient_phone: updatedPhone, summary: updatedComments, alias: patient.alias })
  ...
  for (const query of [updatedName, updatedPhone, patient.alias]) {
    ...
    await expect(page.getByRole('row').filter({ hasText: updatedName })).toContainText(patient.alias)
  }
  await page.setViewportSize({ width: 390, height: 844 })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1)).toBe(true)
})

test('legacy patient without a name can update comments without losing code or diagnosis', async ({ page }) => {
  await signIn(page)
  ...
  expect(legacy, 'The migrated isolated E2E database retains a legacy patient without a stored name.').toBeTruthy()
  const id = legacy!.id
  ...
  try {
    ...
    await expect(page.getByRole('heading', { level: 1, name: original.alias, exact: true })).toBeVisible()
    await page.getByRole('button', { name: 'Редактировать пациента', exact: true }).click()
    ...
    const update = page.waitForResponse(response => new URL(response.url()).pathname === `/api/v1/cases/${id}` && response.request().method() === 'PATCH')
    await dialog.getByRole('button', { name: 'Сохранить', exact: true }).click()
    ...
    expect(saved).toMatchObject({ full_name: '', alias: original.alias, diagnosis: original.diagnosis, summary: comment })
  } finally {
