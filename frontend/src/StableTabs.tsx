...
function tabTarget(event: MouseEvent<HTMLDivElement>) {
  const tab =
    event.target instanceof Element
      ? event.target.closest<HTMLElement>('.ant-tabs-tab-btn[role="tab"]')
      : null
  return tab?.closest('.ant-tabs') === event.currentTarget &&
...
export default function StableTabs({ onMouseDownCapture, onClickCapture, ...props }: TabsProps) {
  ...
  return (
    <Tabs
      {...props}
      locale={{ dropdownAriaLabel: t('tabsOverflow'), ...props.locale }}
      more={{ trigger: 'click', ...props.more }}
