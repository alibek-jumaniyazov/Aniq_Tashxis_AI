    ...
    <section className="settings-card">
      ...
      <div className="settings-identity">
        ...
        <div>
          ...
          <Tag>{t(user.role === 'expert' ? 'expertRole' : user.role)}</Tag>
        </div>
      </div>
      <LocalizedForm
        ...
        onFinish={(values) => save.mutate(values.name)}
      >
        <Form.Item
          name="name"
          label={t('sh_name')}
        ...
        <div className="settings-readonly-grid">
          <Form.Item label={t('email')} htmlFor="settings-profile-email">
            <Input id="settings-profile-email" value={user.email} readOnly aria-readonly="true" />
          </Form.Item>
          <Form.Item label={t('permission')} htmlFor="settings-profile-role">
            <Input
              id="settings-profile-role"
              value={t(user.role === 'expert' ? 'expertRole' : user.role)}
              readOnly
              aria-readonly="true"
            />
          </Form.Item>
...
function SecuritySettings({ user }: { user: User }) {
  ...
  const sessions = useQuery({
    ...
    queryFn: ({ signal }) => get<Sessions>('/auth/sessions', signal),
    refetchInterval: 30000,
  })
  const password = useMutation({
    mutationFn: ({ current_password, new_password }: PasswordValues) =>
      post<SessionAction>('/auth/password', { current_password, new_password }),
    onSuccess: () => {
      form.resetFields()
    ...
    <div className="settings-stack">
      <section className="settings-card">
        <div className="settings-section-heading">
          ...
          <div>
            <h2>{t('sh_changePassword')}</h2>
            <p>{t('sh_passwordHint')}</p>
          </div>
        </div>
        <LocalizedForm
          className="settings-form"
          form={form}
          layout="vertical"
          disabled={password.isPending || revoke.isPending}
          onValuesChange={() => password.reset()}
          onFinish={(values) => password.mutate(values)}
        >
          <Form.Item
            name="current_password"
            label={t('sh_currentPassword')}
            rules={[{ required: true }]}
          >
          ...
          <Form.Item
            name="new_password"
            label={t('sh_newPassword')}
            rules={[{ required: true, min: 10, max: 200, message: t('sh_passwordLength') }]}
          >
            <Input.Password autoComplete="new-password" maxLength={200} />
          </Form.Item>
          <Form.Item
            name="confirm_password"
            label={t('sh_confirmPassword')}
              ...
              ({ getFieldValue }) => ({
                validator: (_, value: string) =>
                  !value || getFieldValue('new_password') === value
                    ? Promise.resolve()
                    : Promise.reject(new Error(t('sh_passwordMismatch'))),
              }),
            ]}
          >
            <Input.Password autoComplete="new-password" maxLength={200} />
      ...
      <section className="settings-card">
        <div className="settings-section-heading">
          ...
          <div>
            <h2>{t('sh_sessions')}</h2>
            <p>{t('sh_sessionsHint')}</p>
          </div>
        </div>
        {sessions.isPending ? (
          <Loading />
        ) : sessions.isError ? (
          <Failure error={sessions.error} retry={() => void sessions.refetch()} />
        ) : (
          <>
            <dl className="settings-session-stats">
              <div>
                <dt>{t('sh_currentSession')}</dt>
                <dd>
                  <ShieldCheck size={18} /> {t('active')}
                </dd>
              </div>
              <div>
                <dt>{t('sh_otherSessions')}</dt>
                <dd>{sessions.data.other_sessions}</dd>
              </div>
              <div>
                <dt>{t('sh_sessionExpires')}</dt>
                <dd>{time(sessions.data.current_expires_at)}</dd>
              </div>
            </dl>
            {revoke.isError && (
              <Alert
                ...
                type="error"
                showIcon
                message={errorText(revoke.error)}
              />
            )}
            {revoke.isSuccess && (
              <Alert
                ...
                type="success"
                showIcon
                message={t('sh_sessionsRevoked')}
              />
            )}
            <Button
              onClick={() => revoke.mutate()}
              loading={revoke.isPending}
              disabled={!sessions.data.other_sessions || password.isPending}
            >
              {t('sh_revokeSessions')}
            </Button>
          </>
  ...
  return (
    <section className="settings-card">
      <div className="settings-section-heading">
        ...
        <div>
          <h2>{t('sh_interface')}</h2>
          <p>{t('sh_interfaceHint')}</p>
        </div>
      </div>
      <div className="settings-preference settings-language-preference">
        <div>
          <strong id="settings-language-label">{t('sh_language')}</strong>
          <p id="settings-language-hint">{t('sh_languageHint')}</p>
        </div>
        <div
        ...
        >
          {[
            ...
            <label
              key={language.value}
              className={i18n.resolvedLanguage === language.value ? 'selected' : ''}
            >
              <input
                type="radio"
                name="settings-language"
                value={language.value}
                checked={i18n.resolvedLanguage === language.value}
                onChange={() => {
      ...
      <div className="settings-preference">
        <div>
          <label id="settings-motion-label">{t('sh_reduceMotion')}</label>
          <p id="settings-motion-hint">{t('sh_reduceMotionHint')}</p>
        </div>
        <Switch
      ...
      <p className="settings-autosaved">
        <Check size={15} />
        {t('sh_browserPreference')}
      </p>
    </section>
  )
}
  ...
  return (
    <div className="settings-stack">
      <section className="settings-card settings-support-card">
        ...
        >
          @avilab_uz_support <ArrowUpRight size={17} />
        </a>
      </section>
      <section className="settings-card">
        <div className="settings-section-heading">
          ...
          <div>
            <h2>{t('sh_dmedTitle')}</h2>
            <Tag>{t('demoShort')}</Tag>
          </div>
        </div>
        <p className="settings-help">{t('sh_dmedHint')}</p>
      </section>
      <section className="settings-card">
        <h2>{t('sh_dataTitle')}</h2>
        <p className="settings-help">{t('sh_dataHint')}</p>
        <hr />
        <h2>{t('sh_aiTitle')}</h2>
        <p className="settings-help">{t('sh_aiHint')}</p>
      </section>
    </div>
  )
}
...
export default function SettingsHub({
  ...
  const sections = [
    ...
    ...(!['owner', 'developer'].includes(user.role) && workspaceAvailable
      ? [{ id: 'system', icon: Cpu }]
      : []),
  ...
  return (
    <div className="settings-hub">
      ...
      <div className="settings-layout">
        <nav className="settings-navigation" aria-label={t('sh_navigation')}>
          {sections.map(({ id, icon: Icon }) => (
            <NavLink key={id} to={`/settings/${id}`}>
              ...
              <span>
                <strong>{t(`sh_${id}`)}</strong>
                <small>{t(`sh_${id}Hint`)}</small>
              </span>
            </NavLink>
          ))}
        </nav>
        <div className={`settings-content settings-content-${section}`} key={section}>
