...
vi.mock('react-i18next', () => ({ useTranslation: () => ({ t: (key: string, values?: Record<string, unknown>) => key === 'rwCoverageCounts' ? `${values?.frames}/${values?.total} frames; ${values?.series}/${values?.allSeries} series` : key }) }))
afterEach(cleanup)

const frame = { ref: 'F1', series_id: 'series-2', series_number: 2, frame_index: 45, series_frames: 90, modality: 'MR', center: 80, width: 220 }
const run = { id: 'r', status: 'partial', result: { image_coverage: { total_frames: 180, total_series: 2, planned_frames: 1, reviewed_frames: 1, reviewed_series: 1, frames: [frame] }, image_review: { frame_assessments: [{ frame_ref: 'F1', quality: 'limited', observations: ['Visible geometric pattern.'], limitations: ['Only one frame was assessed.'] }] } } } as unknown as Run
...
it('distinguishes requested frames from successfully reviewed frames after rejection', () => {
  const result = { image_coverage: { total_frames: 180, total_series: 2, planned_frames: 1, reviewed_frames: 0, reviewed_series: 0, frames: [frame] }, image_review: null }
