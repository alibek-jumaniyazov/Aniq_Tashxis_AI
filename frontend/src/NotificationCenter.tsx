...
export default function NotificationCenter({ userId }: { userId: string }) {
  ...
  const feed = useQuery({
    queryKey,
    queryFn: ({ signal }) => get<NotificationFeed>('/notifications', signal),
  ...
  const readAll = useMutation({
    ...
    onSuccess: async () => {
      await refresh()
      void message.success(t('notificationsMarkedRead'))
    },
    onError: (error) => {
  ...
  const readOne = useMutation({
    mutationFn: (item: Notification) =>
      item.read_by?.includes(userId) ? Promise.resolve() : post(`/notifications/${item.id}/read`),
  ...
  return (
    <Popover
      ...
      afterOpenChange={(visible) => {
        if (visible) panel.current?.focus({ preventScroll: true })
      }}
      content={
        <section
          ref={panel}
        ...
        >
          <header className="notification-center__header">
            <span className="notification-center__symbol">
              <BellRing size={21} />
            </span>
            <div>
              ...
              <p aria-live="polite">
                {feed.isPending
                  ...
                  : feed.data
                    ? unread
                      ? t('notificationsUnreadCount', { count: unread })
                      : t('notificationsAllRead')
                    : t('notificationsActivity')}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="notification-center__toolbar">
            ...
            <Button
              ...
              disabled={!unread || busy || !feed.data}
              title={t('notificationsMarkAllHint')}
            >
          ...
          <div className="notification-center__scroll" aria-busy={feed.isPending || busy}>
            {feed.error && (
              <div className="notification-center__error" role="alert">
                <CircleAlert size={19} />
                <div>
                  <strong>{t('notificationsLoadError')}</strong>
                  <p>{errorText(feed.error)}</p>
                  <Button
                    size="small"
            ...
            {feed.isPending ? (
              ...
              <ul className="notification-center__items">
                {feed.data.items.map((item) => {
                  ...
                  return (
                    <li key={item.id}>
                      <button
                        type="button"
                        className={`notification-center__item${isUnread ? ' is-unread' : ''}`}
                        disabled={busy}
                        onClick={() => readOne.mutate(item)}
                      >
                        <span
                          className={`notification-center__status ${item.status === 'failed' || item.status === 'partial' ? 'needs-attention' : ''}`}
                          aria-hidden="true"
                        >
                          {reading ? (
                            <Spin size="small" />
                        ...
                        <span className="notification-center__copy">
                          <strong>
                            {item.case_alias || `${t('patient')} · ${item.case_id.slice(0, 8)}`}
                          </strong>
                          <span>
                            {t(item.status)}{' '}
                            <span className="notification-center__version">
                              · {t('version')} {item.case_version}
                            </span>
                          </span>
                          <time dateTime={item.created_at}>{date(item.created_at)}</time>
                        </span>
                        <span className="notification-center__item-end">
                          {isUnread && (
                            <span
                              className="notification-center__unread"
                              role="img"
          ...
          <footer className="notification-center__footer">
            {feed.data && feed.data.total > feed.data.items.length
              ? t('notificationsLatestCount', {
                  count: feed.data.items.length,
                  total: feed.data.total,
                })
              : t('notificationsOpenHint')}
          </footer>
        </section>
      }
    >
      <button
        ...
        aria-label={unread ? t('notificationsBellCount', { count: unread }) : t('notifications')}
