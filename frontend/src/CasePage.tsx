...
import type { Case, ClinicalAlert, Fact, Source, Status, User } from './types'
...
export default function CasePage({ user }: { user: User }) {
  ...
  const [quickComparisonRun, setQuickComparisonRun] = useState<string>()
  ...
  const clinicalEntries = useQuery({ queryKey: ['clinical-entries', id, query.data?.version], queryFn: ({ signal }) => get<ClinicalEntriesResponse>(`/cases/${id}/clinical-entries`, signal), enabled: !!query.data })
  ...
  const quickCompare = () => {
    ...
    setToolsOpen(false)
    if (!canEdit || activeRun) return
    void act(async () => {
      const records = await get<ClinicalEntriesResponse>(`/cases/${c.id}/clinical-entries`)
      if (!records.readiness.comparison_ready) { void message.info(t('pw_ready_hint')); return }
      const status = await get<Status>('/system/status')
      if (!status.model.ready) { void message.warning(t(status.model.reason || 'clinicalModelUnavailable')); return }
      if (!supportsClinicalModel(status.model)) { void message.warning(t('clinicalModelProfile')); return }
      const next = await post<{ run_id: string }>(`/cases/${c.id}/clinical-comparisons`, { expected_version: c.version, language })
      setQuickComparisonRun(next.run_id)
    }, false)
  }
  const openSource = (source: string) => setSourceId(source)
  const run = () => void act(async () => { await post(`/cases/${id}/analyses`, { expected_version: c.version, mode, decision_time: mode === 'decision_time' ? dateValue(cutoff) : null, include_ai: includeAI, language }); setSelectedRun(null); setTab('analysis') }, false)
