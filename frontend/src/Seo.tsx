...
function configuration(): SeoConfig {
  try {
    const parsed = JSON.parse(document.getElementById('seo-config')?.textContent || '{}') as Partial<SeoConfig>
    if (!parsed.origin) return { origin: '', indexing: false }
    const url = new URL(parsed.origin)
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.search || url.hash || url.pathname !== '/') return { origin: '', indexing: false }
    return { origin: url.origin, indexing: parsed.indexing === true }
  } catch { return { origin: '', indexing: false } }
}
...
function clearPublicMetadata() {
  document.head.querySelectorAll('link[rel="canonical"], link[hreflang], meta[property^="og:"], meta[name^="twitter:"], #seo-structured-data').forEach(element => element.remove())
}
...
export function updateMetadata(path: string, language: Language, config: SeoConfig = configuration()) {
  const copy = content[language]
  const publicPage = path === '/'
  const segment = path.split('/')[1] as keyof typeof copy.routes
  const routeLabel = Object.hasOwn(copy.routes, segment) ? copy.routes[segment] : copy.routes.workspace
  ...
  meta('name', 'robots', publicPage && config.indexing ? 'index, follow, max-image-preview:large' : 'noindex, nofollow, nosnippet, noarchive')
  clearPublicMetadata()
  ...
  structured.textContent = JSON.stringify({ '@context': 'https://schema.org', '@graph': [
    { '@type': 'Organization', '@id': `${config.origin}/#organization`, name: 'AniqTashxis.ai', url: `${config.origin}/`, logo: `${config.origin}/brand/aniqtashxis-mark.svg`, sameAs: ['https://t.me/avilab_uz_support'] },
    { '@type': 'WebSite', '@id': `${config.origin}/#website`, name: 'AniqTashxis.ai', url: `${config.origin}/`, inLanguage: languages, publisher: { '@id': `${config.origin}/#organization` } },
    { '@type': 'SoftwareApplication', name: 'AniqTashxis.ai', applicationCategory: 'HealthApplication', operatingSystem: 'Web', url: urlFor(language), description: copy.description, inLanguage: language, publisher: { '@id': `${config.origin}/#organization` } },
  ] })
...
export default function Seo() {
  ...
  const language = (languages.includes(i18n.resolvedLanguage as Language) ? i18n.resolvedLanguage : 'ru') as Language
  useEffect(() => { updateMetadata(pathname, language) }, [pathname, language, search])
  return null
}
