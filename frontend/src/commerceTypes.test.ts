...
beforeAll(async () => {
  await i18next.init({
    resources: {
      ru: { translation: commerceRu },
      uz: { translation: commerceUz },
      en: { translation: commerceEn },
    },
    lng: 'ru',
    fallbackLng: 'ru',
...
describe('billing localization without changing customer content', () => {
  ...
  it.each([
    ...
    ['uz', 'Klinika 10'],
    ['en', 'Clinic 10'],
  ])('localizes an unchanged default plan in %s', (language, name) => {
    expect(localizedPlan(defaults, language).name).toBe(name)
    expect(localizedPlan(defaults, language).description).toBe(
      i18next.t('commercePlanClinic10Description', { lng: language }),
    )
  })

  it('preserves administrator-authored fields independently and never translates unrelated plans', () => {
    expect(localizedPlan({ ...defaults, name: 'Shifo premium' }, 'en')).toEqual({
      name: 'Shifo premium',
      description: commerceEn.commercePlanClinic10Description,
    })
    expect(
      localizedPlan({ ...defaults, description: 'Individual kelishuv: 12 o‘rin.' }, 'ru'),
    ).toEqual({ name: 'Клиника 10', description: 'Individual kelishuv: 12 o‘rin.' })
    expect(localizedPlan({ ...defaults, id: 'custom-clinic' }, 'ru')).toEqual({
      name: defaults.name,
  ...
  it('translates shipped payment instructions but keeps custom instructions intact', () => {
    expect(localizedPaymentInstructions(commerceUz.commerceDefaultPaymentInstructions, 'en')).toBe(
      commerceEn.commerceDefaultPaymentInstructions,
    )
    expect(localizedPaymentInstructions('Call our accountant before paying.', 'uz')).toBe(
      'Call our accountant before paying.',
    )
  })
