...
export function useAction() {
  ...
  async function act<T>(action: () => Promise<T>, notify = true) {
    ...
    try {
      ...
      return result
    } catch (error) { void message.error(errorText(error)); return undefined }
    finally { pending.current = false; setBusy(false) }
  }
  return { act, busy }
}
export function Loading() { return <div className="loading"><Spin size="large"/></div> }
export function Failure({ error, retry }: { error: unknown; retry?: () => void }) { const { t } = useTranslation(); return <Alert type="error" showIcon message={errorText(error)} action={retry && <Button onClick={retry}>{t('retry')}</Button>}/> }
export function Blank({ title, hint }: { title?: string; hint?: string }) { const { t } = useTranslation(); return <div className="blank"><Empty image={<Files size={42} strokeWidth={1.25}/>} description={<><strong>{title || t('noData')}</strong><p>{hint || t('noDataHint')}</p></>}/></div> }
export function StateTag({ status }: { status: string }) {
  const { t } = useTranslation()
  const color = ({ succeeded: 'teal', confirmed: 'teal', accepted: 'teal', approved: 'teal', sent: 'teal', partial: 'amber', queued: 'violet', running: 'violet', new: 'amber', failed: 'red', rejected: 'red', closed: 'gray', not_started: 'gray', under_review: 'violet', awaiting_explanation: 'amber' } as Record<string, string>)[status] || 'gray'
  return <span className={`state-tag ${color}`}><i/>{t(status)}</span>
}
