  ...
  it('public copy stays provider-neutral and discloses external cloud processing in all languages', () => {
    ...
    for (const language of ['ru', 'uz', 'en'] as const) {
      ...
      expect(JSON.stringify(copy)).not.toMatch(
        /MedGemma|\b4B\b|локальная модель AI|Lokal AI modeli|Local AI model/i,
      )
      expect(JSON.stringify(content[language])).not.toMatch(/MedGemma|\b4B\b|local-only/i)
      expect(copy.landingProcessingNotice).toMatch(
        { ru: /внешний провайдер/, uz: /tashqi provayder/, en: /external provider/ }[language],
      )
      expect(content[language].limitation).toMatch(
        { ru: /внешний провайдер/, uz: /tashqi provayder/, en: /external provider/ }[language],
      )
    }
  })
  it.each(['ru', 'uz', 'en'] as const)(
    'publishes stable %s landing URLs and translated metadata',
    (language) => {
      ...
      expect(document.querySelector('link[rel="canonical"]')?.getAttribute('href')).toBe(
        `https://clinic.example.org/${language === 'ru' ? '' : `?lang=${language}`}`,
      )
      expect(document.querySelectorAll('link[hreflang]')).toHaveLength(4)
      expect(document.querySelector('meta[property="og:image"]')?.getAttribute('content')).toBe(
        'https://clinic.example.org/brand/aniqtashxis-social.png',
      )
      expect(
        JSON.parse(document.querySelector('#seo-structured-data')!.textContent!)['@graph'],
      ).toHaveLength(3)
    },
  )
  it.each([
    '/cases/patient-secret-id',
    '/cases/patient-secret-id/imaging',
    '/settings/profile',
    '/login',
    '/checkout',
    '/expert',
    '/reports',
    '/developer',
  ])('clears public cards and never publishes private data on %s', (path) => {
