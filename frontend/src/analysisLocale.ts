...
export interface AnalysisLanguageRun {
  ...
  include_ai?: boolean
  is_stale?: boolean
  result: { language?: string; provenance?: string; demo_only?: boolean; model_id?: string }
}
...
export function analysisOriginKey(run?: AnalysisLanguageRun): string {
  ...
  return run?.include_ai === false || run?.result.provenance === 'documentation_rules' ? 'aiRuleResult' : 'aiLocalResult'
}
