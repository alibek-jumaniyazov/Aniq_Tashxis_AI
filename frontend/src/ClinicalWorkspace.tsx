...
export default function ClinicalWorkspace({ c, user, openSource, openFacts }: { c: Case; user: User; openSource: (id: string) => void; openFacts: () => void }) {
  ...
  const [conclusionContext, setConclusionContext] = useState<{ version: number; runId: string | null }>({ version: c.version, runId: null })
  ...
  const launchReason = !canEdit ? t('clinicalOnlyDoctor')
    : pending ? t('clinicalPendingHint')
      : readiness.error ? t('clinicalReadyFailed')
      : readiness.isPending || !readiness.data ? t('clinicalReadyChecking')
        : readiness.data.clinical_population_eligible === false ? t('patientAgeForClinicalAI')
  ...
  const conclusion = (diagnosis = '', factIds: string[] = [], runId: string | null = null) => {
    form.resetFields()
    form.setFieldsValue({ diagnosis, status: 'provisional', fact_ids: factIds, clinician_confirmed: false })
  ...
  return <>
    ...
    <section className="panel clinical-workspace">
      ...
      {readiness.isPending ? <Loading/> : readiness.error ? <Failure error={readiness.error} retry={() => void readiness.refetch()}/> : readiness.data && <>
        ...
        {!!readiness.data.potential_conflicts.length && <Alert className="margin-bottom" type="warning" showIcon message={t('conflictingEvidenceHint')} description={readiness.data.potential_conflicts.map(conflict => <div key={conflict.key}><Tag>{conflict.key}</Tag>{conflict.fact_ids.map(id => { const fact = c.facts.find(item => item.id === id); return fact && <Button type="link" size="small" key={id} onClick={() => openSource(fact.source_id)}>{fact.value} {fact.unit}</Button> })}</div>)}/>}
      </>}
      {canEdit && model.error && <Failure error={model.error} retry={() => void model.refetch()}/>}
      <Space wrap className="margin-top">
        {canEdit && <Button type="primary" icon={<Sparkles size={16}/>} loading={busy} disabled={!!launchReason} onClick={launch}>{t('requestClinicalReview')}</Button>}
        ...
        {canEdit && pending && pending.id !== run?.id && <Button loading={busy} onClick={() => void act(() => post(`/analyses/${pending.id}/cancel`))}>{t('cancel')}</Button>}
      </Space>
      {launchReason && <p className="clinical-action-hint">{launchReason}</p>}
      {canEdit && model.data && !model.data.model.ready && model.data.model.reason && <p className="clinical-action-hint">{t(model.data.model.reason)}</p>}
      {canEdit && model.data && !model.data.model.ready && <Button className="margin-top" size="small" icon={<RefreshCw size={14}/>} loading={model.isFetching} onClick={() => void model.refetch()}>{t('refreshStatus')}</Button>}
    ...
    <section className="panel margin-top">
      ...
      {c.clinical_conclusions?.length ? c.clinical_conclusions.map(item => <article className="note-card" key={item.id}><div><Tag color={item.status === 'confirmed' ? 'green' : 'gold'}>{t(item.status === 'confirmed' ? 'clinicianConfirmed' : 'provisional')}</Tag><small>v{item.case_version} · {time(item.created_at)}</small></div><h3>{item.diagnosis}</h3><p>{item.rationale}</p><small>{item.author_name} · {t('evaluatedVersion')} {item.evaluated_version}</small></article>) : <div className="clinical-conclusion-empty"><h3>{t('noConclusions')}</h3><p>{t('clinicalConclusionsHint')}</p>{canEdit && <Button disabled={!confirmed.length || busy} onClick={() => conclusion()}>{t('recordConclusion')}</Button>}</div>}
    </section>
    <Modal title={t('recordConclusion')} open={open} onCancel={() => setOpen(false)} footer={null} width={640} closable={!busy} maskClosable={!busy} keyboard={!busy}>
      <Alert className="clinical-conclusion-help" type="info" showIcon message={t('clinicalConclusions')} description={t('clinicalConclusionHelp')}/>
      {conclusionChanged && <Alert className="margin-bottom" type="warning" showIcon message={t('clinicalConclusionChanged')}/>}
      <LocalizedForm form={form} layout="vertical" onFinish={values => {
        if (conclusionChanged) return
        void act(async () => {
          await post(`/cases/${c.id}/clinical-conclusions`, { ...values, expected_version: conclusionContext.version, run_id: conclusionContext.runId })
          setOpen(false)
        })
      }}>
        <Form.Item name="diagnosis" label={t('clinicalDiagnosis')} extra={t('clinicalDiagnosisHelp')} rules={[{ required: true, min: 3, whitespace: true }]}><Input.TextArea rows={2} maxLength={2000} showCount placeholder={t('clinicalDiagnosisPlaceholder')}/></Form.Item>
        <Form.Item name="status" label={t('status')} extra={t('clinicalStatusHelp')} rules={[{ required: true }]}><Select options={[{ value: 'provisional', label: t('provisional') }, { value: 'confirmed', label: t('clinicianConfirmed') }]}/></Form.Item>
        <Form.Item name="fact_ids" label={t('supportingEvidence')} extra={t('clinicalEvidenceHelp')} rules={[{ required: true, type: 'array', min: 1 }]}><Select mode="multiple" optionFilterProp="label" placeholder={t('supportingEvidence')} options={confirmed.map(fact => ({ value: fact.id, label: `${fact.label}: ${fact.value} ${fact.unit || ''}` }))}/></Form.Item>
        <Form.Item name="rationale" label={t('explanation')} extra={t('clinicalRationaleHelp')} rules={[{ required: true, min: 5, whitespace: true }]}><Input.TextArea rows={4} maxLength={4000} showCount placeholder={t('clinicalRationalePlaceholder')}/></Form.Item>
        ...
        <Button block type="primary" htmlType="submit" loading={busy} disabled={conclusionChanged}>{t('saveDecision')}</Button>
      </LocalizedForm>
    </Modal>
  </>
}
