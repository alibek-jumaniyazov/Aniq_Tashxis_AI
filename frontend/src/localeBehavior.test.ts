...
describe('language behavior', () => {
  ...
  it.each(['ru', 'uz', 'en'])('persists %s and updates document language and public language URL', async language => {
    await i18n.changeLanguage(language)
    expect(localStorage.getItem('aniq-language')).toBe(language)
    expect(document.documentElement.lang).toBe(language)
    expect(new URLSearchParams(window.location.search).get('lang')).toBe(language === 'ru' ? null : language)
  })
  ...
  it('localizes known catalog defaults while preserving custom clinic text', async () => {
    ...
    expect(localizedPlan({ id: 'solo', name: 'Doctor', description: 'Bir shifokor uchun shaxsiy ish maydoni.' }).description).not.toContain('shifokor')
    const custom = { id: 'solo', name: 'Special clinic plan', description: 'Contract wording entered by an administrator.' }
    expect(localizedPlan(custom)).toEqual({ name: custom.name, description: custom.description })
    expect(localizedPaymentInstructions('Custom bank instructions')).toBe('Custom bank instructions')
  })
  it.each(['ru', 'uz', 'en'])('translates API field validation and preserves constraints in %s', async language => {
    await i18n.changeLanguage(language)
    const result = errorText({ isAxiosError: true, response: { status: 422, data: { error: { code: 'VALIDATION_ERROR', message: 'Please check input', details: { fields: [{ field: 'body.age', type: 'greater_than_equal', message: 'Input should be greater than or equal to 18' }, { field: 'body.email', type: 'value_error', message: 'value is not a valid email address' }] } } } } })
  ...
  it('uses a localized fallback for unknown server errors and keeps the diagnostic code', async () => {
    ...
    const result = errorText({ isAxiosError: true, response: { status: 500, data: { error: { code: 'UNRECOGNIZED_SERVER_ERROR', message: 'Untranslated internal server details' } } } })
