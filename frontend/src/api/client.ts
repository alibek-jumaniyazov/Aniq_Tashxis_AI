...
export function errorText(error: unknown): string {
  if (axios.isAxiosError(error)) {
    ...
    if (payload) {
      const text = i18n.exists(payload.code) ? i18n.t(payload.code) : `${i18n.t('requestFailed')} (${payload.code})`
      const fields = payload.details?.fields as { field: string; message: string; type?: string }[] | undefined
      return fields?.length ? `${text} ${fields.map(field => {
        const name = field.field.split('.').filter(part => !/^(body|query|path|\d+)$/.test(part)).at(-1) || ''
        const label = i18n.exists(`field_${name}`) ? i18n.t(`field_${name}`) : i18n.t('value')
        return `${label}: ${validationText(field)}`
      }).join('; ')}` : text
    }
    return error.code === 'ERR_NETWORK' ? i18n.t('networkError') : error.code === 'ECONNABORTED' ? i18n.t('requestTimeout') : i18n.t(error.response?.status && error.response.status >= 500 ? 'serverError' : 'requestFailed')
  }
  return error instanceof Error && !/^Request failed|^Network Error|^timeout of /i.test(error.message) ? error.message : i18n.t('requestFailed')
}
function validationText(field: { message: string; type?: string }): string {
  const type = field.type || ''
  const key = ({ missing: 'validationRequired', string_too_short: 'validationShort', string_too_long: 'validationLong', greater_than_equal: 'validationMin', less_than_equal: 'validationMax', int_parsing: 'validationInteger', int_type: 'validationInteger', float_parsing: 'validationNumber', float_type: 'validationNumber', bool_parsing: 'validationBool', bool_type: 'validationBool' } as Record<string, string>)[type]
  ...
  if (key) return i18n.t(key.includes('Short') || key.includes('Long') || key.includes('Min') || key.includes('Max') ? value ? key : 'invalidValue' : key, { value })
