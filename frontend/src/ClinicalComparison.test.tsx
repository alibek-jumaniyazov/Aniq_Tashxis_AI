...
const report: ComparisonReport = {
  case_version: 4,
  summary: 'The treatment decision needs clinician review.',
  status: 'requires_clinician_review',
  diagnosis_review: {
    status: 'consistent_with_data',
    summary: 'Diagnosis matches this limited record.',
    refs: ['E1'],
  },
  treatment_review: {
    status: 'needs_review',
    summary: 'Verify the allergy before prescribing.',
    refs: ['E1'],
  },
  ...
  questions: ['What was the prior reaction?'],
  next_steps: ['Check the original medication record.'],
  limitations: ['This is not a validated risk model.'],
  evidence: [
    {
      ref: 'E1',
      entry_id: 'entry-1',
      category: 'subjective',
      text: 'Patient-reported allergy.',
      source_id: 'source-42',
    },
  ],
  five_year_outlook: {
    status: 'qualitative_only',
    summary: 'Long-term outcomes depend on follow-up.',
    scenarios: [
      {
        scenario: 'Stable symptoms with monitoring',
        conditions: 'Follow-up confirms treatment response.',
        monitoring: 'Review changes in symptoms.',
        refs: ['E1'],
      },
    ],
  },
...
describe('Clinical comparison evidence and result states', () => {
  ...
  it.each(['queued', 'running', 'failed', 'cancelled'])(
    'hides a leftover result when the run is %s',
    (status) => {
      render(<ComparisonResult run={analysis({ status })} openSource={vi.fn()} />)
      expect(screen.queryByText(report.summary)).toBeNull()
      expect(screen.queryByText(report.diagnosis_review.summary)).toBeNull()
      expect(screen.getByTestId('clinical-comparison-result').getAttribute('aria-busy')).toBe(
        String(['queued', 'running'].includes(status)),
      )
    },
  )
      ...
      <ComparisonResult
        run={analysis({
          result: {
            comparison: {
              ...report,
              diagnosis_review: { ...report.diagnosis_review, refs: ['E99'] },
            },
          },
        })}
  ...
  it('exposes chronology and stopped medication status beside the original fact', () => {
    const fact = {
      ...
      text: 'Medication documented in the prior visit.',
    ...
    render(
      <ComparisonResult
        run={analysis({
          result: { comparison: { ...report, evidence: [...report.evidence, fact] } },
        })}
        openSource={vi.fn()}
      />,
    )
